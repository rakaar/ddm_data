...
# =============================================================================
def build_pyddm_model(V_A_base, theta_A, bound_slope, t_change):
    """
    Build a PyDDM model for one condition (one t_change).

    - Drift: V_A_base (constant)
    - Bound distance to upper:
      theta_A - bound_slope * max(0, t - t_change), clipped to avoid crossing
      the lower bound.
    """
...
# =============================================================================
def simulate_proactive_single_bound_bound_drop_no_saturate(
    V_A_base,
    theta_A,
    bound_slope,
    t_led,
...
def simulate_single_trial_fit():
    ...
    if np.random.random() < lapse_prob:
        ...
        rt = simulate_proactive_single_bound_bound_drop_no_saturate(
            param_means[0],
            param_means[1],
            param_means[2],
            t_led,
...
ax.axvline(
    ...
    linestyle=":",
    alpha=0.6,
    label=f"del_m_plus_del_LED={param_means[4]:.3f}",
)
...
ax.axvline(x=0.0, color="k", linestyle="--", alpha=0.5, label="LED onset")
ax.axvline(
    x=param_means[4], color="g", linestyle=":", alpha=0.6,
    label=f"del_m_plus_del_LED={param_means[4]:.3f}",
)
ax.set_xlabel("RT - t_LED (s)")
ax.set_ylabel("Rate (area = fraction)")
