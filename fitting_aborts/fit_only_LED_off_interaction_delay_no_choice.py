...
evidence-afferent delay t_E_aff as a linear function of ABL, abs_ILD, and a scaled
ABL x abs_ILD interaction term in milliseconds,
...
ignores observed choice by fitting the choice-collapsed RT density:
    delay_ms = (
        bias_ms
        + abl_coeff_ms_per_abl * ABL
        + abs_ild_coeff_ms_per_unit * abs_ILD
        + interaction_coeff_ms * ((ABL - 40) / 20) * (abs_ILD / 16)
    )
...
output_dir = SCRIPT_DIR / "norm_only_led_off_from_loaded_proactive_truncate_NOT_censor_interaction_delay_no_choice"
...
def vbmc_norm_tied_loglike_fn(params):
    ...
    delay_ms_by_condition = (
        ...
        + float(interaction_coeff_ms) * observed_delay_condition_pairs[:, 2]
    )
...
def vbmc_prior_norm_tied_fn(params):
    ...
    return (
        ...
        + trapezoidal_logpdf(
            interaction_coeff_ms,
            norm_tied_interaction_coeff_ms_bounds[0],
            norm_tied_interaction_coeff_ms_plausible_bounds[0],
            norm_tied_interaction_coeff_ms_plausible_bounds[1],
            norm_tied_interaction_coeff_ms_bounds[1],
        )
        + trapezoidal_logpdf(
            del_go,
...
norm_tied_interaction_coeff_ms_bounds = [-30, 30]
...
norm_tied_interaction_coeff_ms_plausible_bounds = [-10, 10]
...
exp_df["abl_abs_ild_interaction"] = ((exp_df["ABL"] - 40.0) / 20.0) * (exp_df["abs_ILD"] / 16.0)
...
print("Interaction term definition: ((ABL - 40.0) / 20.0) * (abs_ILD / 16.0)")
...
def fit_one_unit(unit_tag, df_unit, proactive_vp_path):
    ...
    observed_delay_condition_pairs = (
        df_valid_animal_truncated[["ABL", "abs_ILD", "abl_abs_ild_interaction"]]
        .drop_duplicates()
        .sort_values(["ABL", "abs_ILD", "abl_abs_ild_interaction"])
    ...
    x_0 = np.array([2.3, 100e-3, 3.0, 0.51, 100.0, -0.425, -0.55, 0.0, 0.13, 0.95])
    ...
    if unit_tag == "aggregate":
        base_name = (
            f"batch_{batch_name}_aggregate_ledoff_1_proactive_loaded_truncate_NOT_censor_interaction_delay_no_choice_{run_tag}"
        )
    else:
        base_name = (
            f"batch_{batch_name}_animal_{unit_tag}_ledoff_1_"
            f"proactive_loaded_truncate_NOT_censor_interaction_delay_no_choice_{run_tag}"
        )
    ...
    interaction_coeff_ms = float(vp_samples[:, 7].mean())
    del_go = float(vp_samples[:, 8].mean())
    rate_norm_l = float(vp_samples[:, 9].mean())
    Z_E = (w - 0.5) * 2 * theta_E
    ...
    posterior_mean_delay_ms_by_abl_abs_ild = {
        int(abl): {
            ...
            * get_t_E_aff_from_abl_abs_ild(
                abl=abl,
                abs_ild=abs_ild,
                abl_abs_ild_interaction=((float(abl) - 40.0) / 20.0) * (float(abs_ild) / 16.0),
                bias_ms=bias_ms,
                abl_delay_coeff_ms_per_abl=abl_delay_coeff_ms_per_abl,
                abs_ild_delay_coeff_ms_per_unit=abs_ild_delay_coeff_ms_per_unit,
                interaction_coeff_ms=interaction_coeff_ms,
    ...
    if save_corner_plot:
        ...
        corner_fig.suptitle(
            f"Normalized TIED Posterior ({unit_tag}, interaction delay, no choice)",
            y=1.02,
        )
    ...
    print(
        ...
        f"interaction_coeff_ms={interaction_coeff_ms:.5f}, "
    ...
    vbmc_norm_tied_results = {
        ...
        "interaction_coeff_ms_samples": vp_samples[:, 7],
    ...
    save_dict = {
        ...
        "fit_config": {
            ...
            "likelihood_mode": "proactive_lapse_only_no_trunc_right_truncated_interaction_delay_no_choice",
            ...
            "interaction_term_definition": "((ABL - 40) / 20) * (abs_ILD / 16)",
            "interaction_term_range_on_supported_grid": [-1.0, 1.0],
...
if fit_mode == "aggregate":
    print(
        ...
        "with interaction-delay no-choice likelihood."
    )
    fit_one_unit(
        unit_tag="aggregate",
    ...
    print(f"Running animal-wise interaction-delay no-choice fit for animals: {selected}")
