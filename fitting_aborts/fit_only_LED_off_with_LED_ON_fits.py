...
Default behavior is aggregate fitting (all filtered animals together) with corner plot saving.
"""
...
output_dir = SCRIPT_DIR / "norm_only_led_off_from_loaded_proactive"
...
def fit_one_unit(unit_tag, df_unit, proactive_vp_path):
    global V_A, theta_A, t_A_aff, lapse_prob, beta_lapse, df_valid_animal_less_than_1
    ...
    df_valid_animal_less_than_1 = df_valid_unit[df_valid_unit["RTwrtStim"] < max_rtwrtstim_for_fit].copy()

    if len(df_valid_animal_less_than_1) == 0:
        print(f"Skipping {unit_tag}: no valid trials after filtering")
        return

    print(
        f"{unit_tag}: fitting with {len(df_valid_animal_less_than_1)} "
        f"valid trials (RTwrtStim < {max_rtwrtstim_for_fit}s)"
    )
    ...
    if unit_tag == "aggregate":
        base_name = f"batch_{batch_name}_aggregate_ledoff_1_proactive_loaded"
    else:
        base_name = f"batch_{batch_name}_animal_{unit_tag}_ledoff_1_proactive_loaded"
    ...
    save_dict = {
        ...
        "fit_config": {
            ...
            "likelihood_mode": "proactive_lapse_only_no_trunc",
            "censoring_rule": "if RT - t_stim > censor_rt_wrt_stim_s, use log(1 - CDF(t_stim + censor_rt_wrt_stim_s))",
