...
output_dir = SCRIPT_DIR / "norm_only_led_off_from_loaded_proactive_truncate_NOT_censor_linear_delay_no_choice"
...
def fit_one_unit(unit_tag, df_unit, proactive_vp_path):
    ...
    if unit_tag == "aggregate":
        base_name = (
            f"batch_{batch_name}_aggregate_ledoff_1_proactive_loaded_truncate_NOT_censor_linear_delay_no_choice_{run_tag}"
        )
    else:
        base_name = (
            f"batch_{batch_name}_animal_{unit_tag}_ledoff_1_"
            f"proactive_loaded_truncate_NOT_censor_linear_delay_no_choice_{run_tag}"
        )
    ...
    if save_corner_plot:
        ...
        corner_fig.suptitle(f"Normalized TIED Posterior ({unit_tag}, linear delay, no choice)", y=1.02)
    ...
    save_dict = {
        ...
        "fit_config": {
            ...
            "likelihood_mode": "proactive_lapse_only_no_trunc_right_truncated_linear_delay_no_choice",
...
if fit_mode == "aggregate":
    print("Running aggregate fit (single fit across all filtered LED-OFF trials) with linear-delay no-choice likelihood.")
    fit_one_unit(
        unit_tag="aggregate",
    ...
    print(f"Running animal-wise linear-delay no-choice fit for animals: {selected}")
