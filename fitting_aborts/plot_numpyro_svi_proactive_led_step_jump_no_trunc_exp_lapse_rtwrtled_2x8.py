# %%
"""Compare animal-wise-mixture and aggregate no-truncation lapse theory."""
...
SUMMARY_DIR = AGGREGATE_FIT_ROOT / "summary_figures"
SUMMARY_DIR.mkdir(parents=True, exist_ok=True)
FIG_PATH = SUMMARY_DIR / "led7_no_trunc_exp_lapse_rtwrtled_six_fits_vs_aggregate_2x8.png"
PAYLOAD_PATH = SUMMARY_DIR / "led7_no_trunc_exp_lapse_rtwrtled_six_fits_vs_aggregate_2x8.pkl"
...
for animal in ANIMALS:
    selected_fit_root = AUDIT_FIT_ROOT if animal in AUDIT_ANIMALS else BASE_FIT_ROOT
    fit_dir = selected_fit_root / f"LED7_{animal}"
    ...
    animal_payload = {
        "animal": animal,
        "fit_root": str(selected_fit_root.resolve()),
...
# %%
# =============================================================================
# Same pooled data with one shared aggregate posterior-mean parameter vector
# =============================================================================
aggregate_fit_dir = AGGREGATE_FIT_ROOT / "LED7_all_animals"
aggregate_summary_path = aggregate_fit_dir / "run_summary.json"
aggregate_posterior_path = aggregate_fit_dir / "main_fullrank_posterior_samples.npz"
if not aggregate_summary_path.exists() or not aggregate_posterior_path.exists():
    raise FileNotFoundError(
        f"Missing aggregate fit artifacts: {aggregate_fit_dir}"
    )

aggregate_summary = json.loads(aggregate_summary_path.read_text())
if aggregate_summary.get("status") not in {"complete", "max_steps_restore_best"}:
    raise RuntimeError(
        f"Aggregate fit status is {aggregate_summary.get('status')}."
    )
aggregate_config = aggregate_summary.get("config", {})
if aggregate_config.get("truncation") != "none":
    raise RuntimeError("Aggregate fit did not use the no-truncation likelihood.")
if aggregate_config.get("led_on_scope") != "all LED_trial == 1 rows":
    raise RuntimeError("Aggregate fit did not use the historical FCT LED-ON scope.")
if aggregate_config.get("pooling") != "trial_weighted":
    raise RuntimeError("Aggregate fit was not pooled by trial contributions.")
if int(aggregate_summary["trial_counts"]["total"]) != len(source_df):
    raise RuntimeError("Aggregate fit and plotting data contain different row counts.")

with np.load(aggregate_posterior_path) as posterior:
    missing = sorted(set(svi_utils.PARAM_NAMES) - set(posterior.files))
    if missing:
    ...
    if any(
        ...
        raise RuntimeError("Aggregate posterior contains non-finite values.")

aggregate_rng = np.random.default_rng(MC_SEED + 1000000)
aggregate_sampled_indices = aggregate_rng.integers(
    0, len(pooled_timing_pool), size=N_MC_TIMING_PAIRS
)
aggregate_sampled_timing = pooled_timing_pool.iloc[aggregate_sampled_indices]
aggregate_payload = {
    "label": "All animals aggregate fit",
    "fit_root": str(AGGREGATE_FIT_ROOT.resolve()),
    "fit_dir": str(aggregate_fit_dir.resolve()),
    "status": aggregate_summary["status"],
    "stop_reason": aggregate_summary["stop_reason"],
    "completed_steps": int(aggregate_summary["completed_steps"]),
    "restored_best_step": int(aggregate_summary["restored_best_step"]),
    "posterior_means": aggregate_params,
    "theory_timing_pool_n": int(len(pooled_timing_pool)),
    "theory_mc_paired_draws": N_MC_TIMING_PAIRS,
    "theory_sampled_t_LED_s": aggregate_sampled_timing[
        "t_LED"
    ].to_numpy(dtype=float),
    "theory_sampled_t_stim_s": aggregate_sampled_timing[
        "t_stim"
    ].to_numpy(dtype=float),
    "conditions": {},
}
...
for condition, led_on in (("on", True), ("off", False)):
    pooled_condition = pooled_payload["conditions"][condition]
    aggregate_theory = no_trunc_lapse_theory_density(
        aggregate_payload["theory_sampled_t_LED_s"],
        aggregate_payload["theory_sampled_t_stim_s"],
        aggregate_params,
        led_on=led_on,
    )
    aggregate_abort_probability = model_abort_probability(
        aggregate_payload["theory_sampled_t_LED_s"],
        aggregate_payload["theory_sampled_t_stim_s"],
        aggregate_params,
        led_on=led_on,
    ...
    aggregate_payload["conditions"][condition] = {
        "n_total": pooled_condition["n_total"],
        "n_abort": pooled_condition["n_abort"],
        "abort_fraction": pooled_condition["abort_fraction"],
        "model_abort_probability_mc": aggregate_abort_probability,
        "rt_wrt_led_s": pooled_condition["rt_wrt_led_s"].copy(),
        "regular_x_s": pooled_condition["regular_x_s"].copy(),
        "regular_edges_s": pooled_condition["regular_edges_s"].copy(),
        "regular_data_density": pooled_condition["regular_data_density"].copy(),
        "zoom_x_s": pooled_condition["zoom_x_s"].copy(),
        "zoom_edges_s": pooled_condition["zoom_edges_s"].copy(),
        "zoom_data_density": pooled_condition["zoom_data_density"].copy(),
        "theory_x_s": THEORY_X_S.copy(),
        "theory_density": aggregate_theory,
    }
...
# =============================================================================
columns = [per_animal[animal] for animal in ANIMALS] + [
    pooled_payload,
    aggregate_payload,
]
column_titles = [f"LED7/{animal}" for animal in ANIMALS] + [
    "Pooled data\nsix-fit mixture",
    "Pooled data\naggregate fit",
]

fig, axes = plt.subplots(2, 8, figsize=(27.2, 7.0), sharey="row")
...
fig.suptitle(
    "LED7 all-ON no-truncation exponential-lapse SVI: six fits versus aggregate fit",
    fontsize=13,
    y=0.995,
)
fig.subplots_adjust(
...
payload = {
    "config": {
        ...
        "base_fit_root": str(BASE_FIT_ROOT.resolve()),
        "audit_fit_root": str(AUDIT_FIT_ROOT.resolve()),
        "audit_animals": AUDIT_ANIMALS,
        "aggregate_fit_root": str(AGGREGATE_FIT_ROOT.resolve()),
        "animals": ANIMALS,
        "truncation": "none",
        ...
        "seventh_column": (
            "pooled observed trials and trial-count-weighted six-fit theory"
        ),
        "eighth_column": (
            "the same pooled observed trials with one shared aggregate-fit "
            "posterior-mean theory"
        ),
    },
    "per_animal": per_animal,
    "pooled_all_animals": pooled_payload,
    "aggregate_all_animals": aggregate_payload,
