...
OUTPUT_FIGURE_PATH = (
    SCRIPT_DIR / "led7_s9_led_on_abort_onset_distribution.png"
)
...
# %%
############ Pool LED ON fixation aborts and audit LED onset time ############
...
led_onset_time = led_on_aborts["LED_onset_time"].to_numpy(dtype=float)
if not np.isfinite(led_onset_time).all():
    raise RuntimeError("LED ON aborts contain non-finite LED_onset_time values.")
if not (
    (led_onset_time >= HISTOGRAM_BINS[0])
    & (led_onset_time <= HISTOGRAM_BINS[-1])
).all():
    raise RuntimeError("Some LED_onset_time values fall outside the histogram bins.")

counts, _ = np.histogram(led_onset_time, bins=HISTOGRAM_BINS)
...
ax.set_xlabel("LED_onset_time (s)")
ax.set_ylabel("Count")
ax.set_title(
    "All animals: LED_onset_time for aborts LED ON (LED7 sessiontype 9)"
)
...
print(f"LED ON fixation aborts plotted: {len(led_onset_time):,}")
print(f"Bin width: {1_000 * BIN_WIDTH_S:.0f} ms")
print(
    "LED_onset_time: "
    f"min={led_onset_time.min():.6f} s, "
    f"median={np.median(led_onset_time):.6f} s, "
    f"mean={led_onset_time.mean():.6f} s, "
    f"max={led_onset_time.max():.6f} s"
)
