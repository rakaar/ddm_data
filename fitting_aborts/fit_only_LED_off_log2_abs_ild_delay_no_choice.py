...
evidence-afferent delay t_E_aff as a linear function of ABL and log2(abs_ILD) in milliseconds,
rejects parameter sets whose implied delay is negative for any observed fit condition, and
ignores observed choice by fitting the choice-collapsed RT density:
    delay_ms = bias_ms + abl_coeff_ms_per_abl * ABL + log2_abs_ild_coeff_ms_per_unit * log2(abs_ILD)
...
output_dir = SCRIPT_DIR / "norm_only_led_off_from_loaded_proactive_truncate_NOT_censor_log2_abs_ild_delay_no_choice"
...
# %%
############ Helpers ############
def get_t_E_aff_from_abl_log2_abs_ild(
    abl,
    log2_abs_ild,
    bias_ms,
    abl_delay_coeff_ms_per_abl,
    log2_abs_ild_delay_coeff_ms_per_unit,
):
    delay_ms = (
        float(bias_ms)
        + float(abl_delay_coeff_ms_per_abl) * float(abl)
        + float(log2_abs_ild_delay_coeff_ms_per_unit) * float(log2_abs_ild)
    )
...
def compute_loglike_norm_fn(
    ...
    t_E_aff = get_t_E_aff_from_abl_log2_abs_ild(
        abl=row["ABL"],
        log2_abs_ild=row["log2_abs_ILD"],
        bias_ms=bias_ms,
        abl_delay_coeff_ms_per_abl=abl_delay_coeff_ms_per_abl,
        log2_abs_ild_delay_coeff_ms_per_unit=log2_abs_ild_delay_coeff_ms_per_unit,
    )
...
def vbmc_norm_tied_loglike_fn(params):
    ...
    delay_ms_by_condition = (
        ...
        + float(log2_abs_ild_delay_coeff_ms_per_unit) * np.log2(observed_delay_condition_pairs[:, 1])
    )
...
def vbmc_prior_norm_tied_fn(params):
    ...
    return (
        ...
        + trapezoidal_logpdf(
            log2_abs_ild_delay_coeff_ms_per_unit,
            norm_tied_log2_abs_ild_delay_coeff_ms_per_unit_bounds[0],
            norm_tied_log2_abs_ild_delay_coeff_ms_per_unit_plausible_bounds[0],
            norm_tied_log2_abs_ild_delay_coeff_ms_per_unit_plausible_bounds[1],
            norm_tied_log2_abs_ild_delay_coeff_ms_per_unit_bounds[1],
        )
        + trapezoidal_logpdf(
            del_go,
...
norm_tied_log2_abs_ild_delay_coeff_ms_per_unit_bounds = [-8, 2]
...
norm_tied_log2_abs_ild_delay_coeff_ms_per_unit_plausible_bounds = [-4, -0.4]
...
if (exp_df["abs_ILD"] <= 0).any():
    raise ValueError("log2(abs_ILD) delay model requires abs_ILD > 0 for all rows.")
exp_df["log2_abs_ILD"] = np.log2(exp_df["abs_ILD"])
...
print(f"Observed log2(abs_ILD) values in filtered data: {np.log2(observed_abs_ild_values).tolist()}")
...
def fit_one_unit(unit_tag, df_unit, proactive_vp_path):
    ...
    print(f"{unit_tag}: observed log2(abs_ILD) values -> {np.log2(observed_unit_abs_ild_values).tolist()}")
    ...
    x_0 = np.array([2.3, 100e-3, 3.0, 0.51, 100.0, -0.425, -2.2, 0.13, 0.95])
    ...
    if unit_tag == "aggregate":
        base_name = (
            f"batch_{batch_name}_aggregate_ledoff_1_proactive_loaded_truncate_NOT_censor_log2_abs_ild_delay_no_choice_{run_tag}"
        )
    else:
        base_name = (
            f"batch_{batch_name}_animal_{unit_tag}_ledoff_1_"
            f"proactive_loaded_truncate_NOT_censor_log2_abs_ild_delay_no_choice_{run_tag}"
        )
    ...
    log2_abs_ild_delay_coeff_ms_per_unit = float(vp_samples[:, 6].mean())
    ...
    posterior_mean_delay_ms_by_abl_abs_ild = {
        int(abl): {
            int(abs_ild): 1e3
            * get_t_E_aff_from_abl_log2_abs_ild(
                abl=abl,
                log2_abs_ild=np.log2(abs_ild),
                bias_ms=bias_ms,
                abl_delay_coeff_ms_per_abl=abl_delay_coeff_ms_per_abl,
                log2_abs_ild_delay_coeff_ms_per_unit=log2_abs_ild_delay_coeff_ms_per_unit,
            )
    ...
    if save_corner_plot:
        ...
        corner_fig.suptitle(
            f"Normalized TIED Posterior ({unit_tag}, log2(|ILD|) delay, no choice)",
            y=1.02,
        )
    ...
    print(
        ...
        f"log2_abs_ild_delay_coeff_ms_per_unit={log2_abs_ild_delay_coeff_ms_per_unit:.5f}, "
    ...
    vbmc_norm_tied_results = {
        ...
        "log2_abs_ild_delay_coeff_ms_per_unit_samples": vp_samples[:, 6],
    ...
    save_dict = {
        ...
        "fit_config": {
            ...
            "likelihood_mode": "proactive_lapse_only_no_trunc_right_truncated_log2_abs_ild_delay_no_choice",
            ...
            "delay_rule_ms": (
                ...
                "log2_abs_ild_coeff_ms_per_unit * log2(abs_ILD)"
            ),
            "delay_covariate_transform": "log2(abs_ILD)",
            "log2_abs_ild_coeff_prior_note": (
                "coefficient prior widened 4x versus the abs_ILD-delay variant "
                "to keep a similar total delay span over abs_ILD in [1, 16]"
            ),
            ...
            "observed_log2_abs_ILD_values": np.log2(observed_unit_abs_ild_values).tolist(),
...
if fit_mode == "aggregate":
    print(
        "Running aggregate fit (single fit across all filtered LED-OFF trials) "
        "with log2(|ILD|)-delay no-choice likelihood."
    )
    fit_one_unit(
        unit_tag="aggregate",
    ...
    print(f"Running animal-wise log2(|ILD|)-delay no-choice fit for animals: {selected}")
