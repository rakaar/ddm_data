...
output_dir = SCRIPT_DIR / "fit_delay_surface_from_5sigma_csv"
...
def build_quadratic_design_matrix(abl_values: np.ndarray, abs_ild_values: np.ndarray):
    return np.column_stack(
        [
            np.ones_like(abl_values, dtype=float),
            abl_values,
...
def format_quadratic_equation(coefficients: np.ndarray):
    return (
        f"delay_ms = {coefficients[0]:.4f} "
        f"+ ({coefficients[1]:.4f}) * ABL "
        f"+ ({coefficients[2]:.4f}) * abs_ILD "
        f"+ ({coefficients[3]:.4f}) * ABL^2 "
        f"+ ({coefficients[4]:.4f}) * abs_ILD^2 "
        f"+ ({coefficients[5]:.4f}) * ABL*abs_ILD"
    )
...
def predict_quadratic_delay_ms(abl_values: np.ndarray, abs_ild_values: np.ndarray, coefficients: np.ndarray):
    return build_quadratic_design_matrix(
        abl_values=np.asarray(abl_values, dtype=float),
        abs_ild_values=np.asarray(abs_ild_values, dtype=float),
    ) @ coefficients
...
# %%
############ Fit linear and quadratic delay surfaces ############
design_matrix_linear = build_linear_design_matrix(abl_values=abl_values, abs_ild_values=abs_ild_values)
design_matrix_quadratic = build_quadratic_design_matrix(abl_values=abl_values, abs_ild_values=abs_ild_values)
...
quadratic_coefficients, quadratic_pred_ms, quadratic_residual_ms, quadratic_rmse_ms, quadratic_r2 = solve_ols_and_metrics(
    design_matrix=design_matrix_quadratic,
    target_values=delay_values_ms,
)

linear_equation = format_linear_equation(linear_coefficients)
quadratic_equation = format_quadratic_equation(quadratic_coefficients)

print("Linear model equation:")
print(f"  {linear_equation}")
print("Quadratic model equation:")
print(f"  {quadratic_equation}")
...
metrics_df = pd.DataFrame(
    [
        ...
        },
        {
            ...
            "n_points": len(delay_long_df),
            "n_parameters": design_matrix_quadratic.shape[1],
            "rmse_ms": quadratic_rmse_ms,
            "r2": quadratic_r2,
        },
    ]
)
...
predictions_df["delay_ms_quadratic_pred"] = quadratic_pred_ms
predictions_df["linear_residual_ms"] = predictions_df["delay_ms_observed"] - predictions_df["delay_ms_linear_pred"]
predictions_df["quadratic_residual_ms"] = predictions_df["delay_ms_observed"] - predictions_df["delay_ms_quadratic_pred"]
...
quadratic_pred_matrix_df = (
    predictions_df
    .pivot(index="ABL", columns="abs_ILD", values="delay_ms_quadratic_pred")
    .reindex(index=observed_matrix_df.index.astype(float), columns=observed_matrix_df.columns.astype(float))
)
...
quadratic_residual_matrix_df = (
    predictions_df
    .pivot(index="ABL", columns="abs_ILD", values="quadratic_residual_ms")
    .reindex(index=observed_matrix_df.index.astype(float), columns=observed_matrix_df.columns.astype(float))
)
...
quadratic_pred_matrix_df.index = quadratic_pred_matrix_df.index.astype(int)
quadratic_pred_matrix_df.columns = quadratic_pred_matrix_df.columns.astype(int)
...
quadratic_residual_matrix_df.index = quadratic_residual_matrix_df.index.astype(int)
quadratic_residual_matrix_df.columns = quadratic_residual_matrix_df.columns.astype(int)

matrix_vmin = float(
    min(
        observed_matrix_df.to_numpy().min(),
        linear_pred_matrix_df.to_numpy().min(),
        quadratic_pred_matrix_df.to_numpy().min(),
    )
)
matrix_vmax = float(
    max(
        ...
        quadratic_pred_matrix_df.to_numpy().max(),
    )
)
residual_abs_max = float(
    max(
        np.abs(linear_residual_matrix_df.to_numpy()).max(),
        np.abs(quadratic_residual_matrix_df.to_numpy()).max(),
    )
)
...
matrix_plot_specs = [
    ...
    (axes[0, 2], quadratic_pred_matrix_df, "Quadratic Fit (ms)", "viridis", matrix_vmin, matrix_vmax),
    (axes[1, 0], linear_residual_matrix_df, "Linear Residual (ms)", "coolwarm", -residual_abs_max, residual_abs_max),
    (axes[1, 1], quadratic_residual_matrix_df, "Quadratic Residual (ms)", "coolwarm", -residual_abs_max, residual_abs_max),
]
...
scatter_ax.scatter(
    predictions_df["delay_ms_observed"],
    predictions_df["delay_ms_quadratic_pred"],
    s=56,
    color="tab:orange",
    alpha=0.85,
    marker="^",
    label=f"Quadratic (R^2 = {quadratic_r2:.3f})",
)
...
scatter_min = float(
    min(
        ...
        predictions_df["delay_ms_quadratic_pred"].min(),
    )
)
scatter_max = float(
    max(
        ...
        predictions_df["delay_ms_quadratic_pred"].max(),
    )
)
...
quadratic_title_text = format_equation_for_title(quadratic_equation)
...
for col_idx, abl in enumerate(observed_matrix_df.index.astype(int)):
    ...
    quadratic_grid_pred_ms = predict_quadratic_delay_ms(
        abl_values=np.full_like(abs_ild_grid, float(abl), dtype=float),
        abs_ild_values=abs_ild_grid,
...
fig_by_abl.text(
    0.5,
    1.00,
    f"Quadratic: {quadratic_title_text}",
    ha="center",
    va="center",
    fontsize=equation_fontsize,
)
