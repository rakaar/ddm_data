...
This variant uses true right-truncation at {cut off=115} ms after stimulus onset and allows the
evidence-afferent delay t_E_aff to depend on ABL:
    ABL=20 -> t_E_aff_20
    ABL=40 -> t_E_aff_40
    ABL=60 -> t_E_aff_60
"""
...
output_dir = SCRIPT_DIR / "norm_only_led_off_from_loaded_proactive_truncate_NOT_censor_ABL_delay"
...
def fit_one_unit(unit_tag, df_unit, proactive_vp_path):
    ...
    if unit_tag == "aggregate":
        base_name = (
            f"batch_{batch_name}_aggregate_ledoff_1_proactive_loaded_truncate_NOT_censor_ABL_delay"
        )
    else:
        base_name = (
            f"batch_{batch_name}_animal_{unit_tag}_ledoff_1_"
            "proactive_loaded_truncate_NOT_censor_ABL_delay"
        )
    ...
    if save_corner_plot:
        ...
        corner_fig.suptitle(f"Normalized TIED Posterior ({unit_tag}, ABL delay)", y=1.02)
    ...
    save_dict = {
        ...
        "fit_config": {
            ...
            "likelihood_mode": "proactive_lapse_only_no_trunc_right_truncated_ABL_delay",
