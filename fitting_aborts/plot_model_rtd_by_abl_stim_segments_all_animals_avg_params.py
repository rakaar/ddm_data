...
def load_data():
    ...
    try:
        for segment_spec in segment_specs:
            ...
            if MODEL_DENSITY_MODE == "valid_conditioned":
                t_pts, p_a_mean, c_a_mean = compute_segment_proactive_matrices(t_stim_samples, abort_params, trunc_time=agg_trunc)
            else:
                t_pts, p_a_mean, c_a_mean = compute_segment_proactive_curves(t_stim_samples, abort_params, trunc_time=agg_trunc)
            if progress_bar is not None:
                progress_bar.update(1)
