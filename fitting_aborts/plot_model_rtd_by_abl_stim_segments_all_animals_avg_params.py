...
SEGMENT_SPECS = [
    {"name": "Q1/2", "left": 0.2, "right": 0.4},
    {"name": "Q2/2", "left": 0.4, "right": 1.5},
]
...
output_base = output_dir / "model_rtd_by_abl_stim_segments_all_animals_avg_params"
...
def load_pooled_valid_df(candidate_pairs):
    ...
    if not frames:
        raise ValueError("No pooled valid trials found for eligible animals after filtering.")
...
def compute_segment_averaged_rtd(t_pts, p_a_mean, c_a_mean, abl_value, ild_value, tied_params):
    ...
    return t_pts[mask], up_mean[mask] + down_mean[mask]
...
def load_data():
    ...
    print(
        f"[progress] Loaded pooled valid dataframe with {len(pooled_valid_df)} rows in {time.perf_counter() - stage_start:.2f}s",
        flush=True,
    )
    ...
    if not included_pairs:
        raise ValueError("No eligible batch-animal pairs remained after filtering pooled valid trials.")
    print(
    ...
    try:
        for segment_spec in SEGMENT_SPECS:
            segment_df = pooled_valid_df[
                (pooled_valid_df["intended_fix"] >= segment_spec["left"])
                & (pooled_valid_df["intended_fix"] <= segment_spec["right"])
            ].copy()
            if len(segment_df) == 0:
                raise ValueError(
...
def plot_data(data):
    ...
    print(f"Filtered pooled valid trials: {len(data['valid_df'])}")
