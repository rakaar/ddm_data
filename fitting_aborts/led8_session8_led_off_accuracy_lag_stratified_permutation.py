...
summary_save_path = save_dir / "led8_session8_led_off_accuracy_lag_stratified_permutation_summary.csv"
null_save_path = save_dir / "led8_session8_led_off_accuracy_lag_stratified_permutation_null.csv"
fig_save_path = save_dir / "led8_session8_led_off_accuracy_lag_stratified_permutation.png"
...
df["correct"] = (df["success"] == 1).astype(float)


# %% Residualize accuracy within animal/session/ABL/abs_ILD strata
df["stratum_mean_accuracy"] = df.groupby(strata_cols)["correct"].transform("mean")
df["accuracy_resid"] = df["correct"] - df["stratum_mean_accuracy"]

accuracy_resid = df["accuracy_resid"].to_numpy()
...
observed_stats = compute_stats(accuracy_resid, lag_codes)
...
for perm_idx in range(n_permutations):
    ...
    perm_stats = compute_stats(accuracy_resid, permuted_labels)
    ...
    summary_rows.append(
        {
            ...
            "observed_percentage_points": 100 * observed_value,
            "null_mean_fraction": null_values.mean(),
            "null_std_fraction": null_values.std(ddof=1),
            "null_ci_low_fraction": np.percentile(null_values, 2.5),
            "null_ci_high_fraction": np.percentile(null_values, 97.5),
            "p_value": p_value,
        }
    )
...
print("\nRaw accuracy by lag group:")
print(df.groupby("lag_group")["correct"].mean().reindex(lag_order).to_string())
print("\nObserved residual accuracy by lag group, percentage points:")
for lag_name in lag_order:
    print(f"{lag_name}: {100 * df.loc[df['lag_group'] == lag_name, 'accuracy_resid'].mean():.3f}")
print("\nPermutation summary:")
print(summary_df[["stat", "observed_percentage_points", "p_value"]].to_string(index=False))
...
for ax, (stat_name, title) in zip(axes, plot_stats):
    ...
    if stat_name == "omnibus_between_lag":
        null_plot = null_values
        observed_plot = observed_value
        xlabel = "Permutation statistic"
    else:
        null_plot = 100 * null_values
        observed_plot = 100 * observed_value
        xlabel = "Residual accuracy contrast (percentage points)"
...
fig.suptitle("Stratified permutation test: LED-off accuracy residuals by lag from LED ON", fontsize=11, y=1.02)
fig.tight_layout()
