...
def run_analysis():
    ...
    # Get all unique animal IDs first
    exp_df_initial = pd.read_csv('/home/rlab/raghavendra/ddm_data/outExp.csv')
    exp_df_initial = exp_df_initial[~((exp_df_initial['TotalFixTime'].isna()) & (exp_df_initial['abort_event'] == 3))].copy()
    all_df_initial = exp_df_initial[(exp_df_initial['batch_name'] == 'Comparable')].copy()
    all_animal_ids = sorted(all_df_initial['animal'].unique())
    print(f"Found {len(all_animal_ids)} unique animals: {all_animal_ids}")
    ...
    for idx, T_max_lag in enumerate([1, 2, 3]): # Renamed T to T_max_lag for clarity
        ...
        for animal_id in all_animal_ids:
            ...
            # Reload data for each animal to ensure clean state
            exp_df = pd.read_csv('/home/rlab/raghavendra/ddm_data/outExp.csv')
            
            # Remove wrong rows 
            if animal_id == all_animal_ids[0]:  # Only print this once
