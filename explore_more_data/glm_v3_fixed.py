...
# This is the fixed function that includes animal_id in the join
def add_k_previous_trial_value_explicit_lookup(df: pd.DataFrame, 
    ...
    Args:
        ...
        animal_col (str): The name of the column identifying the animal.
                          Defaults to 'animal'.
    ...
    if not all(col in df.columns for col in [animal_col, session_col, trial_col, value_col_name]):
        ...
        missing_cols = [col for col in [animal_col, session_col, trial_col, value_col_name] if col not in df.columns]
        if missing_cols: # Raise error if any critical column is missing
            raise ValueError(f"Ensure '{animal_col}', '{session_col}', '{trial_col}', and '{value_col_name}' are columns in the DataFrame. Missing: {missing_cols}")
...
def run_analysis():
    ...
    for idx, T_max_lag in enumerate([1, 2, 3]): # Renamed T to T_max_lag for clarity
        results_auc = []
        # animal_summaries = [] # This was unused, consider removing if not planned for use.
        ...
        for animal_id in all_df['animal'].unique():
            ...
            for k_lag in range(1, T_max_lag + 1):
                for base_var_name in lagged_vars_to_create:
                    ...
                    # Use the fixed function to generate the lagged column.
                    # It returns a new DataFrame with the added column.
                    # The added column's name will be f"{base_var_name}_{k_lag}".
                    temp_df_with_lag = add_k_previous_trial_value_explicit_lookup(
                        df=animal_df, 
                        value_col_name=base_var_name, 
                        k=k_lag,
                        animal_col=ANIMAL_COL,  # Added animal column
                        session_col=SESSION_COL, # Explicitly pass session column
                        trial_col=TRIAL_COL      # Explicitly pass trial column
                    )
                    ...
                    else: # Should not happen if value_col_name existed
            ...
            # Original code filters here: animal_df = animal_df[animal_df['is_abort'] == 1].copy()
            # Applying it to animal_df_cleaned:
            animal_df_cleaned = animal_df_cleaned[animal_df_cleaned['is_abort'] == 1].copy()
                
            if animal_df_cleaned.empty:
                print(f"Animal {animal_id}, T_max_lag={T_max_lag} → No data remaining after NaN drop and abort filter. Skipping.")
            ...
            if len(sessions) < 2 and len(sessions) > 0 : # Need at least 2 sessions for a split, or handle single session case
                 print(f"Animal {animal_id}, T_max_lag={T_max_lag} → Only one session ({sessions[0]}) with data. Using all for training, no test set.")
                 train_sessions = sessions
                 test_sessions = [] # No sessions for testing
            elif len(sessions) == 0:
                 print(f"Animal {animal_id}, T_max_lag={T_max_lag} → No sessions with data. Skipping.")
                 results_auc.append({'animal': animal_id, 'train_auc': np.nan, 'test_auc': np.nan})
                 continue
            else:
            ...
            if X_train.empty or y_train.empty:
                print(f"Animal {animal_id}, T_max_lag={T_max_lag} → Training data is empty. Skipping model fit.")
                results_auc.append({'animal': animal_id, 'train_auc': np.nan, 'test_auc': np.nan})
                continue
            if y_train.nunique() < 2:
                print(f"Animal {animal_id}, T_max_lag={T_max_lag} → Training target 'short_poke' has only one class. Skipping model fit.")
                results_auc.append({'animal': animal_id, 'train_auc': np.nan, 'test_auc': np.nan})
                continue
            ...
            if not test_df.empty:
                ...
                if y_test.empty or y_test.nunique() < 2: # If test set target is empty or single class
                    print(f"Animal {animal_id}, T_max_lag={T_max_lag} → Test target 'short_poke' is empty or has only one class. Test AUC will be NaN.")
            ...
            if cols_to_drop_train:
                print(f"Animal {animal_id}, T_max_lag={T_max_lag} → Dropping constant columns from X_train: {cols_to_drop_train}")
            ...
            if not X_train.empty:
                ...
                if high_corr_cols_to_drop:
                    print(f"Animal {animal_id}, T_max_lag={T_max_lag} → Dropping highly correlated columns from X_train: {high_corr_cols_to_drop}")
            ...
            if X_train.shape[1] == 0: # No predictors left
                print(f"Animal {animal_id}, T_max_lag={T_max_lag} → No predictors remaining after filtering. Skipping model fit.")
                results_auc.append({'animal': animal_id, 'train_auc': np.nan, 'test_auc': np.nan})
                continue
