...
for animal_id in [103]:
    ...
    # VBMC loglike
    # params:
    # Gamma: theta_E, rate_lambda, ILD_bias
    # Omega: o_ratio_scale_20, o_ratio_scale_40, o_ratio_scale_60, norm_factor
    # misc: w, t_E_aff, del_go
    def compute_loglike_trial(row, theta_E, rate_lambda, ILD_bias, o_ratio_scale_20, o_ratio_scale_40, o_ratio_scale_60, norm_factor, w, t_E_aff, del_go):
        # data
        c_A_trunc_time = 0.3
        rt = row['timed_fix']
        ...
        gamma = theta_E * np.tanh(rate_lambda * (ILD - ILD_bias))

        if ABL == 20:
            omega = o_ratio_scale_20 * ( np.cosh(rate_lambda * (ILD - ILD_bias))) / ( np.cosh(rate_lambda * norm_factor * (ILD - ILD_bias)))
        elif ABL == 40:
            omega = o_ratio_scale_40 * ( np.cosh(rate_lambda * (ILD - ILD_bias))) / ( np.cosh(rate_lambda * norm_factor * (ILD - ILD_bias)))
        elif ABL == 60:
            omega = o_ratio_scale_60 * ( np.cosh(rate_lambda * (ILD - ILD_bias))) / ( np.cosh(rate_lambda * norm_factor * (ILD - ILD_bias)))
        else:
    ...
    def vbmc_loglike_fn(params):
        (
            theta_E, rate_lambda, ILD_bias, o_ratio_scale_20, o_ratio_scale_40, o_ratio_scale_60, norm_factor, w, t_E_aff, del_go
        ) = params

        all_loglike = Parallel(n_jobs=30)(
            delayed(compute_loglike_trial)(
                row,
                theta_E, rate_lambda, ILD_bias, o_ratio_scale_20, o_ratio_scale_40, o_ratio_scale_60, norm_factor,
                w, t_E_aff, del_go
            )
    ...
    def vbmc_prior_fn(params):
        ...
        logpdf += trapezoidal_logpdf(ILD_bias, ILD_bias_bounds[0], ILD_bias_plausible_bounds[0], ILD_bias_plausible_bounds[1], ILD_bias_bounds[1])
    ...
    # %%
    lb = np.array([
        theta_E_bounds[0], rate_lambda_bounds[0], ILD_bias_bounds[0], o_ratio_scale_20_bounds[0], o_ratio_scale_40_bounds[0], o_ratio_scale_60_bounds[0], norm_factor_bounds[0],
        w_bounds[0],t_E_aff_bounds[0], del_go_bounds[0]
    ])
    ub = np.array([
        theta_E_bounds[1], rate_lambda_bounds[1], ILD_bias_bounds[1], o_ratio_scale_20_bounds[1], o_ratio_scale_40_bounds[1], o_ratio_scale_60_bounds[1], norm_factor_bounds[1],
        w_bounds[1],t_E_aff_bounds[1], del_go_bounds[1]
    ])
    plb = np.array([
        theta_E_plausible_bounds[0], rate_lambda_plausible_bounds[0], ILD_bias_plausible_bounds[0], o_ratio_scale_20_plausible_bounds[0], o_ratio_scale_40_plausible_bounds[0], o_ratio_scale_60_plausible_bounds[0], norm_factor_plausible_bounds[0],
        w_plausible_bounds[0],t_E_aff_plausible_bounds[0], del_go_plausible_bounds[0]
    ])
    pub = np.array([
        theta_E_plausible_bounds[1], rate_lambda_plausible_bounds[1], ILD_bias_plausible_bounds[1], o_ratio_scale_20_plausible_bounds[1], o_ratio_scale_40_plausible_bounds[1], o_ratio_scale_60_plausible_bounds[1], norm_factor_plausible_bounds[1],
    ...
    ILD_bias_0 = np.random.uniform(ILD_bias_plausible_bounds[0], ILD_bias_plausible_bounds[1])
    ...
    x_0 = np.array([
        theta_E_0, rate_lambda_0, ILD_bias_0, o_ratio_scale_20_0, o_ratio_scale_40_0, o_ratio_scale_60_0, norm_factor_0,
    ...
    # %%
    vbmc.save(f'vbmc_mutiple_gama_omega_at_once_but_parametric_batch_{batch_name}_animal_{animal_id}_params_like_og_MODEL.pkl', overwrite=True)
