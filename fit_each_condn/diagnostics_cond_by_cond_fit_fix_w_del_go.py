...
def get_param_means_by_ABL_ILD(batch_name, animal_id, ABLs_to_fit, ILDs_to_fit, param_names=None):
    ...
    for ABL in ABLs_to_fit:
        for ILD in ILDs_to_fit:
            pkl_file = f'vbmc_cond_by_cond_{batch_name}_{animal_id}_{ABL}_ILD_{ILD}_FIX_w_pt_5_del_go.pkl'
            ...
            print(f'shape: {vp_samples.shape}')
...
for animal_id in [103]:
    ...
    # --- PDF output block ---
    pdf_filename = f'{batch_name}_{animal_id}_diagnostics_cond_by_cond_FIX_w_pt_5_del_go.pdf'
    ...
    plt.xlabel('ILD (dB)')
    plt.ylabel('t_E_aff')
    plt.title(f't_E_aff vs ILD for each ABL\nAnimal: {animal_id}, Batch: {batch_name}')
    plt.xticks(all_ILDs)
    plt.tight_layout()
    pdf.savefig(plt.gcf())
    plt.close()
