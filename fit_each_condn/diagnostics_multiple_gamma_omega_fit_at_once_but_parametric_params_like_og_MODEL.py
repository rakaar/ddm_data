...
for animal_id in [103]:
    ...
    saved_vbmc_file = f'vbmc_mutiple_gama_omega_at_once_but_parametric_batch_{batch_name}_animal_{animal_id}_params_like_og_MODEL.pkl'
    ...
    # --- PDF output block ---
    pdf_filename = f'{batch_name}_{animal_id}_diagnostics_multiple_gama_omega_at_once_but_parametric_batch_{batch_name}_animal_{animal_id}_params_like_og_MODEL.pdf'
    ...
    ILD_bias_samples = vp_samples[:, 2]
    o_ratio_scale_20_samples = vp_samples[:, 3]
    o_ratio_scale_40_samples = vp_samples[:, 4]
    o_ratio_scale_60_samples = vp_samples[:, 5]
    norm_factor_samples = vp_samples[:, 6]

    w_samples = vp_samples[:, 7]
    t_E_aff_samples = vp_samples[:, 8]
    del_go_samples = vp_samples[:, 9]

    # Means (if needed elsewhere)
    theta_E = theta_E_samples.mean()
    rate_lambda = rate_lambda_samples.mean()
    ILD_bias = ILD_bias_samples.mean()
    ...
    corner_samples = np.vstack([
        theta_E_samples, rate_lambda_samples, ILD_bias_samples,
    ...
    def get_gamma(ABL, ILD):
        return theta_E * np.tanh(rate_lambda * (ILD - ILD_bias))

    def get_omega(ABL, ILD):
        if ABL == 20:
            return o_ratio_scale_20 * np.cosh(rate_lambda * (ILD - ILD_bias)) / np.cosh(rate_lambda * norm_factor * (ILD - ILD_bias))
        elif ABL == 40:
            return o_ratio_scale_40 * np.cosh(rate_lambda * (ILD - ILD_bias)) / np.cosh(rate_lambda * norm_factor * (ILD - ILD_bias))
        elif ABL == 60:
            return o_ratio_scale_60 * np.cosh(rate_lambda * (ILD - ILD_bias)) / np.cosh(rate_lambda * norm_factor * (ILD - ILD_bias))
        else:
            return None
