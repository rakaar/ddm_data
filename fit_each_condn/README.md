...
- `compare_cond_gamma_omega_with_npl_alpha_ild2_delay.py` - Compares condition-fit Gamma/Omega means against curves implied by the copied NPL+alpha+ILD2-delay animal-wise fits; supports the same selectable condition-fit source families as `fit_mean_omega_alpha_model.py` and saves source-tagged figure/CSV summaries under `NPL_alpha_ILD2_fit_results/gamma_omega_comparison/`.
