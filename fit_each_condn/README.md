...
- `svi_big_gamma_omega_delay_lapse_single_animal.py` - Lapse extension of the big Gamma/Omega/`t_E_aff` SVI fit; estimates condition-wise Gamma/Omega/`t_E_aff` plus global `w`, `del_go`, `lapse_prob`, and `lapse_prob_right`, initializing the condition parameters from the completed no-lapse patience12 big SVI when available.
- `run_svi_big_gamma_omega_delay_lapse_all_animals.py` - Tmux-friendly all-animal runner for the 94-param Gamma/Omega/delay+lapse SVI fit, using the same patience restore-best controls and output bundle layout as the no-lapse big SVI runner.
- `validate_svi_gamma_omega_lapse_likelihood_port.py` - Formula-level validation comparing the new JAX Gamma/Omega/delay+lapse likelihood against the legacy VBMC condition-lapse likelihood on real trial rows, including the `LED34_even` abort-truncation branch.
- `plot_big_gamma_omega_delay_lapse_single_animal_smoke.py` - Smoke-test diagnostic plotter that overlays one animal's lapse SVI condition parameters against the completed no-lapse big SVI fit and compares fitted lapse scalars against accepted IPL+lapse and NPL+alpha+lapse SVI references.
- `plot_big_gamma_omega_delay_lapse_patience12_loss_grid.py` - Plots the completed all-30 patience12 restore-best Gamma/Omega/delay+lapse SVI loss curves as a 5 x 6 grid, marking restored-best checkpoints and final checked steps.
- `compare_big_gamma_omega_delay_lapse_vs_no_lapse_condition_params.py` - Compares completed no-lapse and lapse big Gamma/Omega/delay SVI condition posterior means, plotting Gamma by ABL, Gamma averaged across ABLs within animal, and Omega by ABL with SEM across animals.
...
- `evaluate_sd_delay_extrapolation_methods.py` - Tests SD `t_E_aff` extrapolation candidates by holding out `|ILD|=16` in the 24 non-SD patience12 big-SVI animals, fitting signed-branch delay curves from `|ILD|=1,2,4,8`, and plotting validation metrics plus SD extrapolated curves.
...
- `compare_big_svi_gamma_omega_with_mse_alpha_model.py` - Fits the Gamma/Omega alpha model by MSE to each animal's completed big Gamma/Omega/delay SVI condition means, then plots averaged condition Gamma/Omega against averaged per-animal MSE functional curves with SEM shading; set `BIG_SVI_GAMMA_OMEGA_MSE_OBJECTIVE=gamma_omega`, `gamma_only`, or `omega_only` to fit both targets or hold one out.
- `compare_patience12_big_svi_gamma_omega_with_mse_alpha_model.py` - Runs the same per-animal Gamma/Omega MSE alpha-model comparison on the all-30 patience12 restore-best big SVI outputs.
- `run_patience12_big_svi_gamma_omega_mse_objective_variants.py` - Runs the patience12 per-animal MSE alpha-model comparison for Gamma+Omega, Gamma-only, and Omega-only objectives, writing each variant into its own `objective_variants/` folder plus a metrics summary CSV.
