# %%
"""
Prototype one-animal SVI fit with condition-wise Gamma/Omega/delay parameters.

This is the large direct Gamma/Omega fit:

    gamma[ABL, ILD], omega[ABL, ILD], t_E_aff[ABL, ILD], plus global w and del_go
...
BATCH_NAME = os.environ.get("NUMPYRO_BIG_GAMMA_OMEGA_DELAY_BATCH", "LED8")
ANIMAL = int(os.environ.get("NUMPYRO_BIG_GAMMA_OMEGA_DELAY_ANIMAL", "105"))

GUIDE_KIND = os.environ.get("NUMPYRO_BIG_GAMMA_OMEGA_DELAY_GUIDE", "fullrank")
GUIDE_INIT_SCALE = float(os.environ.get("NUMPYRO_BIG_GAMMA_OMEGA_DELAY_GUIDE_INIT_SCALE", "0.02"))
LOWRANK_RANK = int(os.environ.get("NUMPYRO_BIG_GAMMA_OMEGA_DELAY_LOWRANK_RANK", "20"))
LEARNING_RATE = float(os.environ.get("NUMPYRO_BIG_GAMMA_OMEGA_DELAY_LR", "0.0003"))
OPTIMIZER_KIND = os.environ.get("NUMPYRO_BIG_GAMMA_OMEGA_DELAY_OPTIMIZER", "clipped_adam")
CLIP_NORM = float(os.environ.get("NUMPYRO_BIG_GAMMA_OMEGA_DELAY_CLIP_NORM", "2.0"))
SVI_STEPS = int(os.environ.get("NUMPYRO_BIG_GAMMA_OMEGA_DELAY_STEPS", "5000"))
SVI_CHECK_EVERY = int(os.environ.get("NUMPYRO_BIG_GAMMA_OMEGA_DELAY_CHECK_EVERY", "500"))
SVI_STOP_MODE = os.environ.get("NUMPYRO_BIG_GAMMA_OMEGA_DELAY_STOP_MODE", "legacy").strip().lower()
SVI_MIN_STEPS = int(os.environ.get("NUMPYRO_BIG_GAMMA_OMEGA_DELAY_MIN_STEPS", "0"))
SVI_EARLY_STOP = os.environ.get("NUMPYRO_BIG_GAMMA_OMEGA_DELAY_EARLY_STOP", "1").strip().lower() in {
    "1",
    "true",
    "yes",
}
SVI_REL_TOL = float(os.environ.get("NUMPYRO_BIG_GAMMA_OMEGA_DELAY_REL_TOL", "0.001"))
SVI_PATIENCE_WINDOWS = int(os.environ.get("NUMPYRO_BIG_GAMMA_OMEGA_DELAY_PATIENCE_WINDOWS", "3"))
SVI_MIN_IMPROVEMENT_REL = float(os.environ.get("NUMPYRO_BIG_GAMMA_OMEGA_DELAY_MIN_IMPROVEMENT_REL", "0.001"))
SVI_NO_IMPROVE_PATIENCE_WINDOWS = int(
    os.environ.get("NUMPYRO_BIG_GAMMA_OMEGA_DELAY_NO_IMPROVE_PATIENCE_WINDOWS", "5")
)
SVI_STABLE_UPDATE = os.environ.get("NUMPYRO_BIG_GAMMA_OMEGA_DELAY_STABLE_UPDATE", "1").strip().lower() in {
    "1",
    "true",
    "yes",
}
if SVI_STOP_MODE not in {"legacy", "stable_or_no_improve", "patience_restore_best"}:
    raise ValueError(
        "NUMPYRO_BIG_GAMMA_OMEGA_DELAY_STOP_MODE must be one of "
        "'legacy', 'stable_or_no_improve', or 'patience_restore_best', "
        f"got {SVI_STOP_MODE!r}."
    )
POSTERIOR_N_SAMPLES = int(os.environ.get("NUMPYRO_BIG_GAMMA_OMEGA_DELAY_POSTERIOR_N_SAMPLES", "2000"))
RNG_SEED = int(os.environ.get("NUMPYRO_BIG_GAMMA_OMEGA_DELAY_SEED", "0"))
K_MAX = int(os.environ.get("NUMPYRO_BIG_GAMMA_OMEGA_DELAY_K_MAX", "10"))
MIN_TRIALS_PER_CONDITION = int(os.environ.get("NUMPYRO_BIG_GAMMA_OMEGA_DELAY_MIN_TRIALS", "10"))
...
OUTPUT_ROOT = Path(
    os.environ.get(
        "NUMPYRO_BIG_GAMMA_OMEGA_DELAY_OUTPUT_ROOT",
        str(SCRIPT_DIR / "svi_big_gamma_omega_delay_single_animal_outputs"),
    )
).expanduser()
...
OUTPUT_PREFIX = f"{BATCH_NAME}_{ANIMAL}_big_gamma_omega_delay"
...
if missing_modules:
    print("Missing dependencies for big Gamma/Omega/delay SVI:")
...
def make_optimizer():
    ...
    raise ValueError(f"Unknown NUMPYRO_BIG_GAMMA_OMEGA_DELAY_OPTIMIZER={OPTIMIZER_KIND!r}")
...
def big_gamma_omega_delay_model(data, n_conditions):
    ...
    loglike = likelihood_utils.gamma_omega_delay_loglike(params, data, K_MAX)
...
def plot_loss(losses, convergence_df):
    ...
    ax.set_title(f"{BATCH_NAME}/{ANIMAL} big Gamma/Omega/delay SVI loss")
...
w_init = float(np.clip(animal_svi_means["w"], 0.30001, 0.69999))
del_go_init = float(np.clip(animal_svi_means["del_go"], 1e-5, 0.19999))
init_values = {
    "gamma": gamma_init,
    "omega": omega_init,
...
print(f"  init w={w_init:.6f}, del_go={1e3 * del_go_init:.3f} ms")
...
def log_joint_from_values(values):
    log_joint, _ = log_density(big_gamma_omega_delay_model, (data, n_conditions), {}, values)
    return log_joint
...
initial_loglike = float(likelihood_utils.gamma_omega_delay_loglike(init_values, data, K_MAX))
...
# Run SVI
# =============================================================================
guide = make_guide(big_gamma_omega_delay_model, init_values)
svi = SVI(big_gamma_omega_delay_model, guide, make_optimizer(), Trace_ELBO())
...
svi_result, convergence_df, stop_reason = run_svi_with_convergence_checks(
    ...
    data,
    n_conditions,
    f"{BATCH_NAME}/{ANIMAL} big_gamma_omega_delay",
)
