...
# for animal_id in all_animals:
    ...
    # --- Load parametric fit ---
    pkl_file = os.path.join(f'vbmc_mutiple_gama_omega_at_once_but_parametric_batch_{batch_name}_animal_{animal_id}_BETTER_BOUNDS_V2.pkl')
    if not os.path.exists(pkl_file):
        print(f"Skipping {animal_id} as VBMC file {pkl_file} does not exist.")
        continue
    ...
    w_20 = vp_samples[:, 21].mean()
    w_40 = vp_samples[:, 22].mean()
    w_60 = vp_samples[:, 23].mean()
    t_E_aff = vp_samples[:, 24].mean()
    del_go = vp_samples[:, 25].mean()
    ...
    # Left: mean RT (data)
    for ABL in ABLs_to_fit:
        axes[0].scatter(ILDs_to_fit, mean_rts[ABL], marker='o', label=f'ABL={ABL}')
    axes[0].set_title('Mean RT (data) vs ILD')
    axes[0].set_xlabel('ILD (dB)')
    axes[0].set_ylabel('Mean RT (s)')
    axes[0].legend()
    # Middle: mean RT (model)
    for ABL in ABLs_to_fit:
        axes[1].scatter(ILDs_to_fit, model_rts[ABL], marker='o', label=f'ABL={ABL}')
    axes[1].set_title('Mean RT (model) vs ILD')
    ...
    for ABL in ABLs_to_fit:
        ...
        axes[2].plot(ILD_continuous, omega_curve, label=f'ABL={ABL}')
    # Optionally, overlay the discrete points as scatter
    for ABL in ABLs_to_fit:
        omegas = [get_omega(ABL, ILD) for ILD in ILDs_to_fit]
        axes[2].scatter(ILDs_to_fit, omegas, marker='o', s=40, alpha=0.6)
    axes[2].set_title('Omega vs ILD')
    axes[2].set_xlabel('ILD (dB)')
    axes[2].set_ylabel('Omega')
    axes[2].legend()

    plt.suptitle(f'Animal: {animal_id}, Batch: {batch_name}')
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.savefig(f'see_omega_parametric_mean_RTs_{batch_name}_animal_{animal_id}.png')
    plt.close()
print('Done.')
