...
# %%
"""
Run the big condition-wise Gamma/Omega/delay SVI fit for all animals.

This wraps svi_big_gamma_omega_delay_single_animal.py so the likelihood and
model stay in one place. Each animal gets condition-wise gamma, omega, and
t_E_aff plus global w and del_go.
"""
...
# CLI and preflight
# =============================================================================
parser = argparse.ArgumentParser(description="Run big Gamma/Omega/delay SVI fits for all animals.")
...
if args.dry_run:
    ...
    for run_index, (batch, animal) in enumerate(pairs, start=1):
        ...
        print(
            "  env "
            f"NUMPYRO_BIG_GAMMA_OMEGA_DELAY_BATCH={batch} "
            f"NUMPYRO_BIG_GAMMA_OMEGA_DELAY_ANIMAL={animal} "
            f"NUMPYRO_BIG_GAMMA_OMEGA_DELAY_OUTPUT_ROOT={OUTPUT_ROOT} "
    ...
    env.update(
        {
            ...
            "NUMPYRO_BIG_GAMMA_OMEGA_DELAY_OUTPUT_ROOT": str(OUTPUT_ROOT),
