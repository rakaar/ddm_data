...
# %%
def compute_loglike_trial(row, gamma, omega, w):
        # data
        c_A_trunc_time = 0.3
        rt = row['timed_fix']
...
def vbmc_prior_fn(params):
    ...
    return gamma_logpdf + omega_logpdf + w_logpdf
...
for cond_ABL in all_ABLs_cond:
    for cond_ILD in all_ILDs_cond:
        ...
        def vbmc_loglike_fn(params):
            ...
            all_loglike = Parallel(n_jobs=30)(delayed(compute_loglike_trial)(row, gamma, omega, w) \
                                            for _, row in df_led_off_valid_trials_cond_filtered.iterrows())
        ...
        lb = np.array([gamma_bounds[0], omega_bounds[0], w_bounds[0]])
        ub = np.array([gamma_bounds[1], omega_bounds[1], w_bounds[1]])

        plb = np.array([gamma_plausible_bounds[0], omega_plausible_bounds[0], w_plausible_bounds[0]])
        pub = np.array([gamma_plausible_bounds[1], omega_plausible_bounds[1], w_plausible_bounds[1]])
        ...
        x_0 = np.array([gamma_0, omega_0, w_0])
        ...
        # save vbmc 
        vbmc.save(f'vbmc_cond_by_cond_{batch_name}_{animal_id}_{cond_ABL}_ILD_{cond_ILD}_FIX_t_E_del_go.pkl', overwrite=True)
