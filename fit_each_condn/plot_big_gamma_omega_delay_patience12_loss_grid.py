# %%
"""
Plot all-animal loss curves for the patience-12 restore-best big SVI rerun.
...
FIG_PATH = SUMMARY_DIR / "big_gamma_omega_delay_patience12_loss_grid.png"
SUMMARY_CSV = SUMMARY_DIR / "big_gamma_omega_delay_patience12_loss_grid_summary.csv"
...
for _, ledger_row in ledger_df.sort_values("run_index").iterrows():
    ...
    prefix = f"{batch}_{animal}_big_gamma_omega_delay"
...
print("Loaded completed patience-12 runs:")
...
for ax, payload in zip(axes, plot_payload):
    ...
    ax.set_ylabel("mean loss", fontsize=7)

for ax in axes[len(plot_payload) :]:
    ax.axis("off")

legend_handles = [
    Line2D([0], [0], color=LOSS_COLOR, lw=1.4, label="1k-window mean loss"),
...
fig.suptitle(
    "Patience-12 restore-best big Gamma/Omega/delay SVI loss curves",
    y=0.995,
    fontsize=14,
)
