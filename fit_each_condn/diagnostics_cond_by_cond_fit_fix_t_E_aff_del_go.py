...
def get_param_means_by_ABL_ILD(batch_name, animal_id, ABLs_to_fit, ILDs_to_fit, param_names=None):
    ...
    if param_names is None:
        param_names = ['gamma', 'omega', 'w']
    
    param_dict = {}
    for ABL in ABLs_to_fit:
        for ILD in ILDs_to_fit:
            pkl_file = f'vbmc_cond_by_cond_{batch_name}_{animal_id}_{ABL}_ILD_{ILD}_FIX_t_E_del_go.pkl'
...
for animal_id in [103]:
    ...
    # --- PDF output block ---
    pdf_filename = f'{batch_name}_{animal_id}_diagnostics_cond_by_cond_FIX_t_E_aff_del_go.pdf'
