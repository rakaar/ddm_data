# %%
"""
Plot all-animal loss curves for the patience-12 restore-best Gamma/Omega/delay+lapse SVI run.

Each subplot shows the 1k-window mean negative ELBO used by the stopping rule.
The green vertical line is the restored-best checkpoint, and the red dashed
line is the final step checked before the rule stopped.
"""
...
OUTPUT_ROOT = SCRIPT_DIR / "svi_big_gamma_omega_delay_lapse_patience12_restore_best_all_animals_outputs"
LOG_DIR = OUTPUT_ROOT / "_batch_logs"
LEDGER_CSV = LOG_DIR / "batch_run_status.csv"
SUMMARY_DIR = OUTPUT_ROOT / "summary_figures"
SUMMARY_DIR.mkdir(parents=True, exist_ok=True)

FIG_PATH = SUMMARY_DIR / "big_gamma_omega_delay_lapse_patience12_loss_grid.png"
SUMMARY_CSV = SUMMARY_DIR / "big_gamma_omega_delay_lapse_patience12_loss_grid_summary.csv"
...
bad_status = ledger_df[ledger_df["status"] != "completed"]
if not bad_status.empty:
    raise RuntimeError("Some animals are not completed:\n" + bad_status.to_string(index=False))
...
for _, ledger_row in ledger_df.sort_values("run_index").iterrows():
    ...
    prefix = f"{batch}_{animal}_big_gamma_omega_delay_lapse"
    ...
    plot_payload.append(
        {
            ...
            "stop_reason": str(ledger_row["stop_reason"]),
        }
    )
    summary_rows.append(
        {
...
print("Loaded completed patience-12 Gamma/Omega/delay+lapse runs:")
...
fig.suptitle(
    "Patience-12 restore-best Gamma/Omega/delay+lapse SVI loss curves",
    y=0.995,
    fontsize=14,
)
