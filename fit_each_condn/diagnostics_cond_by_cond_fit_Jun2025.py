...
def get_param_means_by_ABL_ILD(batch_name, animal_id, ABLs_to_fit, ILDs_to_fit, param_names=None):
    ...
    for ABL in ABLs_to_fit:
        for ILD in ILDs_to_fit:
            pkl_file = f'vbmc_cond_by_cond_{batch_name}_{animal_id}_{ABL}_ILD_{ILD}.pkl'
...
# %%
# average of t_E_aff, w, del_go
all_t_E_aff = []
all_w = []
all_del_go = []
for ABL in ABLs_to_fit:
    for ILD in ILDs_to_fit:
        all_t_E_aff.append(param_dict[(ABL, ILD)]['t_E_aff'])
        all_w.append(param_dict[(ABL, ILD)]['w'])
        all_del_go.append(param_dict[(ABL, ILD)]['del_go'])
print(f'Average t_E_aff: {np.mean(all_t_E_aff)}')
print(f'Average w: {np.mean(all_w)}')
print(f'Average del_go: {np.mean(all_del_go)}')
# %%
# bounds
# omega_bounds = [0.1, 15]
...
for animal_id in [103]:
    ...
    # --- PDF output block ---
    pdf_filename = f'{batch_name}_{animal_id}_diagnostics_cond_by_cond.pdf'
    ...
    for ABL in ABLs_to_fit:
        for ILD in ILDs_to_fit:
            ...
            t_E_aff = param_dict[(ABL, ILD)]['t_E_aff']
            w = param_dict[(ABL, ILD)]['w']
            del_go = param_dict[(ABL, ILD)]['del_go']
    ...
    for ABL in ABLs_to_fit:
        ...
        for ILD in ILDs_to_fit:
            t_E_aff = param_dict[(ABL, ILD)]['t_E_aff']
            if t_E_aff is not None:
                t_E_affs.append(t_E_aff)
                ILDs_plot.append(ILD)
        plt.scatter(ILDs_plot, t_E_affs, marker='o', label=f'ABL={ABL}')
    plt.xlabel('ILD (dB)')
    plt.ylabel('delta E')
    plt.title(f'delta E vs ILD for each ABL\nAnimal: {animal_id}, Batch: {batch_name}')
    plt.xticks(all_ILDs)
    plt.tight_layout()
    pdf.savefig(plt.gcf())
    plt.close()
    ...
    for ABL in ABLs_to_fit:
        ...
        for ILD in ILDs_to_fit:
            ...
            if w is not None:
                ws.append(w)
                ILDs_plot.append(ILD)
        plt.scatter(ILDs_plot, ws, marker='o', label=f'ABL={ABL}')
    plt.xlabel('ILD (dB)')
    plt.ylabel('w')
    plt.title(f'w vs ILD for each ABL\nAnimal: {animal_id}, Batch: {batch_name}')
    plt.xticks(all_ILDs)
    plt.tight_layout()
    pdf.savefig(plt.gcf())
    plt.close()
    ...
    for ABL in ABLs_to_fit:
        ...
        for ILD in ILDs_to_fit:
            del_go = param_dict[(ABL, ILD)]['del_go']
            if del_go is not None:
                del_gos.append(del_go)
                ILDs_plot.append(ILD)
        plt.scatter(ILDs_plot, del_gos, marker='o', label=f'ABL={ABL}')
    plt.xlabel('ILD (dB)')
    plt.ylabel('del_go')
    plt.title(f'del_go vs ILD for each ABL\nAnimal: {animal_id}, Batch: {batch_name}')
    plt.xticks(all_ILDs)
    plt.tight_layout()
    pdf.savefig(plt.gcf())
    plt.close()
