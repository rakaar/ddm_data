...
Runs on `asd_wt.csv` (WT-only ASD dataset) and saves PNGs in the current directory.
"""
...
# Input CSV (generated from explore_asd_dataset.py)
INPUT_CSV = "asd_wt.csv"
...
# Output
RTD_PNG = 'asd_wt_all_animals_rtd_1ms.png'
CDF_PNG = 'asd_wt_all_animals_rtd_cdf_by_ild_1ms.png'
DCDF_PNG = 'asd_wt_all_animals_dcdf_combined_1ms.png'
...
def load_data() -> pd.DataFrame:
    """Load WT ASD data and filter to valid RTs."""
...
def compute_histograms(df: pd.DataFrame) -> dict[tuple[int, int, int], np.ndarray]:
    ...
    def _hist(group: pd.DataFrame, ani: int):
        ...
        for abl in ABL_VALUES:
            for ild in ABS_ILD_VALUES:
                ...
                if len(subset):
                    hist, _ = np.histogram(subset['timed_rt'], bins=BINS, density=True)
                else:
    ...
    results = Parallel(n_jobs=n_jobs, verbose=10)(
        delayed(_hist)(grp, ani) for ani, grp in grouped
    )
...
def plot_average_cdf(agg: dict[tuple[int, int], list[np.ndarray]]):
    ...
    colour_cycle = plt.cm.gray_r(np.linspace(0.4, 1, len(ABS_ILD_VALUES)))
