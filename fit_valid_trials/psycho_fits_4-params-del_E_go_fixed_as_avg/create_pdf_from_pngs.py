...
def extract_batch_animal(filename):
    ...
    Expected format: psycho_corner_4-params-del_E_go_fixed_as_avg_{batch_name}_{animal_num}.png
    """
    pattern = r'psycho_corner_4-params-del_E_go_fixed_as_avg_(.+)_(\d+)\.png'
...
def create_pdf_from_pngs(directory_path, output_filename="combined_plots.pdf", page_size=A4):
    """
    Create a PDF with each PNG file on a separate page with title.
    """
    # Get all PNG files matching the pattern
    png_files = [f for f in os.listdir(directory_path) if f.endswith('.png') and f.startswith('psycho_corner_4-params-del_E_go_fixed_as_avg_')]
