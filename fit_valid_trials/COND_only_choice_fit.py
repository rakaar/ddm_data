...
def compute_loglike(row, rate_lambda, T_0, theta_E, t_E_aff, Z_E, L):
    ...
    P_t = all_RTs_fit_OPTIM_fn([rt], V_A, theta_A, ABL, ILD, rate_lambda, T_0, theta_E, Z_E, t_stim, t_A_aff, t_E_aff, t_motor, K_max)[0]

    if choice == 3:
        likelihood_up_t = up_RTs_fit_fn(
            [rt], V_A, theta_A, ABL, ILD, rate_lambda, T_0,
            theta_E, Z_E, t_stim, t_A_aff, t_E_aff, t_motor, L, K_max
        )[0]
        likelihood = likelihood_up_t / P_t
    elif choice == 2:
        likelihood_down_t = down_RTs_fit_fn(
            [rt], V_A, theta_A, ABL, ILD, rate_lambda, T_0,
            theta_E, Z_E, t_stim, t_A_aff, t_E_aff, t_motor, L, K_max
        )[0]

        likelihood = likelihood_down_t / P_t
...
# %%
vbmc.save('COND_choice_all_trials.pkl')
