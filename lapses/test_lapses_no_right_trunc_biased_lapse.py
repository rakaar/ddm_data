...
def psiam_tied_data_gen_wrapper_rate_norm_fn(V_A, theta_A, ABL, ILD, rate_lambda, T_0, theta_E, Z_E, t_A_aff, t_E_aff, del_go, \
                                t_stim, rate_norm_l, iter_num, N_print, dt, lapse_prob=0.0, lapse_prob_right=0.5, T_lapse_max=1.0):
    ...
    choice, rt, is_act = simulate_psiam_tied_rate_norm(V_A, theta_A, ABL, ILD, rate_lambda, T_0, theta_E, Z_E, \
                                                       t_stim, t_A_aff, t_E_aff, del_go, rate_norm_l, dt, lapse_prob, lapse_prob_right, T_lapse_max)
...
# Calculate T_lapse_max from the real data (maximum RTwrtStim)
T_lapse_max = df_valid['RTwrtStim'].max()
print(f"T_lapse_max calculated from real data: {T_lapse_max:.3f} s")
...
lapse_prob_right = 0.7  # Probability of choosing right during lapse (0.5 = unbiased)
...
sim_results = Parallel(n_jobs=30)(
    delayed(psiam_tied_data_gen_wrapper_rate_norm_fn)(
        ...
        t_stim_samples[iter_num], rate_norm_l, iter_num, N_print, dt, lapse_prob, lapse_prob_right, T_lapse_max
...
# Use the same T_lapse_max from real data for VBMC fitting (not recalculated from simulated data)
T_lapse_max_fixed = T_lapse_max
print(f"\nT_lapse_max for VBMC fitting (from real data): {T_lapse_max_fixed:.3f} s")
print(f"Max rt_minus_t_stim in simulated data: {valid_rt_trials['rt_minus_t_stim'].max():.3f} s")
...
# %%
# VBMC helper funcs
def compute_loglike_vanilla(row, rate_lambda, T_0, theta_E, Z_E, t_E_aff, del_go, lapse_prob, lapse_prob_right, T_lapse_max_fixed):
    ...
    ILD = row['ILD']
    ABL = row['ABL']
    lapse_rt_window = T_lapse_max_fixed
    ...
    # Lapse PDF: biased choice probability * uniform RT density
    in_lapse_window = (rt >= t_stim) and (rt < t_stim + lapse_rt_window)
    if in_lapse_window:
        # Probability of this choice given lapse
        choice_prob = lapse_prob_right if choice == 1 else (1 - lapse_prob_right)
        lapse_pdf = choice_prob / lapse_rt_window
    else:
        lapse_pdf = 0.0
...
def vbmc_vanilla_tied_loglike_fn(params):
    ...
    Z_E = (w - 0.5) * 2 * theta_E
    all_loglike = Parallel(n_jobs=30)(delayed(compute_loglike_vanilla)(row, rate_lambda, T_0, theta_E, Z_E, t_E_aff, del_go, lapse_prob, lapse_prob_right, T_lapse_max_fixed)\
...
# %%
# VBMC parameter bounds (covering true parameter values)
# True values: rate_lambda=0.1372, T_0=0.0009, theta_E=17.8567, w=0.4810, t_E_aff=0.0751, del_go=0.1579
vanilla_rate_lambda_bounds = [0.05, 0.20]    # Wide enough to cover 0.1372
vanilla_T_0_bounds = [0.0001, 0.0015]        # Wide enough to cover 0.0009
vanilla_theta_E_bounds = [10, 60]            # Wide enough to cover 17.8567
vanilla_w_bounds = [0.45, 0.57]              # Covers 0.4810
vanilla_t_E_aff_bounds = [0.05, 0.10]        # Covers 0.0751
vanilla_del_go_bounds = [0.10, 0.25]         # Wide enough to cover 0.1579
vanilla_lapse_prob_bounds = [0.05, 0.15]     # Covers 0.1000
vanilla_lapse_prob_right_bounds = [0.5, 0.9] # Covers 0.7000

# Plausible bounds (tighter around true values, inside regular bounds)
vanilla_rate_lambda_plausible_bounds = [0.10, 0.17]    # Around 0.1372
vanilla_T_0_plausible_bounds = [0.0005, 0.0012]        # Around 0.0009
vanilla_theta_E_plausible_bounds = [15, 25]            # Around 17.8567
vanilla_w_plausible_bounds = [0.47, 0.52]              # Around 0.4810
vanilla_t_E_aff_plausible_bounds = [0.065, 0.085]      # Around 0.0751
vanilla_del_go_plausible_bounds = [0.14, 0.19]         # Around 0.1579
vanilla_lapse_prob_plausible_bounds = [0.08, 0.12]     # Around 0.1000
vanilla_lapse_prob_right_plausible_bounds = [0.6, 0.8] # Around 0.7000
...
# %%
# VBMC fitting on simulated datalapses/test_lapses_no_right_trunc.py
...
lapse_prob_right_0 = np.random.uniform(vanilla_lapse_prob_right_plausible_bounds[0], vanilla_lapse_prob_right_plausible_bounds[1])
...
print(f"t_E_aff={t_E_aff_0:.3f}, del_go={del_go_0:.3f}, lapse_prob={lapse_prob_0:.3f}, lapse_prob_right={lapse_prob_right_0:.3f}")
...
print(f"lapse_prob: {lapse_prob:.3f}, lapse_prob_right: {lapse_prob_right:.3f}")
print(f"T_lapse_max (fixed): {T_lapse_max_fixed:.3f}")
...
true_values = [rate_lambda, T_0, theta_E, w, t_E_aff, del_go, lapse_prob, lapse_prob_right]
