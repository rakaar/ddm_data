...
# %%
"""
Run NPL+alpha+lapse condition-delay NumPyro SVI for all 30 animals.

This runner discovers animals from `aborts_ipl_npl_time_fit_results/`, runs the
single-animal NPL+alpha+lapse SVI script with environment variables, and keeps a
...
FIT_SCRIPT = SCRIPT_DIR / "numpyro_svi_npl_alpha_lapse_condition_delay_single_animal.py"
ABORT_RESULTS_DIR = REPO_DIR / "aborts_ipl_npl_time_fit_results"
NPL_REFERENCE_ROOT = SCRIPT_DIR / "numpyro_svi_npl_alpha_condition_delay_patience12_restore_best_outputs"
LAPSE_REFERENCE_ROOT = SCRIPT_DIR / "numpyro_svi_vanilla_lapse_condition_delay_patience12_min50k_restore_best_outputs"
OUTPUT_ROOT = SCRIPT_DIR / "numpyro_svi_npl_alpha_lapse_condition_delay_patience12_min50k_restore_best_outputs"
...
def animal_paths(output_root: Path, batch: str, animal: int, label: str):
    ...
    return {
        ...
        "comparison_csv": output_dir / f"{label}_reference_param_comparison.csv",
...
def npl_reference_dir(batch: str, animal: int) -> Path:
    return NPL_REFERENCE_ROOT / f"{batch}_{animal}"


def lapse_reference_dir(batch: str, animal: int) -> Path:
    return LAPSE_REFERENCE_ROOT / f"{batch}_{animal}"


def npl_reference_summary_path(batch: str, animal: int) -> Path:
    return npl_reference_dir(batch, animal) / "main_fullrank_posterior_summary.csv"


def npl_reference_condition_table_path(batch: str, animal: int) -> Path:
    return npl_reference_dir(batch, animal) / "condition_table.csv"


def lapse_reference_summary_path(batch: str, animal: int) -> Path:
    return lapse_reference_dir(batch, animal) / "main_fullrank_posterior_summary.csv"
...
parser = argparse.ArgumentParser(
    description="Run NumPyro SVI NPL+alpha+lapse condition-delay fits for all 30 animals."
)
...
parser.add_argument(
    ...
    type=int,
    default=50000,
    help="Minimum SVI steps before early stopping can trigger.",
)
...
print(f"NPL reference root: {NPL_REFERENCE_ROOT}")
print(f"IPL+lapse reference root: {LAPSE_REFERENCE_ROOT}")
...
for batch, animal in pairs:
    required_paths = [
        ...
        npl_reference_summary_path(batch, animal),
        npl_reference_condition_table_path(batch, animal),
        lapse_reference_summary_path(batch, animal),
    ]
