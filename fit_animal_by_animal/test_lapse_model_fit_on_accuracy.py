...
plt.show()
# %%
def log_sigmoid_v2(x, a, d, th, ILD_bias):
...
bounds = ([0.0, 0.01, 0.01, -1.0], [0.1, 10, 50, 1.0])
# Fixed initialization
p0 = [0.02, 0.09, 3.0, -0.5]  # [a, d, th, ILD_bias]

try:
    popt, pcov = curve_fit(log_sigmoid_v2, all_x_data, all_y_data, p0=p0, bounds=bounds)
    a_fit, d_fit, th_fit, ILD_bias_fit = popt
    
    # Calculate R^2
    y_pred = log_sigmoid_v2(all_x_data, a_fit, d_fit, th_fit, ILD_bias_fit)
    ...
    y_fitted = log_sigmoid_v2(x_model, a_fit, d_fit, th_fit, ILD_bias_fit)
    ...
    for idx, abl in enumerate(ABL_vals[:3]):
        ...
        ax.plot(x_model, y_fitted, '-', color='red', linewidth=2, 
                label=f'Fit: a={a_fit:.3f}, d={d_fit:.2f}, th={th_fit:.2f}, bias={ILD_bias_fit:.2f}')
...
# Bounds for [a, d, th, lapse_pR, ILD_bias]
# a: (0, 0.1), d: (0.01, 10), th: (0.01, 50), lapse_pR: (0, 1), ILD_bias: (-1, 1)
bounds_v3 = ([0.0, 0.01, 0.01, 0.0, -1.0], [0.1, 10, 50, 1.0, 1.0])
# Fixed initialization
p0_v3 = [0.02, 0.09, 3.0, 0.5, -0.5]  # [a, d, th, lapse_pR, ILD_bias]

try:
    popt_v3, pcov_v3 = curve_fit(log_sigmoid_v3_lapse_biased, all_x_data, all_y_data, 
                                  p0=p0_v3, bounds=bounds_v3)
    a_fit_v3, d_fit_v3, th_fit_v3, lapse_pR_fit, ILD_bias_fit_v3 = popt_v3
    
    # Calculate R^2
    y_pred_v3 = log_sigmoid_v3_lapse_biased(all_x_data, a_fit_v3, d_fit_v3, th_fit_v3, lapse_pR_fit, ILD_bias_fit_v3)
    ...
    print(f"  ILD_bias = {ILD_bias_fit_v3:.6f}")
    print(f"  R²       = {r2_v3:.4f}")
    ...
    x_model = np.linspace(-16, 16, 300)
    y_fitted_v3 = log_sigmoid_v3_lapse_biased(x_model, a_fit_v3, d_fit_v3, th_fit_v3, lapse_pR_fit, ILD_bias_fit_v3)
    ...
    for idx, abl in enumerate(ABL_vals[:3]):
        ...
        ax.plot(x_model, y_fitted_v3, '-', color='blue', linewidth=2, 
                label=f'Fit: a={a_fit_v3:.3f}, d={d_fit_v3:.2f}, th={th_fit_v3:.2f}, pR={lapse_pR_fit:.3f}, bias={ILD_bias_fit_v3:.2f}')
...
# %%
def psyc_lapse_UNBIASED(x, a, d, th, ILD_bias):
    f =  th * np.tanh(d * (x + ILD_bias))
    p0 = 1.0 / (1.0 + (np.exp(-2*f)))  # sigma(f)
    pR = a/2.0 + (1.0 - a) * p0
    return pR
...
# %%
# Fit psyc_lapse_UNBIASED (4 parameters including ILD_bias)
bounds_psyc_unbiased = ([0.0, 0.01, 0.01, -1.0], [0.1, 10, 50, 1.0])
# Fixed initialization
p0_psyc_unbiased = [0.02, 0.09, 3.0, -0.5]  # [a, d, th, ILD_bias]
...
try:
    ...
    a_psyc_unbiased, d_psyc_unbiased, th_psyc_unbiased, ILD_bias_psyc_unbiased = popt_psyc_unbiased
    
    # Calculate R^2
    y_pred_psyc_unbiased = psyc_lapse_UNBIASED(all_x_psyc, a_psyc_unbiased, d_psyc_unbiased, th_psyc_unbiased, ILD_bias_psyc_unbiased)
    ...
    print(f"  ILD_bias = {ILD_bias_psyc_unbiased:.6f}")
    ...
    x_model = np.linspace(-16, 16, 300)
    y_fitted_psyc_unbiased = psyc_lapse_UNBIASED(x_model, a_psyc_unbiased, d_psyc_unbiased, th_psyc_unbiased, ILD_bias_psyc_unbiased)
    ...
    for idx, abl in enumerate(ABL_vals[:3]):
        ...
        ax.plot(x_model, y_fitted_psyc_unbiased, '-', color='red', linewidth=2, 
                label=f'Fit: a={a_psyc_unbiased:.3f}, d={d_psyc_unbiased:.2f}, th={th_psyc_unbiased:.2f}, bias={ILD_bias_psyc_unbiased:.2f}')
...
# %%
# Fit psyc_lapse_biased (5 parameters)
bounds_psyc_biased = ([0.0, 0.01, 0.01, 0.0, -1.0], [0.1, 10, 50, 1.0, 1.0])
# Fixed initialization
p0_psyc_biased = [0.02, 0.09, 3.0, 0.5, -0.5]  # [a, d, th, lapse_pR, ILD_bias]
...
try:
    ...
    a_psyc_biased, d_psyc_biased, th_psyc_biased, lapse_pR_psyc, ILD_bias_psyc_biased = popt_psyc_biased
    
    # Calculate R^2
    y_pred_psyc_biased = psyc_lapse_biased(all_x_psyc, a_psyc_biased, d_psyc_biased, th_psyc_biased, lapse_pR_psyc, ILD_bias_psyc_biased)
    ...
    print(f"  ILD_bias = {ILD_bias_psyc_biased:.6f}")
    ...
    x_model = np.linspace(-16, 16, 300)
    y_fitted_psyc_biased = psyc_lapse_biased(x_model, a_psyc_biased, d_psyc_biased, th_psyc_biased, lapse_pR_psyc, ILD_bias_psyc_biased)
    ...
    for idx, abl in enumerate(ABL_vals[:3]):
        ...
        ax.plot(x_model, y_fitted_psyc_biased, '-', color='blue', linewidth=2, 
                label=f'Fit: a={a_psyc_biased:.3f}, d={d_psyc_biased:.2f}, th={th_psyc_biased:.2f}, pR={lapse_pR_psyc:.3f}, bias={ILD_bias_psyc_biased:.2f}')
...
def psyc_from_logodds_params(x, a, d, th, lapse_pR, ILD_bias):
...
y_psyc_fit = psyc_lapse_biased(x_model, a_psyc_biased, d_psyc_biased, th_psyc_biased, lapse_pR_psyc, ILD_bias_psyc_biased)
y_logodds_fit = psyc_from_logodds_params(x_model, a_fit_v3, d_fit_v3, th_fit_v3, lapse_pR_fit, ILD_bias_fit_v3)
...
for idx, abl in enumerate(ABL_vals[:3]):
    ...
    ax.plot(x_model, y_psyc_fit, '-', color='blue', linewidth=2, 
            label=f'Psyc fit (pR={lapse_pR_psyc:.3f}, bias={ILD_bias_psyc_biased:.2f})', alpha=0.8)
    
    # Plot log-odds fit converted to P(right)
    ax.plot(x_model, y_logodds_fit, '--', color='red', linewidth=2, 
            label=f'Log-odds fit (pR={lapse_pR_fit:.3f}, bias={ILD_bias_fit_v3:.2f})', alpha=0.8)
...
print(f"{'ILD_bias':<15} {ILD_bias_psyc_biased:<20.6f} {ILD_bias_fit_v3:<20.6f}")
