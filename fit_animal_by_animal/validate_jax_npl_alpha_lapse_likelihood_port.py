# %%
"""
Validate the JAX NPL+alpha+lapse likelihood port against the old NumPy functions.

The compatibility comparison sets alpha=1, where the new NPL+alpha equations
reduce to the old NPL equations used by `lapses_fit_single_animal_norm_model.py`.
"""
...
MAX_ROWS_PER_ANIMAL = int(os.environ.get("NPL_ALPHA_LAPSE_JAX_VALIDATE_MAX_ROWS", "300"))
RNG_SEED = int(os.environ.get("NPL_ALPHA_LAPSE_JAX_VALIDATE_SEED", "0"))
K_MAX = int(os.environ.get("K_MAX", "10"))
LOG_TOL = float(os.environ.get("NPL_ALPHA_LAPSE_JAX_VALIDATE_LOG_TOL", "1e-6"))
TOTAL_LOG_TOL = float(os.environ.get("NPL_ALPHA_LAPSE_JAX_VALIDATE_TOTAL_LOG_TOL", "1e-4"))
...
for batch_name, animal in TEST_ANIMALS:
    ...
    init_values = dict(npl_alpha_lapse_utils.DEFAULT_INIT_VALUES)
    init_values["alpha"] = 1.0
    init_values["t_E_aff"] = condition_table["t_E_aff_s"].to_numpy(dtype=float)
    init_values = npl_alpha_lapse_utils.clip_init_to_hard_bounds(init_values)
    ...
    pdf_jax, trunc_jax, log_pdf_jax = npl_alpha_lapse_utils.npl_alpha_lapse_condition_delay_loglike_terms(
        init_values,
        data,
        K_max=K_MAX,
    )
    ...
    log_joint = npl_alpha_lapse_utils.npl_alpha_lapse_condition_delay_loglike(
        init_values,
        data,
        K_max=K_MAX,
    )
    grad = jax.grad(
        lambda values: npl_alpha_lapse_utils.npl_alpha_lapse_condition_delay_loglike(
            values,
            data,
            K_max=K_MAX,
        )
    )(init_values)
    gradients_finite = npl_alpha_lapse_utils.tree_all_finite(grad)

    print(f"Rows compared: {len(compare_df)}")
    print(f"Conditions compared: {len(condition_table)}")
    print(f"T_trunc: {T_trunc:.3f} s")
    print("Compatibility alpha: 1.0")
