...
# Output directory for psychometric fits (T_0 fixed from vanilla)
output_dir = '/home/rlab/raghavendra/ddm_data/fit_valid_trials/psycho_fits_T_0_fixed_from_vanilla'
...
def fit_psychometric_for_animal(batch_name, animal_id_str):
    ...
    try:
        ...
        # Get fixed t_E_aff, del_go, and T_0 from vanilla tied fits only
        vanilla_keyname = "vbmc_vanilla_tied_results"
        if vanilla_keyname not in fit_results_data:
            raise KeyError(f"Missing required key in {pkl_file}: {vanilla_keyname}")
        ...
        def psiam_tied_prior_fn(params):
            ...
            return rate_lambda_logpdf + theta_E_logpdf + w_logpdf
        ...
        # Run VBMC
        lb = np.array([rate_lambda_bounds[0], theta_E_bounds[0], w_bounds[0]])
        ub = np.array([rate_lambda_bounds[1], theta_E_bounds[1], w_bounds[1]])

        plb = np.array([rate_lambda_plausible_bounds[0], theta_E_plausible_bounds[0], w_plausible_bounds[0]])
        pub = np.array([rate_lambda_plausible_bounds[1], theta_E_plausible_bounds[1], w_plausible_bounds[1]])
        ...
        x_0 = np.array([rate_lambda_0, theta_E_0, w_0])
        ...
        # Save corner plot
        corner_plot_file = os.path.join(output_dir, f"psycho_corner_3-params-T_0_fixed_from_vanilla_{batch_name}_{animal_id}.png")
        plt.savefig(corner_plot_file, dpi=300, bbox_inches='tight')
        plt.close(fig)
        
        # Save VBMC results
        vbmc_file = os.path.join(output_dir, f"psycho_fit_3-params-T_0_fixed_from_vanilla_{batch_name}_{animal_id}.pkl")
