# %%
"""
Build paper Fig. 2-style data products for the direct IPL SVI fit.

This repackages the completed patience12/min50k IPL condition-delay SVI
diagnostic payload into smaller Fig. 2 v2 pickles. It does not rerun the
expensive RTD integrations.
"""
...
IPL_SVI_ROOT = ANIMAL_DIR / "numpyro_svi_vanilla_condition_delay_patience12_min50k_restore_best_outputs"
INPUT_PAYLOAD = IPL_SVI_ROOT / "fig2_like_diagnostics" / "ipl_svi_50k_fig2_like_diagnostics.pkl"

OUTPUT_DIR = SCRIPT_DIR
PSY_OUTPUT = OUTPUT_DIR / "ipl_svi_psy_fig2_v2_data.pkl"
QUANT_OUTPUT = OUTPUT_DIR / "ipl_svi_quant_fig2_v2_data.pkl"
SLOPES_OUTPUT = OUTPUT_DIR / "ipl_svi_slopes_fig2_v2_data.pkl"
GAMMA_OUTPUT = OUTPUT_DIR / "ipl_svi_gamma_fig2_v2_data.pkl"
BUNDLE_OUTPUT = OUTPUT_DIR / "ipl_svi_fig2_v2_bundle.pkl"

METHOD_KEY = "ipl_svi_50k"
MODEL_LABEL = "Direct IPL SVI condition-delay"
...
def subset_quantile_theory_dict(source, indices):
    out = {}
    for abl in ABLS:
        out[abl] = {}
        for abs_ild, values in source[abl].items():
...
# =============================================================================
print(f"Input IPL SVI Fig. 2-like payload: {INPUT_PAYLOAD}")
...
payload_root = Path(payload["ipl_svi_root"]).resolve()
if payload_root != IPL_SVI_ROOT.resolve():
    raise RuntimeError(f"Payload root mismatch: {payload_root} != {IPL_SVI_ROOT.resolve()}")
...
if METHOD_KEY not in set(method_summary_df["method_key"]):
...
slopes_data = {
    ...
    "vanilla_means": np.asarray(slopes_src["model_means"], dtype=float),
...
continuous_sd_flat = subset_quantile_theory_dict(quant_src["continuous_plot_data_sd_flat"], quantile_indices)
continuous_standard = subset_quantile_theory_dict(quant_src["continuous_plot_data"], quantile_indices)
...
gamma_data = {
    ...
    "model_summary_rows": method_summary_df[method_summary_df["method_key"] == METHOD_KEY].to_dict("records"),
    "condition_source": "patience12 92-param Gamma/Omega/delay SVI condition posterior means",
    "model_source": "direct patience12/min50k IPL condition-delay SVI posterior means",
}
