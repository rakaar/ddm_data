...
def negative_log_likelihood_psyc(params, x_data, k_right, n_trials):
    """
    Binomial negative log-likelihood for psychometric model.
    ...
    # Binomial NLL
    nll = -np.sum(k_right * np.log(p_model) + 
                  (n_trials - k_right) * np.log(1 - p_model))
    return nll
...
NLL_OR_MSE = "nll"  # "nll" for binomial negative log-likelihood, "mse" for mean squared error
...
def process_animal(batch_name, animal_id):
    ...
    all_x_psyc = []
    all_y_psyc = []
    all_k_right = []  # For NLL: count of right choices
    all_n_trials = []  # For NLL: total trials
    ...
    for abl in ABL_vals:
        for ild in ILD_vals:
            ...
            if len(empirical_subset) > 0:
                n_right = np.sum(empirical_subset['choice'] == 1)
                n_total = len(empirical_subset)
                p_right_empirical = n_right / n_total
                
                all_x_psyc.append(ild)
                all_y_psyc.append(p_right_empirical)
                all_k_right.append(n_right)
                all_n_trials.append(n_total)
    
    all_x_psyc = np.array(all_x_psyc)
    all_y_psyc = np.array(all_y_psyc)
    all_k_right = np.array(all_k_right)
    all_n_trials = np.array(all_n_trials)
    
    # Remove any NaN values
    valid_mask_psyc = ~np.isnan(all_y_psyc)
    all_x_psyc = all_x_psyc[valid_mask_psyc]
    all_y_psyc = all_y_psyc[valid_mask_psyc]
    all_k_right = all_k_right[valid_mask_psyc]
    ...
    try:
        if NLL_OR_MSE == "nll":
            # Binomial NLL fitting
            result = minimize(
                negative_log_likelihood_psyc,
                x0=p0_psyc,
                args=(all_x_psyc, all_k_right, all_n_trials),
                bounds=list(zip(bounds_psyc[0], bounds_psyc[1])),
                method='L-BFGS-B'
            )
            popt_psyc = result.x
            nll_value = result.fun
        else:
            # MSE fitting (curve_fit)
            popt_psyc, _ = curve_fit(psyc_lapse_biased, all_x_psyc, all_y_psyc, 
                                      p0=p0_psyc, bounds=bounds_psyc)
        
        # Compute R² for evaluation (regardless of fitting method)
...
def plot_animal_data(animal_data, pdf):
    ...
    fitting_method = "NLL" if NLL_OR_MSE == "nll" else "MSE"
    title_lines = [f'Batch {batch_name}, Animal {animal_id} - Lapse Model Fits (Psychometric: {fitting_method})']
...
print(f"Fitting method: Psychometric = {NLL_OR_MSE.upper()}, Log-odds = MSE")
print("="*60)
    ...
    for batch_name, animal_id in tqdm(batch_animal_pairs, desc="Processing animals"):
        try:
            ...
            if animal_data['logodds_params'] is not None:
                print(f"  Log-odds fit (MSE): R² = {animal_data['logodds_params']['r2']:.4f}")
            if animal_data['psyc_params'] is not None:
                if NLL_OR_MSE == "nll":
                    print(f"  Psychometric fit (NLL): R² = {animal_data['psyc_params']['r2']:.4f}, NLL = {animal_data['psyc_params']['nll']:.2f}")
                else:
                    print(f"  Psychometric fit (MSE): R² = {animal_data['psyc_params']['r2']:.4f}")
...
# %%
# Combine NLL and MSE psychometric results for comparison
print("\n" + "="*60)
...
try:
    # Read both CSV files
    import pandas as pd
    df_nll = pd.read_csv(nll_csv)
    df_mse = pd.read_csv(mse_csv)
    
    # Merge on Batch and Animal
    df_combined = pd.merge(
        df_nll[['Batch', 'Animal', 'R²_Psychometric (%)', 'a_Psychometric (%)', 'lapse_pR_Psychometric (%)']],
        df_mse[['Batch', 'Animal', 'R²_Psychometric (%)', 'a_Psychometric (%)', 'lapse_pR_Psychometric (%)', 
                'R²_LogOdds (%)', 'a_LogOdds (%)', 'lapse_pR_LogOdds (%)']],
        on=['Batch', 'Animal'],
        suffixes=('_NLL', '_MSE')
    )
    ...
    df_combined.columns = [
        'Batch', 'Animal',
        'R²_Psychometric_NLL (%)', 'a_Psychometric_NLL (%)', 'lapse_pR_Psychometric_NLL (%)',
        'R²_Psychometric_MSE (%)', 'a_Psychometric_MSE (%)', 'lapse_pR_Psychometric_MSE (%)',
        'R²_LogOdds (%)', 'a_LogOdds (%)', 'lapse_pR_LogOdds (%)'
    ]
    ...
    print(f"✓ Combined comparison saved to: {combined_csv}")
    print(f"  Rows: {len(df_combined)}")
    print(f"  Columns: {', '.join(df_combined.columns)}")
    
except FileNotFoundError as e:
    print(f"⚠ Warning: Could not create combined CSV - missing file: {e}")
    print(f"  Make sure both '{nll_csv}' and '{mse_csv}' exist.")
except Exception as e:
    print(f"⚠ Warning: Error creating combined CSV: {e}")
# %%
