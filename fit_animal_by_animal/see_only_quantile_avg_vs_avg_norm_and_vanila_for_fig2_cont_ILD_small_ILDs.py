...
def process_animal_for_quantiles(batch_animal_pair):
    ...
    try:
        ...
        continuous_ild_values = np.arange(-4.0, 4.1, 0.1)
...
continuous_abs_ild = np.round(np.arange(1.0, 4.1, 0.1), 1)
...
plt.savefig(f'small_ILDs_only_quantile_plot_separate_abls_{MODEL_TYPE}.png', dpi=300)
plt.show()
...
plt.tight_layout()
plt.savefig(f'small_ILDs_only_quantile_plot_with_errorbars_{MODEL_TYPE}.png', dpi=300)
plt.show()
...
with open('small_ILDs_only_vanila_quant_fig2_data.pkl', 'wb') as f:
