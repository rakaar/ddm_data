...
- 'psycho' : read 3-parameter VBMC samples from psycho_fits_T_0_fixed_from_vanilla (T_0 fixed)
In both cases, abort params and fixed T_0, t_E_aff, del_go are read from vanilla tied results in the results PKL.
...
def get_params_from_animal_pkl_file(batch_name, animal_id):
    ...
    if PARAM_SOURCE == 'results':
        ...
        # Use psycho-fit 3-param VBMC means for tied params (T_0 fixed separately)
    ...
    # Attach fixed T_0, t_E_aff and del_go (from vanilla)
...
plt.savefig(f'rtd_average_by_stimulus_T_0_also_fixed_{MODEL_TYPE}.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.subplots_adjust(bottom=0.15)
plt.savefig(f'rtd_average_by_abs_ILD_FOLDED_T_0_also_fixed_{MODEL_TYPE}.png', dpi=300, bbox_inches='tight')
plt.show()
            ...
            for batch_animal_pair, animal_data in rtd_data.items():
                if stim_key in animal_data and not found:
                    ...
                    if abort_keyname in fit_results_data:
                        ...
                        continue
                    # Fixed parameters from vanilla results
                    try:
                        fixed_T_0, fixed_t_E_aff, fixed_del_go = get_fixed_params_from_vanilla(fit_results_data, batch_name, animal_id)
                    except Exception as e:
                        print(f"Missing fixed params from vanilla for {batch_name},{animal_id}: {e}")
                        continue
                    ...
                    if PARAM_SOURCE == 'results':
                        ...
                        try:
                            ...
                            continue
                        n_pool = min(len(rate_lambda_s), len(theta_E_s), len(w_s), len(rate_norm_l_s))
                        if n_pool < 5:
...
plt.tight_layout()
plt.subplots_adjust(bottom=0.15)
plt.savefig(f'5_rtd_individual_and_mean_by_abs_ILD_FOLDED_T_0_also_fixed_{MODEL_TYPE}.png', dpi=300, bbox_inches='tight')
plt.show()