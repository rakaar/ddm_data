...
- `numpyro_svi_npl_alpha_condition_delay_single_animal.py` - exploratory `# %%` NumPyro/JAX SVI prototype for one animal, fitting shared NPL+alpha parameters plus per-condition `t_E_aff`; defaults to a full-rank Gaussian guide initialized from the upstream VBMC global posterior covariance with chunked convergence checks, accepts `NUMPYRO_SVI_ABORT_RESULT_PKL` for explicit proactive-posterior sensitivity reruns, and the JAX likelihood/guide helpers live in `numpyro_npl_alpha_svi_utils.py`.
- `numpyro_npl_alpha_reactive_svi_utils.py` - pure-reactive NPL+alpha likelihood/model helpers for valid trials retained in a stimulus-relative RT window; the bound-specific RT+choice density is normalized by the probability of either evidence bound being reached inside the retained window, with no proactive process and no `del_go`.
- `validate_jax_npl_alpha_reactive_likelihood.py` - checks the reactive-only JAX density, retained-window mass, log likelihood, and gradients against an independent NumPy implementation on real LED7/92 trials, including numerical integration of the normalized two-choice density.
- `numpyro_svi_npl_alpha_reactive_condition_delay_single_animal.py` - 36-parameter reactive-only NPL+alpha SVI fit for valid `0.100 <= RTwrtStim < 1.000` trials: six shared parameters (`rate_lambda`, `T_0`, `theta_E`, `w`, `rate_norm_l`, `alpha`) plus 30 condition-wise delays. It initializes the full-rank posterior mean/covariance from the completed patience12 37-parameter fit after dropping `del_go`, uses patience12/min50k restore-best stopping, and writes posterior/guide bundles, loss and convergence tables, corners, and delay intervals.
- `run_numpyro_svi_npl_alpha_reactive_condition_delay_led7.py` - sequential six-animal LED7 runner for the reactive-only fit above. It preflights animals 92/93/98/99/100/103, records the 100 ms trial removal counts, and writes per-animal logs plus `batch_run_status.csv` under `numpyro_svi_npl_alpha_reactive_only_rt_ge_100ms_condition_delay_patience12_min50k_restore_best_outputs/`.
- `plot_numpyro_svi_npl_alpha_reactive_led7_loss_grid.py` - plots the six LED7 reactive-only negative-ELBO traces using 1k-step window means, with restored-best and final-checked checkpoints marked and a CSV audit of the stopping behavior.
- `../fitting_aborts/plot_led7_npl_alpha_reactive_ge100ms_fit_aligned_rtds.py` - compares the six LED7 reactive-only fits with valid-trial RTDs over `0.100 <= RTwrtStim < 1.000`; every signed-condition empirical and model RTD is normalized to unit area before equal averaging across signs, animals, ABLs, and absolute ILDs.
- `../fitting_aborts/plot_led7_npl_alpha_reactive_ge100ms_t_E_aff_vs_ild_by_abl.py` - plots posterior-mean condition-wise `t_E_aff` from the six LED7 reactive-only fits against signed ILD, with ABL 20/40/60 separated by color and SEM calculated across animals.
- `numpyro_npl_alpha_rt_only_svi_utils.py` - choice-collapsed NPL+alpha likelihood helpers for matched `reactive_only` and fixed-`proactive_reactive` condition-delay fits. Both modes fit six shared NPL+alpha parameters plus condition-wise `t_E_aff`; `del_go` is excluded because it cancels exactly after summing the two choice densities.
- `validate_jax_npl_alpha_rt_only_likelihood.py` - validates the two RT-only likelihoods on real LED7/92 trials, including bound-sum identities, `del_go` cancellation, finite gradients, and numerical 0-1 s normalization checks.
- `numpyro_svi_npl_alpha_rt_only_condition_delay_single_animal.py` - mode-configurable 36-parameter RT-only SVI fit on successful `0 <= RTwrtStim < 1` trials. The proactive+reactive mode fixes `V_A`, `theta_A`, and `t_A_aff` from the existing animal abort fit; both modes share the same full-rank initialization and patience12/min50k restore-best settings.
- `run_numpyro_svi_npl_alpha_rt_only_condition_delay_led7.py` - tmux-friendly six-animal LED7 runner for either RT-only process mode, with distinct mode-specific output roots, complete artifact checks, per-animal logs, and a batch status ledger.
- `plot_numpyro_svi_npl_alpha_rt_only_led7_loss_grids.py` - plots separate `2 x 3` LED7 negative-ELBO grids for the RT-only proactive+reactive and reactive-only fits. Each animal panel uses 1k-window means and ends at the restored-best checkpoint plus 5k steps, capped at the final checked step, so late iterations do not compress the visible descent.
- `diagnose_numpyro_svi_npl_alpha_condition_delay_single_animal.py` - single-animal SVI diagnostics for RTDs by ABL and `|ILD|` plus psychometric curves, using 1 ms model RTDs, 20 ms data bins for the standard RTD panels, and the same abort-event/truncation convention as the all-animal RTD comparison. Optional `NUMPYRO_SVI_SELECTED_RTD_ABLS`/`NUMPYRO_SVI_SELECTED_RTD_ILD` overrides add a signed-ILD ABL comparison panel with 5 ms data bins and reactive-only rows.
...
- `numpyro_npl_alpha_lapse_svi_utils.py` - JAX/NumPyro port of the NPL+alpha + lapse condition-delay likelihood, adding global `lapse_prob` and `lapse_prob_right` to the existing NPL+alpha SVI likelihood.
- `validate_jax_npl_alpha_lapse_likelihood_port.py` - Validates the NPL+alpha + lapse JAX likelihood against the old NumPy NPL+lapse likelihood at `alpha=1` on real LED8 and LED34_even rows.
- `numpyro_svi_npl_alpha_lapse_condition_delay_single_animal.py` - Single-animal NPL+alpha + lapse condition-delay SVI fit, initialized from the completed patience12 NPL+alpha condition-delay SVI parameters and the IPL+lapse lapse estimates; saves posterior samples, summaries, loss/convergence CSVs, delay intervals, reference-comparison CSV, and a VP bundle.
- `run_numpyro_svi_npl_alpha_lapse_condition_delay_all_animals.py` - Tmux-friendly all-30 runner for the NPL+alpha + lapse condition-delay SVI fits, using the same patience12 restore-best rule with a 50k-step minimum and writing per-animal logs plus a batch ledger.
- `plot_numpyro_svi_npl_alpha_lapse_condition_delay_patience12_loss_grid.py` - Plots a 5 x 6 grid of all 30 patience12 min-50k restore-best NPL+alpha + lapse condition-delay SVI loss curves.
- `plot_npl_alpha_lapse_svi_params_by_animal_earlybest.py` - Plots all 30 patience12 min-50k NPL+alpha + lapse condition-delay SVI scalar parameters by animal with 95% posterior intervals, highlighting animals whose restored-best checkpoint was at or before 2k steps.
- `lapses_figure_from_svi_fits.py` - Recreates the 2 x 4 lapses supplementary figure from the current all-animal SVI roots for IPL, NPL+alpha, IPL+lapse, and NPL+alpha+lapse; the old condition-by-condition lapse Gamma panel is intentionally annotated as omitted.
- `compare_npl_vs_npl_alpha_lapse_condition_delay_params.py` - Compares posterior parameter means from the no-lapse NPL+alpha SVI and NPL+alpha + lapse SVI fits, with scalar parameters by animal and across-animal condition-delay curves.
- `plot_npl_alpha_lapse_svi_lapse_params_by_animal.py` - Plots NPL+alpha + lapse SVI posterior mean lapse rate and lapse right-choice probability by animal with 95% posterior intervals, overlaid with IPL + lapse values as x markers.
- `plot_npl_alpha_lapse_low_lr_rerun_loss_curves.py` - Plots low-learning-rate reference-initialized NPL+alpha + lapse rerun loss curves for suspicious first-window animals, plus a hybrid all-animal loss grid.
- `plot_npl_alpha_lapse_random_init_rerun_loss_curves.py` - Plots random-plausible low-learning-rate NPL+alpha + lapse rerun loss curves and compares their best losses against reference-initialized reruns.
- `plot_npl_alpha_lapse_reference_vs_random_100k_loss_curves.py` - Compares four suspicious low-learning-rate reference-initialized NPL+alpha + lapse SVI runs against random-plausible 100k reruns using side-by-side ELBO curves and restored-best/final-checked markers.
- `vbmc_npl_lapse_refreshed_single_animal.py` - Refreshed old VBMC NPL+lapse single-animal fit with updated raw-data and abort-reference paths; saves VBMC posterior summaries and samples without rerunning the old heavy diagnostics.
- `vbmc_npl_alpha_lapse_scalar_delay_refreshed_single_animal.py` - Refreshed VBMC NPL+alpha+lapse single-animal fit that keeps the old scalar `t_E_aff` shared across conditions while fitting `alpha`, `w`, `del_go`, and lapse parameters; includes an `alpha=1` compatibility check against the no-alpha likelihood.
- `run_vbmc_npl_alpha_lapse_scalar_delay_four_animals.py` - Narrow tmux-friendly runner for the four suspicious animals using the refreshed VBMC NPL+alpha+lapse scalar-delay script; skips existing posterior summaries unless `--force` is passed.
- `run_vbmc_npl_lapse_four_animals.py` - Narrow tmux-friendly runner for the four suspicious animals used to compare refreshed old VBMC NPL+lapse fits against the early and random-100k SVI reruns.
- `compare_vbmc_npl_lapse_to_svi_four_animals.py` - CSV comparison of refreshed VBMC NPL+lapse posterior means against early-best and random-100k NPL+alpha + lapse SVI posterior means for the four suspicious animals.
- `compare_npl_alpha_lapse_svi_vbmc_params_elbo_likelihood.py` - Result-book figure script comparing early SVI, random-100k SVI, and refreshed VBMC parameter means with 95% intervals, ELBO/objective values, and common posterior-mean likelihoods; evaluates VBMC under the shared likelihood with `alpha=1`.
- `compare_npl_alpha_lapse_svi_vbmc_alpha_scalar_params_elbo_likelihood.py` - Result-book figure script matching the SVI/VBMC comparison above, but using the refreshed scalar-delay VBMC NPL+alpha+lapse fits and their fitted `alpha` posterior instead of fixing VBMC alpha to 1.
...
- `numpyro_vanilla_lapse_condition_delay_svi_utils.py` - JAX/NumPyro port of the old vanilla/IPL + lapse likelihood, fitting `rate_lambda`, `T_0`, `theta_E`, `w`, `del_go`, global `lapse_prob`, global `lapse_prob_right`, and condition-wise `t_E_aff`.
- `validate_jax_vanilla_lapse_likelihood_port.py` - Compares the vanilla/IPL + lapse JAX likelihood against the old NumPy lapse-mixture likelihood on real LED8 and LED34_even rows before running SVI.
- `numpyro_svi_vanilla_lapse_condition_delay_single_animal.py` - Single-animal vanilla/IPL + lapse condition-delay SVI fit, using the same patience12 restore-best convergence machinery as the no-lapse IPL SVI and saving posterior samples, summaries, loss curves, and VP bundles.
- `run_numpyro_svi_vanilla_lapse_condition_delay_all_animals.py` - Tmux-friendly all-30 runner for the vanilla/IPL + lapse condition-delay SVI fits, using the patience12 restore-best rule with a 50k-step minimum by default and writing per-animal logs plus a batch ledger.
...
- `plot_numpyro_svi_vanilla_lapse_condition_delay_patience12_loss_grid.py` - Plots a 5 x 6 grid of all 30 patience12 min-50k restore-best vanilla/IPL + lapse condition-delay SVI loss curves.
- `compare_ipl_vs_ipl_lapse_condition_delay_params.py` - Compares posterior parameter means from the no-lapse IPL SVI and IPL + lapse SVI fits, with scalar parameters by animal and across-animal condition-delay curves.
- `plot_ipl_lapse_svi_lapse_params_by_animal.py` - Plots the IPL + lapse SVI posterior mean lapse rate and lapse right-choice probability by animal with 95% posterior intervals.
...
- `figure4_v2/` - Compact paper Figure 4-style reproduction using the first-row Gamma+Omega MSE NPL+alpha parameter source from the patience12 92-parameter big Gamma/Omega/delay SVI diagnostic, including a combined Figure 4 + upper-triangular MSE parameter matrix version.
- `figure4_npl_svi_supplementary/` - Supplementary Figure 4-style reproduction using the direct patience12 37-parameter NPL+alpha condition-delay SVI predictions; the upper-triangular five-parameter corner shows animal-wise posterior means, 95% covariance ellipses, and ranked marginal 95% intervals.
...
- `npl_alpha_uniform_delay/uniform_delay_likelihood_utils.py` - Experimental JAX likelihood helpers for a uniform evidence-delay distribution. Implements the `K=100` analytic delay-averaged bound CDF/density identities, a numerically stable proactive CDF, and a fixed-delay reference used for independent Gauss-Legendre checks; it does not modify the production point-delay likelihood.
- `npl_alpha_uniform_delay/validate_uniform_delay_simulator_vs_likelihood.py` - Simulator-versus-likelihood validation for three posterior-derived proactive/reactive parameter sets and three uniform delay widths. The first output retains the complete RT support. The second applies the batch-specific absolute-RT left abort cutoff, retains `0 <= RTwrtStim < 1 s`, and conditionally renormalizes both choices with one shared window mass. Both compare the `K=100` analytic likelihood with a 64-node numerical delay mixture; deterministic supplemental caches give every retained-window cell at least 100,000 trials.
- `npl_alpha_uniform_delay/numpyro_svi_npl_alpha_uniform_delay_single_animal.py` - Fits the 67-parameter NPL+alpha RT+choice model with one Uniform delay center and width per condition, using a full-rank NumPyro guide and patience-12/minimum-50k restore-best stopping.
- `npl_alpha_uniform_delay/run_numpyro_svi_npl_alpha_uniform_delay_led7.py` - Resumable LED7 runner that fits missing animals, writes a run ledger, and regenerates the RTD and posterior-parameter diagnostics.
- `npl_alpha_uniform_delay/plot_npl_alpha_uniform_delay_led7_all_animal_diagnostics.py` - Summarizes the completed LED7 fits with a 1 x 6 loss grid, animal-wise delay supports, and an across-animal delay-center/support comparison. See `npl_alpha_uniform_delay/README.md` for the full pipeline and rerun commands.
