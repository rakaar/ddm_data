...
def fit_psychometric_sigmoid(ild_values, right_choice_probs):
    ...
    # Define 4-parameter sigmoid function with explicit bounds
...
def get_psycho_params(batch_name, animal_id):
    ...
    vp = vbmc_obj.vp
    samples = vp.sample(int(1e6))[0]
    print(f"Loaded 3-param VBMC samples (T_0 fixed from vanilla) for {batch_name}, {animal_id}: shape {samples.shape}")
    tied_params = {
        'rate_lambda' : samples[:,0].mean(),
...
def get_fixed_params_from_vanilla(fit_results_data, batch_name, animal_id):
    """Get fixed T_0, t_E_aff and del_go from vanilla tied fits only."""
    ...
    print(f"Using fixed t_E_aff={fixed_t_E_aff:.6f}, del_go={fixed_del_go:.6f}, T_0={fixed_T_0:.6f} for batch {batch_name}, animal {animal_id}")
...
def get_params_from_animal_pkl_file(batch_name, animal_id):
    ...
    # Fixed T_0, t_E_aff and del_go from vanilla tied fits only
    ...
    tied_params['T_0'] = float(fixed_T_0)
    tied_params['t_E_aff'] = float(fixed_t_E_aff)
    tied_params['del_go'] = float(fixed_del_go)

    # If normalized TIED is selected, include rate_norm_l from results
    if IS_NORM_TIED and ('vbmc_norm_tied_results' in fit_results_data):
        model_samples = fit_results_data['vbmc_norm_tied_results']
        if 'rate_norm_l_samples' in model_samples:
            tied_params['rate_norm_l'] = float(np.mean(np.asarray(model_samples['rate_norm_l_samples'])))
            print(f"Using rate_norm_l={tied_params['rate_norm_l']:.6f} for batch {batch_name}, animal {animal_id}")
        else:
            warnings.warn(f"Key 'rate_norm_l_samples' missing in vbmc_norm_tied_results for {batch_name},{animal_id}")
        ...
        for ild in ild_values:
            try:
                t_pts_0_1, up_mean, down_mean = get_theoretical_RTD_up_down(
                    batch_name, p_a, c_a, ts_samp, abort_params, tied_params, ABL, ild
                )
...
# %%
def get_theoretical_RTD_up_down(batch_name, P_A_mean, C_A_mean, t_stim_samples, abort_params, tied_params, ABL, ILD):
...
def plot_theoretical_psychometric_data(theoretical_psychometric_data):
    ...
    for i, abl in enumerate(abls):
        ...
        ax.legend()
    plt.tight_layout()
    plt.savefig('theoretical_psychometric_by_abl_T_0_also_fixed_from_vanilla.png', dpi=300, bbox_inches='tight')
    plt.show()
    return fig
...
for abl in [20, 40, 60]:
    ...
    if np.sum(valid_idx) >= 4:
        try:
            from scipy.optimize import curve_fit
            # 4-parameter bounded sigmoid (upper/lower asymptotes)
            def bounded_sigmoid(x, upper, lower, x0, k):
                return lower + (upper - lower) / (1 + np.exp(-k * (x - x0)))
            x_fit = ilds[valid_idx]
            y_fit = theo_mean[valid_idx]
            upper0 = float(np.nanmax(y_fit))
            lower0 = float(np.nanmin(y_fit))
            x0_0 = float(np.median(x_fit))
            k0 = 0.1
            p0 = [upper0, lower0, x0_0, k0]
            bounds = ([0.0, 0.0, -np.inf, -np.inf], [1.0, 1.0, np.inf, np.inf])
            popt, _ = curve_fit(bounded_sigmoid, x_fit, y_fit, p0=p0, bounds=bounds)
            ilds_smooth = np.linspace(min(ilds), max(ilds), 200)
            fit_curve = bounded_sigmoid(ilds_smooth, *popt)
