...
for i, animal_info in enumerate(animal_data):
    ...
    for abl in prob_choice_dict.keys():
        ...
        if len(x_fit) > 3:
            try:
                # Initial guess: lambda_L=0.05, lambda_R=0.05, k=1, x0=0
                p0 = [0.05, 0.05, 1, 0]
                # Bounds: lambda_L and lambda_R in [0, 1], k and x0 unbounded
                bounds = ([0, 0, -np.inf, -np.inf], [1, 1, np.inf, np.inf])
                popt, _ = curve_fit(sigmoid, x_fit, y_fit, p0=p0, bounds=bounds, maxfev=5000)
