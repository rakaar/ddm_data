# %%
"""
Refreshed VBMC NPL+alpha+lapse fit for one animal.

This keeps the refreshed scalar-delay VBMC structure, adds alpha to the
normalized model, and stops after saving fit summaries. The model fits one
scalar t_E_aff shared across all conditions.
"""
...
from time_vary_norm_alpha_utils import up_or_down_RTs_fit_alpha_fn, cum_pro_and_reactive_time_vary_alpha_fn


DEFAULT_OUTPUT_ROOT = SCRIPT_DIR / "vbmc_npl_alpha_lapse_scalar_delay_rerun"


parser = argparse.ArgumentParser(description="Fit VBMC NPL+alpha+lapse scalar-delay model for one animal.")
...
parser.add_argument("--alpha1-check-rows", type=int, default=100, help="Rows used to check alpha=1 against no-alpha likelihood")
parser.add_argument("--alpha1-check-atol", type=float, default=1e-8, help="Max absolute row-loglike tolerance for alpha=1 check")
parser.add_argument("--skip-alpha1-check", action="store_true", help="Skip the alpha=1 no-alpha compatibility check")
...
print("=" * 80)
print(f"VBMC NPL+alpha+lapse scalar-delay refreshed fit: {batch_name}/{animal}")
...
# %%
# =============================================================================
# NPL+alpha+lapse likelihood and bounds
# =============================================================================
def compute_loglike_norm_alpha(row, rate_lambda, T_0, theta_E, Z_E, t_E_aff, del_go, rate_norm_l, alpha, lapse_prob, lapse_prob_right):
    rt = row["TotalFixTime"]
    ...
    if DO_RIGHT_TRUNCATE:
        ...
        trunc_factor_p_joint = 1 - cum_pro_and_reactive_time_vary_alpha_fn(
            t_stim,
            T_trunc,
            V_A,
            theta_A,
...
def compute_loglike_norm_no_alpha_check(row, rate_lambda, T_0, theta_E, Z_E, t_E_aff, del_go, rate_norm_l, lapse_prob, lapse_prob_right):
    rt = row["TotalFixTime"]
    t_stim = row["intended_fix"]
    ILD = row["ILD"]
    ABL = row["ABL"]
...
def vbmc_norm_tied_loglike_fn(params):
    rate_lambda, T_0, theta_E, w, t_E_aff, del_go, rate_norm_l, alpha, lapse_prob, lapse_prob_right = params
    Z_E = (w - 0.5) * 2 * theta_E
    all_loglike = Parallel(n_jobs=-5)(
        delayed(compute_loglike_norm_alpha)(
            row,
            rate_lambda,
            T_0,
            theta_E,
...
def vbmc_norm_tied_prior_fn(params):
    ...
    return (
        ...
        + trapezoidal_logpdf(alpha, norm_alpha_bounds[0], norm_alpha_plausible_bounds[0], norm_alpha_plausible_bounds[1], norm_alpha_bounds[1])
...
norm_alpha_plausible_bounds = [0.1, 1.8]
...
if init_type == "vanilla":
    x_0 = np.array([0.173793, 1.958508e-3, 14.194689, 0.502231, 87.604997e-3, 169.585535e-3, 0.00001, 1.0, 0.02, 0.5])
else:
    x_0 = np.array([1.8, 150e-3, 5, 0.51, 0.071, 0.13, 0.9, 1.0, 0.02, 0.5])

if not args.skip_alpha1_check:
    alpha1_params = x_0.copy()
    alpha1_params[PARAM_NAMES.index("alpha")] = 1.0
    rate_lambda, T_0, theta_E, w, t_E_aff, del_go, rate_norm_l, alpha, lapse_prob, lapse_prob_right = alpha1_params
    Z_E = (w - 0.5) * 2 * theta_E
    n_check_rows = min(int(args.alpha1_check_rows), len(df_valid_animal))
    check_diffs = []
    for _, row in df_valid_animal.head(n_check_rows).iterrows():
        alpha_loglike = compute_loglike_norm_alpha(
            row,
            rate_lambda,
            T_0,
            theta_E,
        ...
        no_alpha_loglike = compute_loglike_norm_no_alpha_check(
            row,
            rate_lambda,
            T_0,
            theta_E,
        ...
        check_diffs.append(abs(alpha_loglike - no_alpha_loglike))
    max_alpha1_diff = float(np.max(check_diffs)) if check_diffs else 0.0
    print(f"alpha=1 no-alpha compatibility check: rows={n_check_rows}, max_abs_loglike_diff={max_alpha1_diff:.3e}")
    if max_alpha1_diff > float(args.alpha1_check_atol):
        raise RuntimeError(
            f"alpha=1 compatibility check failed: max_abs_loglike_diff={max_alpha1_diff:.3e}, "
            f"tolerance={args.alpha1_check_atol:.3e}"
        )
...
vbmc_pkl_path = output_dir / f"vbmc_norm_alpha_tied_results_batch_{batch_name}_animal_{animal}_lapses_scalar_t_E_aff_truncate_1s_{init_type}{stim_filter_suffix}.pkl"
...
posterior_npz_path = output_dir / "vbmc_norm_alpha_lapse_scalar_t_E_aff_posterior_samples.npz"
...
summary_csv = output_dir / "vbmc_norm_alpha_lapse_scalar_t_E_aff_posterior_summary.csv"
...
run_summary_csv = output_dir / "vbmc_norm_alpha_lapse_scalar_t_E_aff_run_summary.csv"
