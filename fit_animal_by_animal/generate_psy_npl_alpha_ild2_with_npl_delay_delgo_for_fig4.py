...
PSY_OUTPUT_PKL = os.path.join(OUTPUT_DIR, "npl_alpha_ild2_with_npl_delay_delgo_psy_fig4_data.pkl")
SLOPES_OUTPUT_PKL = os.path.join(OUTPUT_DIR, "npl_alpha_ild2_with_npl_delay_delgo_slopes_fig4_data.pkl")
...
def load_hybrid_posterior_mean_params(batch_name, animal_id, ild2_paths, npl_paths):
    ...
    tied_params = {
        ...
        "del_go": float(np.mean(npl_samples["del_go_samples"])),
...
def process_animal(batch_animal_pair, ild2_paths, npl_paths):
    ...
    print(f"Processing hybrid delay+del_go psychometric data for {batch_name}, animal {animal_id}")
    abort_params, tied_params = load_hybrid_posterior_mean_params(batch_name, animal_id, ild2_paths, npl_paths)
    print(f"  NPL t_E_aff used = {1e3 * tied_params['t_E_aff']:.3f} ms; NPL del_go used = {tied_params['del_go']:.5f}")
    ...
    for ABL in ABL_arr:
        ...
        for ILD in ILD_arr:
            try:
                ...
                print(f"  failed hybrid delay+del_go theory {batch_name}/{animal_id} ABL={ABL} ILD={ILD}: {exc}")
...
print(f"Using {len(batch_animal_pairs)} matched animals for hybrid delay+del_go psychometric/slopes")
...
psy_plot_data = {
    ...
    "MODEL_TYPE": "npl_alpha_ild2_with_npl_delay_delgo",
}
...
slopes_plot_data = {
    ...
    "slopes_npl_alpha_ild2_with_npl_delay_delgo": slopes_hybrid,
    "common_pairs_sorted": common_pairs_sorted,
    "data_means": data_means,
    "npl_alpha_ild2_with_npl_delay_delgo_means": hybrid_means,
