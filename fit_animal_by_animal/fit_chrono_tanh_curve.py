...
for idx, animal_info in enumerate(animal_data):
    ...
    for abl in sorted(df['ABL'].unique()):
        ...
        # Fit curve
        try:
            popt, _ = curve_fit(chrono_func, xvals, yvals, p0=[0.2, 0.2, 0.2], maxfev=10000)
            a, b, c = popt
            xfit = np.linspace(min(xvals), max(xvals), 200)
            yfit = chrono_func(xfit, a, b, c)
            ax.plot(xfit, yfit, linestyle=':', color=color, linewidth=4, alpha=0.5)
            legend_entries.append(f'ABL {abl}: a={a:.2f}, b={b:.2f}, c={c:.2f}')
        except Exception as e:
            legend_entries.append(f'ABL {abl}: fit failed')
