# %%
"""
Plot lapse-specific parameters from the all-animal vanilla/IPL+lapse SVI run.

The first panel shows lapse_prob as a percentage. The second panel shows
lapse_prob_right directly.
"""

# %%
# =============================================================================
# Editable parameters
# =============================================================================
from pathlib import Path
import os

SCRIPT_DIR = Path(__file__).resolve().parent
OUTPUT_ROOT = (
    SCRIPT_DIR / "numpyro_svi_vanilla_lapse_condition_delay_patience12_min50k_restore_best_outputs"
)
OUTPUT_DIR = OUTPUT_ROOT / "summary_figures"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

EXPECTED_N_ANIMALS = 30
DESIRED_BATCHES = ["SD", "LED34", "LED6", "LED8", "LED7", "LED34_even"]

FIG_PATH = OUTPUT_DIR / "ipl_lapse_svi_lapse_params_by_animal.png"
VALUES_CSV = OUTPUT_DIR / "ipl_lapse_svi_lapse_params_by_animal.csv"

PARAMS = [
    ("lapse_prob", "Lapse rate (%)", 100.0),
    ("lapse_prob_right", "lapse_prob_right", 1.0),
]


# %%
# =============================================================================
# Imports and helpers
# =============================================================================
os.environ.setdefault("MPLCONFIGDIR", "/tmp/matplotlib-cache")
os.environ.setdefault("XDG_CACHE_HOME", "/tmp")

import matplotlib

matplotlib.use("Agg")
import matplotlib.pyplot as plt
import numpy as np
import pandas as pd


def parse_animal_dir_name(path):
    batch_name, animal_text = path.name.rsplit("_", 1)
    return batch_name, int(animal_text)


def animal_sort_key(row):
    batch_name = row["batch_name"]
    animal = int(row["animal"])
    batch_idx = DESIRED_BATCHES.index(batch_name) if batch_name in DESIRED_BATCHES else len(DESIRED_BATCHES)
    return batch_idx, animal


# %%
# =============================================================================
# Load posterior summaries
# =============================================================================
summary_paths = sorted(OUTPUT_ROOT.glob("*/main_fullrank_posterior_summary.csv"))
if len(summary_paths) != EXPECTED_N_ANIMALS:
    raise RuntimeError(
        f"Expected {EXPECTED_N_ANIMALS} posterior summaries, found {len(summary_paths)} in {OUTPUT_ROOT}"
    )

rows = []
for summary_path in summary_paths:
    batch_name, animal = parse_animal_dir_name(summary_path.parent)
    summary_df = pd.read_csv(summary_path)

    for param_name, ylabel, scale in PARAMS:
        param_rows = summary_df[summary_df["parameter"] == param_name]
        if len(param_rows) != 1:
            raise RuntimeError(f"{summary_path} has {len(param_rows)} rows for {param_name}")
        row = param_rows.iloc[0]
        rows.append(
            {
                "batch_name": batch_name,
                "animal": animal,
                "animal_label": f"{batch_name}/{animal}",
                "parameter": param_name,
                "mean": float(row["mean"]) * scale,
                "q025": float(row["q025"]) * scale,
                "q975": float(row["q975"]) * scale,
                "unit": "%" if param_name == "lapse_prob" else "probability",
            }
        )

params_df = pd.DataFrame(rows)
if len(params_df) != EXPECTED_N_ANIMALS * len(PARAMS):
    raise RuntimeError(f"Expected {EXPECTED_N_ANIMALS * len(PARAMS)} parameter rows, found {len(params_df)}")
if params_df.duplicated(["batch_name", "animal", "parameter"]).any():
    raise RuntimeError("Duplicate animal/parameter rows found")
if not np.all(np.isfinite(params_df[["mean", "q025", "q975"]].to_numpy(dtype=float))):
    raise RuntimeError("Parameter values contain NaN/Inf")

animal_rows = (
    params_df[["batch_name", "animal", "animal_label"]]
    .drop_duplicates()
    .sort_values(["batch_name", "animal"])
    .to_dict("records")
)
animal_rows = sorted(animal_rows, key=animal_sort_key)
animal_keys = [(row["batch_name"], int(row["animal"])) for row in animal_rows]
animal_labels = [row["animal_label"] for row in animal_rows]

params_df.to_csv(VALUES_CSV, index=False)

print(f"Loaded lapse parameters for {len(animal_keys)} animals")
print(params_df.groupby("parameter")["mean"].describe().to_string())
print(f"Saved values: {VALUES_CSV}")


# %%
# =============================================================================
# Plot 1 x 2 lapse-parameter panels
# =============================================================================
fig, axes = plt.subplots(1, 2, figsize=(16, 5.5), constrained_layout=True)
x = np.arange(len(animal_labels))

for ax, (param_name, ylabel, _scale) in zip(axes, PARAMS):
    sub = (
        params_df[params_df["parameter"] == param_name]
        .set_index(["batch_name", "animal"])
        .loc[animal_keys]
        .reset_index()
    )
    y = sub["mean"].to_numpy(dtype=float)
    yerr = np.vstack(
        [
            y - sub["q025"].to_numpy(dtype=float),
            sub["q975"].to_numpy(dtype=float) - y,
        ]
    )
    ax.errorbar(
        x,
        y,
        yerr=yerr,
        fmt="o",
        color="#D55E00",
        ecolor="#D55E00",
        elinewidth=1.0,
        capsize=2.5,
        markersize=5.5,
        alpha=0.9,
    )
    ax.set_xticks(x)
    ax.set_xticklabels(animal_labels, rotation=55, ha="right", fontsize=8)
    ax.set_xlabel("Animal")
    ax.set_ylabel(ylabel)
    ax.set_title(ylabel)
    ax.grid(axis="y", alpha=0.25)
    if param_name == "lapse_prob_right":
        ax.set_ylim(0.0, 1.0)

fig.suptitle("Patience-12 min-50k IPL+lapse SVI lapse parameters", fontsize=14)
fig.savefig(FIG_PATH, dpi=200, bbox_inches="tight")

print(f"Saved figure: {FIG_PATH}")

plt.show()


# %%
