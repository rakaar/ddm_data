...
QUANT_PKL = os.path.join(SCRIPT_DIR, "vanilla_quant_fig2_data.pkl")
OUTPUT_DIR = os.path.join(REPO_DIR, "NPL_alpha_ILD2_fit_results", "figure_4_diagnostics")
OUTPUT_PNG = os.path.join(OUTPUT_DIR, "fig2_quantile_sd_ild_mismatch_check.png")
OUTPUT_PDF = os.path.join(OUTPUT_DIR, "fig2_quantile_sd_ild_mismatch_check.pdf")
...
def get_figure2_batch_animal_pairs():
...
def print_sanity_counts(label, data):
    ...
    for abl in data["ABL_arr"]:
        theory_n = len(data["continuous_plot_data"][abl][16.0]["theoretical"])
        ...
        print(
            f"  ABL={abl}: theory signed entries at |ILD|=16 = {theory_n}; "
...
with open(QUANT_PKL, "rb") as handle:
    quantile_data = pickle.load(handle)

batch_animal_pairs = get_figure2_batch_animal_pairs()
sd_animal_indices = [idx for idx, (batch_name, _animal) in enumerate(batch_animal_pairs) if batch_name == "SD"]

print(f"Figure 2 batch-animal pairs ({len(batch_animal_pairs)}):")
...
print_sanity_counts("Figure 2 paper data as saved", quantile_data)
print_sanity_counts("Figure 2 matched SD grid data", matched_sd_data)
...
axes[0].set_title("Figure 2 all 30: paper model includes SD to |ILD|=16", fontsize=12)

plot_quantiles(axes[1], matched_sd_data)
axes[1].set_title("Figure 2 all 30: SD model included only to |ILD|=8", fontsize=12)
axes[1].set_ylabel("")
