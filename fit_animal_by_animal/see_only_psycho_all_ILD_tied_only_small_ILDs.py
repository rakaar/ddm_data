...
def get_params_from_animal_pkl_file(batch_name, animal_id):
    ...
    with open(pkl_file, 'rb') as f:
        fit_results_data_aborts = pickle.load(f)
    ...
    abort_params = {}
    if abort_keyname in fit_results_data_aborts:
        abort_samples = fit_results_data_aborts[abort_keyname]
        for param_samples_name, param_label in vbmc_aborts_param_keys_map.items():
            abort_params[param_label] = np.mean(abort_samples[param_samples_name])
    
    # TIED params
    pkl_file = f'small_ILDs_only_results_{batch_name}_animal_{animal_id}.pkl'
    with open(pkl_file, 'rb') as f:
        fit_results_data_valid = pickle.load(f)
    ...
    if abort_keyname in fit_results_data_aborts:
        ...
        for param_samples_name, param_label in vbmc_aborts_param_keys_map.items():
            abort_params[param_label] = np.mean(abort_samples[param_samples_name])
    if tied_keyname in fit_results_data_valid:
        tied_samples = fit_results_data_valid[tied_keyname]
...
def plot_theoretical_psychometric_data(theoretical_psychometric_data):
    ...
    ild_smooth = np.linspace(-4, 4, 100)
    abls = [20, 40, 60]
    abl_colors = ['b', 'g', 'r']
    for i, abl in enumerate(abls):
        ...
        ax.legend()
    plt.tight_layout()
    plt.savefig('small_ILDs_only_theoretical_psychometric_by_abl.png', dpi=300, bbox_inches='tight')
    plt.show()
    return fig
...
import copy
print(f'is norm tied = {IS_NORM_TIED}')
...
pickle_filename = f"small_ILDs_only_theoretical_psychometric_data_{'norm' if IS_NORM_TIED else 'vanilla'}.pkl"
...
with open('small_ILDs_only_theoretical_psychometric_data_vanilla.pkl', 'rb') as f:
    vanilla_psychometric_data = pickle.load(f)
# with open('small_ILDs_only_theoretical_psychometric_data_norm.pkl', 'rb') as f:
...
common_pairs = set(slopes_vanilla) & set(slopes_data)
...
with open('small_ILDs_only_vanilla_slopes_fig2_data.pkl', 'wb') as f:
...
ax_vanilla.scatter(data_means, vanilla_means, facecolors='none', edgecolors='k', marker='o', s=45, alpha=0.7)
