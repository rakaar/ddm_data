# %%
"""
JAX/NumPyro helpers for vanilla/IPL+lapse condition-delay SVI fits.
...
This extends the no-lapse IPL condition-delay SVI likelihood with the old
vanilla+lapse mixture used by `lapses_fit_single_animal.py`.
"""
...
GLOBAL_BOUNDS = OrderedDict(
    [
        ...
        ("lapse_prob", {"hard": (1e-4, 0.2), "plausible": (1e-3, 0.1)}),
        ("lapse_prob_right", {"hard": (0.001, 0.999), "plausible": (0.4, 0.6)}),
    ]
)
...
def make_guide(model, guide_kind, init_values, init_scale=0.1):
    ...
    raise ValueError(f"Use fullrank or meanfield guide for vanilla+lapse SVI, got {guide_kind!r}.")
