...
vanilla_lapse_prob_bounds = [0.05, 0.5]
vanilla_lapse_prob_right_bounds = [0.1, 0.9]
...
vanilla_lapse_prob_plausible_bounds = [0.1, 0.3]
vanilla_lapse_prob_right_plausible_bounds = [0.4, 0.7]
...
for animal_idx in [0]:
    ...
    vbmc = VBMC(vbmc_vanilla_tied_joint_fn, x_0, vanilla_tied_lb, vanilla_tied_ub, vanilla_plb, vanilla_pub, options={'display': 'on', 'max_fun_evals': 200 * (2 + 7)})
...
# read frm pkl file: 
# animal = animal_ids[0]
# pkl_file = f'vbmc_vanilla_tied_results_batch_{batch_name}_animal_{animal}_lapses.pkl'
# with open(pkl_file, 'rb') as f:
#     vp = pickle.load(f)
...
print(f"{'lapse_prob_right':<20} {'N/A':<15} {lapse_prob_right_mean:<15.6f} {'N/A':<15}")
print("="*60)
...
# %%
# %%
# Plot lapse probability right distribution separately
plt.figure(figsize=(6, 4))
# bins = np.arange(vanilla_lapse_prob_right_bounds[0], vanilla_lapse_prob_right_bounds[1], 1e-4)
bins = np.arange(vanilla_lapse_prob_right_bounds[0], 0.9, 1e-4)

mean_lapse_prob_right = np.mean(lapse_prob_right_samples)
plt.hist(lapse_prob_right_samples, bins=bins, density=True, histtype='step', color='red', linewidth=2)

plt.xlabel('Lapse probability', fontsize=12)
plt.ylabel('Density', fontsize=12)
plt.title(f'Lapse Probability Right Distribution - Batch {batch_name}, Animal {animal_ids[0]}', fontsize=13)
plt.xlim(0, 0.9)
plt.axvline(x=mean_lapse_prob_right, color='red', linestyle='--', linewidth=2, label=f'Mean: {mean_lapse_prob_right:.4f}')
plt.legend(fontsize=10)
plt.grid(axis='y', alpha=0.3)
plt.tight_layout()
plt.show()
...
def simulate_single_trial_vanilla(i):
    choice, rt, is_act = simulate_psiam_tied_rate_norm(
        ...
        0.0, dt, lapse_prob=0.0, lapse_prob_right=0.5, T_lapse_max=T_lapse_max
    )
    return {
        'choice': choice,
        'rt': rt,
...
def simulate_single_trial_lapse(i):
    choice, rt, is_act = simulate_psiam_tied_rate_norm(
        ...
        0.0, dt, lapse_prob=lapse_prob_mean, lapse_prob_right=lapse_prob_right_mean, T_lapse_max=T_lapse_max
    )
    return {
        'choice': choice,
        'rt': rt,
