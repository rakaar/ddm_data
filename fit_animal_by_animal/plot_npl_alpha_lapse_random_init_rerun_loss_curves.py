# %%
"""
Plot NPL+alpha+lapse random-init rerun loss curves.

This is the companion to the low-LR reference-init rerun plot. It focuses on the
animals whose low-LR reference-init rerun still restored an early 1k/2k window.
"""
...
RANDOM_ROOT = Path(
    os.environ.get(
        "NPL_ALPHA_LAPSE_RANDOM_ROOT",
        str(SCRIPT_DIR / "numpyro_svi_npl_alpha_lapse_condition_delay_random_plausible_low_lr_earlybest_reruns"),
    )
).expanduser()
REFERENCE_LOW_LR_ROOT = Path(
    os.environ.get(
        "NPL_ALPHA_LAPSE_LOW_LR_ROOT",
        str(SCRIPT_DIR / "numpyro_svi_npl_alpha_lapse_condition_delay_low_lr_patience12_min12k_restore_best_reruns"),
    )
).expanduser()

ORIGINAL_LEDGER = ORIGINAL_ROOT / "_batch_logs" / "batch_run_status.csv"
RANDOM_LEDGER = RANDOM_ROOT / "_batch_logs" / "random_plausible_low_lr_earlybest_status.csv"

SUMMARY_DIR = RANDOM_ROOT / "summary_figures"
SUMMARY_DIR.mkdir(parents=True, exist_ok=True)

RANDOM_FIG_PATH = SUMMARY_DIR / "npl_alpha_lapse_random_plausible_low_lr_earlybest_loss_grid.png"
HYBRID_FIG_PATH = SUMMARY_DIR / "npl_alpha_lapse_hybrid_random_plausible_earlybest_loss_grid.png"
RANDOM_SUMMARY_CSV = SUMMARY_DIR / "npl_alpha_lapse_random_plausible_low_lr_earlybest_loss_summary.csv"
HYBRID_SUMMARY_CSV = SUMMARY_DIR / "npl_alpha_lapse_hybrid_random_plausible_earlybest_loss_summary.csv"
COMPARISON_CSV = SUMMARY_DIR / "npl_alpha_lapse_random_vs_reference_init_loss_summary.csv"

LOSS_COLOR = "0.15"
BEST_COLOR = "tab:green"
STOP_COLOR = "tab:red"
RANDOM_TITLE_COLOR = "tab:purple"
...
def plot_grid(payloads, n_rows, n_cols, fig_path, title):
    fig, axes = plt.subplots(n_rows, n_cols, figsize=(3.2 * n_cols, 2.55 * n_rows), sharex=False, sharey=False)
    ...
    for ax, payload in zip(axes, payloads):
        ...
        title_color = RANDOM_TITLE_COLOR if payload["source"] == "random" else "black"
...
# %%
# =============================================================================
# Random-init rerun grid
# =============================================================================
random_ledger_df = load_ledger(RANDOM_LEDGER, expected_rows=4)
random_payloads = []
for _, ledger_row in random_ledger_df.sort_values("run_index").iterrows():
...
random_summary_df = payloads_to_summary(random_payloads)
random_summary_df.to_csv(RANDOM_SUMMARY_CSV, index=False)
print("Random-init rerun summaries:")
print(
    random_summary_df[
        [
            "batch_name",
...
print(f"Saved summary CSV: {RANDOM_SUMMARY_CSV}")

plot_grid(
    random_payloads,
    n_rows=2,
    n_cols=2,
    fig_path=RANDOM_FIG_PATH,
    title="Random-plausible init reruns for early-best NPL+alpha+lapse SVI loss curves",
)


# %%
# =============================================================================
# Compare random init against low-LR reference init for the same animals
# =============================================================================
comparison_rows = []
for payload in random_payloads:
    batch = payload["batch_name"]
    animal = payload["animal"]
    reference_payload = load_payload(REFERENCE_LOW_LR_ROOT, batch, animal, "low_lr_reference")
    comparison_rows.append(
        {
            "batch_name": batch,
            "animal": animal,
            "reference_best_step": reference_payload["best_step"],
            "reference_checked_step": reference_payload["checked_step"],
            "reference_best_mean_loss": reference_payload["best_loss"],
            "random_best_step": payload["best_step"],
            "random_checked_step": payload["checked_step"],
            "random_best_mean_loss": payload["best_loss"],
            "random_minus_reference_best_loss": payload["best_loss"] - reference_payload["best_loss"],
        }
    )

comparison_df = pd.DataFrame(comparison_rows)
comparison_df.to_csv(COMPARISON_CSV, index=False)
print("\nRandom init vs low-LR reference init:")
...
# %%
# =============================================================================
# Hybrid all-animal payloads: random-init reruns replace selected original panels
# =============================================================================
original_ledger_df = load_ledger(ORIGINAL_LEDGER, expected_rows=30)
random_pairs = {(payload["batch_name"], payload["animal"]) for payload in random_payloads}
...
for _, ledger_row in original_ledger_df.sort_values("run_index").iterrows():
    ...
    if (batch, animal) in random_pairs:
        hybrid_payloads.append(load_payload(RANDOM_ROOT, batch, animal, "random"))
    else:
...
plot_grid(
    ...
    title="Hybrid NPL+alpha+lapse SVI loss curves: random-init reruns replace early-best animals",
)
