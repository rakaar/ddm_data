# %%
"""
Run refreshed old VBMC NPL+lapse fits for the four suspicious animals.

This runner is deliberately narrow: it defaults to only the four animals used in
the SVI early-best versus random-init comparison.
"""
...
parser = argparse.ArgumentParser(description="Run refreshed old VBMC NPL+lapse fits for selected animals.")
...
single_script = SCRIPT_DIR / "vbmc_npl_lapse_refreshed_single_animal.py"
if not single_script.exists():
    raise FileNotFoundError(single_script)

run_id = time.strftime("%Y%m%d_%H%M%S")
ledger_path = log_dir / f"vbmc_npl_lapse_four_animal_status_{run_id}.csv"
latest_ledger_path = log_dir / "vbmc_npl_lapse_four_animal_status_latest.csv"
    ...
    rows.append(
        {
            ...
            "fit_log": "",
            "posterior_summary_csv": str(output_dir / "vbmc_norm_lapse_posterior_summary.csv"),
            "run_summary_csv": str(output_dir / "vbmc_norm_lapse_run_summary.csv"),
            "error": "",
        }
    )
