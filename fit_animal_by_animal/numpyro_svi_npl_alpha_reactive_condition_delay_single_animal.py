# %%
"""
Reactive-only NPL+alpha SVI fit for one animal.

The fit uses valid stimulus-relative RTs in [0.1, 1.0) seconds and contains:

    rate_lambda, T_0, theta_E, w, rate_norm_l, alpha,
    t_E_aff[ABL, signed ILD]

There is no proactive process and no `del_go`. The likelihood retains RT and
choice information and is normalized by the probability that either evidence
bound is reached inside the selected RT window.
"""
...
RT_LOWER = float(os.environ.get("NPL_REACTIVE_RT_LOWER", "0.100"))
RT_UPPER = float(os.environ.get("NPL_REACTIVE_RT_UPPER", "1.000"))
ABLS = [20.0, 40.0, 60.0]
EXPECTED_N_CONDITIONS = int(os.environ.get("NPL_REACTIVE_EXPECTED_N_CONDITIONS", "30"))
...
FULLRANK_COV_JITTER = float(os.environ.get("NPL_REACTIVE_FULLRANK_COV_JITTER", "1e-8"))
CORNER_MAX_SAMPLES = int(os.environ.get("NPL_REACTIVE_CORNER_MAX_SAMPLES", "5000"))

BATCH_CSV = REPO_DIR / "raw_data" / "batch_csvs" / f"batch_{BATCH_NAME}_valid_and_aborts.csv"
REFERENCE_ROOT = Path(
    os.environ.get(
        "NPL_REACTIVE_INIT_ROOT",
        str(
            SCRIPT_DIR
...
print(f"Reactive RT window: [{RT_LOWER:.3f}, {RT_UPPER:.3f}) s wrt stimulus")
...
print("Likelihood: pure reactive RT+choice; no proactive process and no del_go")

for required_path in [BATCH_CSV, REFERENCE_NPZ, REFERENCE_CONDITION_CSV]:
...
observed_choices = set(valid_df["choice"].unique().tolist())
if not observed_choices.issubset({-1, 1}):
    raise RuntimeError(f"Unexpected choice values in fitting data: {observed_choices}")
...
print(f"Valid trials before 100 ms removal: {len(animal_valid_df)}")
print(f"Retained fitting trials: {len(valid_df)}")
print(f"Removed valid RT < {1e3 * RT_LOWER:.0f} ms: {len(animal_valid_df) - len(valid_df)}")
...
with np.load(REFERENCE_NPZ) as saved:
    missing_keys = [
        name
        for name in [*reactive_utils.GLOBAL_PARAM_NAMES, "t_E_aff"]
...
n_reference_samples = len(reference_samples[reactive_utils.GLOBAL_PARAM_NAMES[0]])
...
init_values = reactive_utils.clip_init_to_hard_bounds(init_values)
...
latent_columns = []
for name in reactive_utils.GLOBAL_PARAM_NAMES:
    hard_low, hard_high = reactive_utils.GLOBAL_BOUNDS[name]["hard"]
    latent_columns.append(
        bounded_logit(reference_samples[name], hard_low, hard_high)
    )
hard_low, hard_high = reactive_utils.DELAY_BOUNDS["hard"]
...
latent_dim = len(reactive_utils.GLOBAL_PARAM_NAMES) + len(condition_table)
...
print(f"Parameter count: {len(reactive_utils.GLOBAL_PARAM_NAMES)} + {n_conditions} = {latent_dim}")
...
model = lambda data, n_conditions: (
    reactive_utils.npl_alpha_reactive_condition_delay_model(
        data,
        n_conditions,
        K_max=K_MAX,
    )
...
print(
    ...
    f"{reactive_utils.tree_all_finite(initial_grad)}"
)
if (
    not np.isfinite(float(initial_main_log_joint))
    or not reactive_utils.tree_all_finite(initial_grad)
):
...
if RUN_MAIN_SVI:
    ...
    main_guide = reactive_utils.make_guide(
        model,
        GUIDE_KIND,
        init_values,
...
if active_result is None:
    ...
    guide_params_np = reactive_utils.tree_to_numpy(active_result.params)
    finite_report_df, all_posterior_finite = (
        reactive_utils.finite_sample_report(posterior_np)
    )
    posterior_summary_df = reactive_utils.posterior_samples_to_frame(
        posterior_np,
        condition_table,
    )
    ...
    ax.set_title(f"{BATCH_NAME}/{ANIMAL} reactive-only NPL+alpha")
    ...
    if clean_global is not None:
        ...
        fig.suptitle(
            f"{BATCH_NAME}/{ANIMAL} reactive-only global posterior",
            y=1.02,
        )
    ...
    if selected_ids:
        ...
        if clean_selected is not None:
            ...
            fig.suptitle(
                f"{BATCH_NAME}/{ANIMAL} reactive-only selected-delay posterior",
                y=1.02,
            )
            fig.savefig(
                selected_corner_png,
    ...
    fig.suptitle(
        f"{BATCH_NAME}/{ANIMAL} reactive-only condition-delay posterior"
    )
    fig.tight_layout()
    ...
    posterior_mean_loglike = float(
        reactive_utils.npl_alpha_reactive_condition_delay_loglike(
            posterior_mean_params,
            full_data,
            K_max=K_MAX,
        )
    ...
    config = {
        ...
        "n_global_parameters": len(reactive_utils.GLOBAL_PARAM_NAMES),
        "n_condition_delays": int(n_conditions),
        "n_parameters": int(latent_dim),
        "includes_proactive_process": False,
        "includes_del_go": False,
    }
    metadata = {
        "schema_version": 1,
        "model_name": "npl_alpha_reactive_only_condition_delay_svi",
        ...
        "posterior_mean_reactive_loglike": posterior_mean_loglike,
    ...
    bundle = {
        **metadata,
        "note": (
            "Pure reactive NPL+alpha RT+choice SVI fit on valid trials with "
            "0.100 <= RTwrtStim < 1.000 s. The model has no proactive process "
            "and no del_go. It uses six shared parameters and one t_E_aff per "
            "observed signed ABL/ILD condition."
        ),
        "label": active_label,
    ...
    print(f"  posterior-mean reactive loglike: {posterior_mean_loglike:.6f}")
