# %%
"""
NumPyro SVI fit for one vanilla/IPL animal with condition-wise t_E_aff.

The fitted parameters are:

    rate_lambda, T_0, theta_E, w, del_go, t_E_aff[condition]

Abort/proactive parameters are fixed from the animal-wise abort fit. The
likelihood is the JAX port of the old vanilla branch in `time_vary_norm_utils`.
"""
...
OUTPUT_ROOT = Path(
    os.environ.get(
        ...
        str(SCRIPT_DIR / "numpyro_svi_vanilla_condition_delay_single_animal_outputs"),
    )
).expanduser()
...
if missing_modules:
    print("Missing dependencies for NumPyro vanilla SVI:")
...
import numpyro_vanilla_condition_delay_svi_utils as vanilla_utils
...
init_values = dict(vanilla_utils.DEFAULT_INIT_VALUES)
init_values["t_E_aff"] = condition_table["t_E_aff_s"].to_numpy(dtype=float)
init_values = vanilla_utils.clip_init_to_hard_bounds(init_values)
...
for param_name in vanilla_utils.GLOBAL_PARAM_NAMES:
...
# =============================================================================
model = lambda data, n_conditions: vanilla_utils.vanilla_condition_delay_model(
    data,
    n_conditions,
    K_max=K_MAX,
)
...
print(f"Initial main gradient finite: {vanilla_utils.tree_all_finite(initial_grad)}")
if not np.isfinite(float(initial_main_log_joint)) or not vanilla_utils.tree_all_finite(initial_grad):
...
if RUN_MAIN_SVI:
    ...
    main_guide = vanilla_utils.make_guide(
        model,
        GUIDE_KIND,
        init_values,
...
if active_result is None:
    ...
    guide_params_np = vanilla_utils.tree_to_numpy(active_result.params)

    finite_report_df, all_posterior_finite = vanilla_utils.finite_sample_report(posterior_np)
    posterior_summary_df = vanilla_utils.posterior_samples_to_frame(posterior_np, condition_table)
    ...
    fig.suptitle(f"{BATCH_NAME}/{ANIMAL} {active_label} vanilla condition-delay intervals")
    fig.tight_layout()
    fig.savefig(delay_png, dpi=200)

    bundle = {
        "schema_version": 1,
        "model_name": "vanilla_ipl_condition_delay_svi",
        "note": (
            "NumPyro SVI fit of vanilla/IPL parameters with condition-wise t_E_aff. "
