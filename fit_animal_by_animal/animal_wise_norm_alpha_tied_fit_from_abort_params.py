# %%
"""
Animal-wise alpha-normalized TIED fit using previously fitted proactive parameters.

This standalone fit adds alpha to the normalized TIED rate model while loading
V_A, theta_A, and t_A_aff from an existing animal-wise result pickle.
"""
import os
import pickle
import random
...
FINISHED_ALPHA_RESULT_KEY = 'vbmc_norm_alpha_tied_results'
K_max = 10
...
def alpha_result_pkl_path(batch_name, animal):
    return os.path.join(output_dir, f'results_{batch_name}_animal_{animal}_NORM_ALPHA_FROM_ABORTS.pkl')
...
def compute_loglike_norm_alpha_fn(row, rate_lambda, T_0, theta_E, Z_E, t_E_aff, del_go, rate_norm_l, alpha, t_trunc):
    rt = row['TotalFixTime']
...
def vbmc_norm_alpha_tied_loglike_fn(params):
    rate_lambda, T_0, theta_E, w, t_E_aff, del_go, rate_norm_l, alpha = params
    Z_E = (w - 0.5) * 2 * theta_E
    all_loglike = Parallel(n_jobs=30)(
        delayed(compute_loglike_norm_alpha_fn)(
            row, rate_lambda, T_0, theta_E, Z_E, t_E_aff, del_go, rate_norm_l, alpha, T_trunc
        )
...
for batch_name, animal in batch_animal_pairs:
    ...
    pdf_filename = os.path.join(output_dir, f'results_{batch_name}_animal_{animal}_NORM_ALPHA_FROM_ABORTS.pdf')
    pdf = PdfPages(pdf_filename)

    fig_text = plt.figure(figsize=(8.5, 11))
    fig_text.clf()
    fig_text.text(0.1, 0.9, "Alpha-Normalized TIED From Loaded Abort Parameters", fontsize=20, weight='bold')
    ...
    if (
        ...
        x_0 = np.array([
            ...
            np.mean(norm_tied_samples['t_E_aff_samples']),
            np.mean(norm_tied_samples['del_go_samples']),
            np.mean(norm_tied_samples['rate_norm_l_samples']),
            1.0
        ])
        print("Initializing alpha-normalized fit from existing normalized TIED posterior mean.")
    else:
        x_0 = np.array([
            2.3,
            100 * 1e-3,
    ...
    print(f"  w           = {x_0[3]:.5f}")
    print(f"  t_E_aff     = {1e3*x_0[4]:.5f} ms")
    print(f"  del_go      = {x_0[5]:.5f}")
    print(f"  rate_norm_l = {x_0[6]:.5f}")
    print(f"  alpha       = {x_0[7]:.5f}")
    ...
    vbmc = VBMC(
        ...
        norm_tied_plb,
        norm_tied_pub,
        options={'display': 'on', 'max_fun_evals': 200 * (2 + 8)}
    )
    vp, results = vbmc.optimize()
    vbmc.save(
        os.path.join(output_dir, f'vbmc_PKL_file_norm_alpha_tied_results_batch_{batch_name}_animal_{animal}_FROM_ABORTS.pkl'),
        overwrite=True
    )
    ...
    norm_tied_corner_fig.suptitle(f'Alpha-Normalized Tied Posterior (Animal: {animal})', y=1.02)
    vp_samples[:, 1] /= 1e3
    ...
    norm_alpha_tied_loglike = vbmc_norm_alpha_tied_active_loglike_fn([
        rate_lambda, T_0, theta_E, w, t_E_aff, del_go, rate_norm_l, alpha
    ])
    save_posterior_summary_page(
        pdf_pages=pdf,
        title=f'Alpha-Normalized Tied Model - Posterior Means ({animal})',
    ...
    theory_results_up_and_down, theory_time_axis, bins, bin_centers = plot_rt_distributions(
        ...
        animal, pdf, model_name="Alpha-Normalized Tied"
    )

    plot_tachometric_curves(
        sim_df_1, data_df_1, ILD_arr, ABL_arr, theory_results_up_and_down,
        theory_time_axis, bins, animal, pdf, model_name="Alpha-Normalized Tied"
    )
    ...
    save_dict = {
        ...
        'vbmc_norm_alpha_tied_results': vbmc_norm_alpha_tied_results
    }
    ...
    if tied_df is not None:
        ...
        print(f"Added alpha-normalized TIED results table to PDF for animal {animal}")
