...
# sim_results_df_valid_lt_1 = sim_results_df_valid[sim_results_df_valid['rt'] - sim_results_df_valid['t_stim'] <= 1]
# sim_rt = sim_results_df_valid_lt_1['rt'] - sim_results_df_valid_lt_1['t_stim']
# # plt.hist(sim_rt, bins=np.arange(-0.12, 1, 0.02), density=True, histtype='step', label='Simulated', alpha=0.4, lw=3)

# # KDE smoothing
# kde = gaussian_kde(sim_rt)
# x_vals = np.arange(-0.12, 1, 0.01)
# plt.plot(x_vals, kde(x_vals), color='green', label='Simulated (KDE)', alpha=0.4, lw=3)
# plt.legend()
# plt.show()

# # %%
# # sim results for sure check
# abort_params, tied_params, rate_norm_l, is_norm = get_params_from_animal_pkl_file('SD', 50)
# p,c,ts_samp = get_P_A_C_A('SD', 50, abort_params)
# ABL = 20
# ILD = 1
# t_pts_0_1,ud = get_theoretical_RTD_from_params(p, c, ts_samp, abort_params, tied_params, rate_norm_l, is_norm, ABL, ILD)
...
# print(f'is_norm={is_norm}, rate_norm_l={rate_norm_l}')
...
# plt.hist(sim_rt, bins=np.arange(-0.12, 1, 0.02), density=True, histtype='step', label='Simulated')

# # KDE smoothing
# kde = gaussian_kde(sim_rt)
# x_vals = np.arange(-0.12, 1, 0.01)
# plt.plot(x_vals, kde(x_vals), color='blue', label='Simulated (KDE)')
