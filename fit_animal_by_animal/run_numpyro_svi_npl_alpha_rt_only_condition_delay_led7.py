...
# %%
"""
Sequential tmux-friendly runner for six LED7 NPL+alpha RT-only fits.

The selected process mode is either reactive-only or fixed
proactive+reactive. Both fits use successful trials with
0 <= RTwrtStim < 1 s and ignore observed choice.
"""
...
FIT_SCRIPT = (
    SCRIPT_DIR
    / "numpyro_svi_npl_alpha_rt_only_condition_delay_single_animal.py"
)
REFERENCE_ROOT = (
    SCRIPT_DIR
...
parser = argparse.ArgumentParser(
    description=(
        "Run six LED7 NPL+alpha RT-only condition-delay SVI fits "
        "sequentially."
    )
)
parser.add_argument(
    "--process-mode",
    choices=PROCESS_MODES,
    default="reactive_only",
    help="Use a pure reactive RT density or a fixed proactive+reactive race.",
)
parser.add_argument(
    "--dry-run",
    action="store_true",
...
default_output_root = (
    SCRIPT_DIR
    / (
        f"numpyro_svi_npl_alpha_rt_only_{args.process_mode}_0_to_1s_"
        "condition_delay_patience12_min50k_restore_best_outputs"
    )
)
output_root = Path(args.output_root or default_output_root).expanduser()
...
for run_index, animal in enumerate(selected_animals, start=1):
    ...
    if args.process_mode == "proactive_reactive":
        abort_pkl = (
            ABORT_ROOT / f"results_{BATCH_NAME}_animal_{animal}.pkl"
        )
        if not abort_pkl.exists():
            preflight_errors.append(
                f"{BATCH_NAME}/{animal} missing {abort_pkl}"
            )
    ...
    rows.append(
        {
            ...
            "n_retained_rt_0_to_1": len(retained_df),
...
print(
    f"Fit: {args.process_mode} RT-only, "
    "0.000 <= RTwrtStim < 1.000 s, observed choice ignored, "
    "six shared NPL+alpha parameters + 30 condition delays, no del_go"
)
print(
    "Stopping: "
    ...
    env.update(
        {
            ...
            "NPL_RT_ONLY_PROCESS_MODE": args.process_mode,
            "NPL_RT_ONLY_INIT_ROOT": str(REFERENCE_ROOT),
            "NPL_RT_ONLY_RT_LOWER": str(RT_LOWER),
            "NPL_RT_ONLY_RT_UPPER": str(RT_UPPER),
...
print("\n" + "=" * 80)
print(f"LED7 {args.process_mode} RT-only batch summary")
print("=" * 80)
status_counts = {}
for row in rows:
    status = row["status"]
