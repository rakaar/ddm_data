...
PSY_OUTPUT_PKL = os.path.join(OUTPUT_DIR, "npl_alpha_ild2_psy_fig4_data.pkl")
SLOPES_OUTPUT_PKL = os.path.join(OUTPUT_DIR, "npl_alpha_ild2_slopes_fig4_data.pkl")
THEORETICAL_PSY_OUTPUT_PKL = os.path.join(OUTPUT_DIR, "theoretical_psychometric_data_npl_alpha_ild2.pkl")
...
def process_animal(batch_animal_pair, result_paths):
    batch_name, animal_id = batch_animal_pair
    print(f"Processing psychometric data for {batch_name}, animal {animal_id}")
    ...
    for ABL in ABL_arr:
        ...
        for ILD in ILD_arr:
            try:
                ...
                print(f"  failed theory {batch_name}/{animal_id} ABL={ABL} ILD={ILD}: {exc}")
...
print(f"Using {len(batch_animal_pairs)} matched animals for psychometric/slopes")
print(f"Running with {N_JOBS} jobs")

results = Parallel(n_jobs=N_JOBS, verbose=10)(
    delayed(process_animal)(pair, result_paths) for pair in batch_animal_pairs
)
...
# %%
def extract_slopes(data_dict, fit_data_key):
    slopes = {}
...
slopes_data = extract_slopes(psychometric_data, "empirical")
slopes_ild2 = extract_slopes(theoretical_psychometric_data, "theoretical")
common_pairs = sorted(set(slopes_data) & set(slopes_ild2), key=lambda item: (DESIRED_BATCHES.index(item[0]), item[1]))
...
ild2_means = np.array([np.nanmean([slopes_ild2[pair][ABL] for ABL in ABL_arr]) for pair in common_pairs_sorted])

slopes_plot_data = {
    "slopes_data": slopes_data,
    "slopes_npl_alpha_ild2": slopes_ild2,
    "common_pairs_sorted": common_pairs_sorted,
    "data_means": data_means,
    "npl_alpha_ild2_means": ild2_means,
    "norm_means": ild2_means,
    "ABL_arr": ABL_arr,
    "MODEL_TYPE": "npl_alpha_ild2_delay",
}
