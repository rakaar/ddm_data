# %%
"""
Validate the JAX vanilla/IPL likelihood port against the old NumPy functions.

This script compares per-trial normalized log likelihoods from:

    time_vary_norm_utils.up_or_down_RTs_fit_fn
    time_vary_norm_utils.cum_pro_and_reactive_time_vary_fn

against the JAX port in `numpyro_vanilla_condition_delay_svi_utils.py`.
"""
...
MAX_ROWS_PER_ANIMAL = int(os.environ.get("VANILLA_JAX_VALIDATE_MAX_ROWS", "300"))
RNG_SEED = int(os.environ.get("VANILLA_JAX_VALIDATE_SEED", "0"))
K_MAX = int(os.environ.get("K_MAX", "10"))
LOG_TOL = float(os.environ.get("VANILLA_JAX_VALIDATE_LOG_TOL", "1e-6"))
TOTAL_LOG_TOL = float(os.environ.get("VANILLA_JAX_VALIDATE_TOTAL_LOG_TOL", "1e-4"))
...
def old_numpy_log_terms(df, condition_table, abort_params, T_trunc, params):
    ...
    for row_idx, row in df.iterrows():
        ...
        log_pdf = np.log(max(pdf / (trunc_factor + 1e-20), 1e-50))
        rows.append(
            {
                "row_idx": int(row_idx),
                "pdf_numpy": float(pdf),
...
for batch_name, animal in TEST_ANIMALS:
    ...
    pdf_jax, trunc_jax, log_pdf_jax = vanilla_utils.vanilla_condition_delay_loglike_terms(
        init_values,
        data,
        K_max=K_MAX,
    )
    ...
    log_joint = vanilla_utils.vanilla_condition_delay_loglike(init_values, data, K_max=K_MAX)
    grad = jax.grad(lambda values: vanilla_utils.vanilla_condition_delay_loglike(values, data, K_max=K_MAX))(
        init_values
    )
    gradients_finite = vanilla_utils.tree_all_finite(grad)
