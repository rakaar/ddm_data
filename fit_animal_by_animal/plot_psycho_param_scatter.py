#!/usr/bin/env python3
"""
Create 4 scatter plots (one per parameter: rate_lambda, T_0, theta_E, w).
...
Reads VBMC psycho-fit pickles saved by
...
Usage:
  python plot_psycho_param_scatter.py [--n-samples 50000] [--out psycho_param_scatter.png]
"""
...
# Directory with psycho-fit pickles
PSYCHO_DIR = \
    '/home/rlab/raghavendra/ddm_data/fit_valid_trials/psycho_fits_4-params-del_E_go_fixed_as_avg'
...
BOUNDS = {
    ...
    'T_0': {
        'bounds': (0.01 * (1e-3), 2 * (1e-3)),
        'plausible': (0.1 * (1e-3), 0.9 * (1e-3)),
    },
    'w': {
        'bounds': (0.2, 0.8),
        'plausible': (0.4, 0.6),
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description='Plot psycho-fit param means per animal (4 scatter plots)')
...
def plot_scatter(df: pd.DataFrame, out_path: str):
    params = ['rate_lambda', 'T_0', 'theta_E', 'w']

    fig, axes = plt.subplots(2, 2, figsize=(14, max(10, len(df) * 0.35)))
    axes = axes.ravel()
    ...
    for ax, p in zip(axes, params):
        ...
        # Convert to ms if T_0
        if p == 'T_0':
            pb_vals = [v * 1000.0 for v in pb]
            b_vals = [v * 1000.0 for v in b]
        else:
            pb_vals = list(pb)
            b_vals = list(b)
        for x in pb_vals:
        ...
        ax.set_xlabel('T_0 (ms)' if p == 'T_0' else p)
...
def plot_param_vs_param(df: pd.DataFrame, out_path: str, df_overlay: pd.DataFrame = None):
    """
    Create a 4x4 grid of parameter vs parameter scatter plots.
    Each subplot shows one animal's mean param1 vs mean param2 values.
    """
    params = ['rate_lambda', 'T_0', 'theta_E', 'w']
    
    # Convert T_0 to ms for plotting
    df_plot = df.copy()
    df_plot['T_0'] = df_plot['T_0'] * 1000.0
    df_overlay_plot = None
    if df_overlay is not None and not df_overlay.empty:
        df_overlay_plot = df_overlay.copy()
        if 'T_0' in df_overlay_plot.columns:
            df_overlay_plot['T_0'] = df_overlay_plot['T_0'] * 1000.0
    ...
    for i, p1 in enumerate(params):
        for j, p2 in enumerate(params):
            ...
            # Convert T_0 bounds to ms if needed
            p1_bounds = BOUNDS[p1]['bounds']
            p2_bounds = BOUNDS[p2]['bounds']
            
            if p1 == 'T_0':
                p1_bounds = (p1_bounds[0] * 1000.0, p1_bounds[1] * 1000.0)
            if p2 == 'T_0':
                p2_bounds = (p2_bounds[0] * 1000.0, p2_bounds[1] * 1000.0)
            ...
            if p1 == 'T_0' and v1_bounds is not None:
                v1_bounds = (v1_bounds[0] * 1000.0, v1_bounds[1] * 1000.0)
            if p2 == 'T_0' and v2_bounds is not None:
                v2_bounds = (v2_bounds[0] * 1000.0, v2_bounds[1] * 1000.0)
            ...
            ax.set_xlabel(p1 if p1 != 'T_0' else 'T_0 (ms)')
            ax.set_ylabel(p2 if p2 != 'T_0' else 'T_0 (ms)')
