...
for animal_idx in [-1]:
    ...
    ### Generate and add parameter summary tables to PDF ###
    # Function to create a table for abort results
    ...
    # Function to render DataFrame as PDF table
    ...
    # Function to create a table for tied models
    def create_tied_table(all_results):
        ...
        # Use specific order for parameters
        desired_order = [
    ...
    # Create and add tables to PDF
    print("\nGenerating parameter summary tables...")
    
    # Create abort results table if present
    if 'vbmc_aborts_results' in locals():
    ...
    # Create tied models table
    results_for_table = {
        'vbmc_vanilla_tied_results': vbmc_vanilla_tied_results if 'vbmc_vanilla_tied_results' in locals() else None,
        'vbmc_norm_tied_results': vbmc_norm_tied_results if 'vbmc_norm_tied_results' in locals() else None,
        'vbmc_time_vary_norm_tied_results': vbmc_time_vary_norm_tied_results if 'vbmc_time_vary_norm_tied_results' in locals() else None
    }
    # Filter out None values
    results_for_table = {k: v for k, v in results_for_table.items() if v is not None}
    
    if results_for_table:
        tied_df = create_tied_table(results_for_table)
        if tied_df is not None:
            render_df_to_pdf(tied_df, f"Tied Models Comparison (Animal: {animal})", pdf)
    
    print("Parameter summary tables added to PDF\n")
