...
def get_theoretical_psychometric_data(batch_name, animal_id, ABL):
    try:
        ...
        ild_values = np.array(ILD_arr, dtype=float)
        right_choice_probs = []
        for ild in ild_values:
            try:
...
# %%
# x,y = process_batch_animal_theoretical_psychometric(['LED34_even', '52'])
# %%
...
with open('small_ILDs_only_vanila_psy_fig2_data.pkl', 'wb') as f:
...
# plt.title('Average Psychometric Curves (All ABLs)')
plt.xticks([-3,-1,1,3], fontsize=14)
...
ax.spines['right'].set_visible(False)
plt.tight_layout()
plt.savefig('small_ILDs_only_N_26_psychometric_curve_vanila_data.pdf')
plt.show()
# %%
...
ax.spines['right'].set_visible(False)
plt.tight_layout()
plt.savefig('small_ILDs_only_empirical_psychometric_fit.pdf', bbox_inches='tight')
# plt.legend()
plt.show()
# %%
