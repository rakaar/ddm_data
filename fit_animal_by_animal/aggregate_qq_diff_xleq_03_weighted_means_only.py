...
abs_ILD_colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd']
abs_ILD_color_map = {abs_ILD: abs_ILD_colors[i] for i, abs_ILD in enumerate(abs_ILDs)}
...
for j, abl in enumerate([20, 40]):
    ...
    for i, abs_ILD in enumerate(abs_ILDs):
        ...
        ax.plot(mean_Q_60[mask], mean_diff[mask], color=abs_ILD_color_map[abs_ILD], label=f'|ILD|={abs_ILD}', linewidth=2)
        # Weighted linear regression
        if np.sum(mask) > 1:
            fit = np.polyfit(mean_Q_60[mask], mean_diff[mask], 1, w=weights[mask])
            slope, intercept = fit
            y_fit = slope * mean_Q_60[mask] + intercept
            ax.plot(mean_Q_60[mask], y_fit, linestyle=':', color=abs_ILD_color_map[abs_ILD], linewidth=2)
            ...
            fit_texts.append(f"|ILD|={abs_ILD}: slope={slope:.3f}, int={intercept:.3f}, wR²={r2_weighted:.3f}")
    # Print fit statistics for this panel
    if fit_texts:
        print(f"Panel: {labels[abl]}")
        for stat in fit_texts:
            print(stat)
...
# Print fit statistics for each panel
