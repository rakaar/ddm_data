...
# We load both ILD=+16 and ILD=-16 to properly average over both
ABL_arr = [20, 40, 60]
ILD_target = 16  # We load both +16 and -16

# Store: {(batch, animal): {ABL: {'gamma_pos', 'gamma_neg', 'omega', 'w', ...}}}
animal_params_ild16 = {}
...
for batch_name, animal_id in batch_animal_pairs:
    ...
    for ABL in ABL_arr:
        ...
        if cond_params_pos is None or cond_params_neg is None:
            print(f"  Missing cond-fit for {batch_name}/{animal_id_str} ABL={ABL} |ILD|={ILD_target}")
            continue
        
        animal_params_ild16[(batch_name, animal_id)][ABL] = {
            'gamma_pos': cond_params_pos['gamma'],
            'omega_pos': cond_params_pos['omega'],
            'gamma_neg': cond_params_neg['gamma'],
            'omega_neg': cond_params_neg['omega'],
...
for abl, params in animal_params_ild16[sample_key].items():
    print(f"  ABL={abl}: gamma_pos={params['gamma_pos']:.3f}, gamma_neg={params['gamma_neg']:.3f}, w={params['w']:.3f}")
...
for animal_num, ((batch_name, animal_id), abl_dict) in enumerate(animal_params_ild16.items(), 1):
    ...
    # Compute CDF for each ABL, averaging over ILD=+16 and ILD=-16
    for ABL in ABL_arr:
        ...
        omega_pos = params['omega_pos']
        gamma_neg = params['gamma_neg']
        omega_neg = params['omega_neg']
        w = params['w']
        t_E_aff = params['t_E_aff']
        
        # Compute CDF samples for each t_stim, for both ILD signs
        ...
        for idx, t_stim in enumerate(t_stim_samples):
            ...
            for t_idx, t_fix in enumerate(t_pts_wrt_fix):
                # Abort CDF (same for both ILD signs)
                c_A = cum_A_t_fn(t_fix - t_A_aff, V_A, theta_A)
                
                t_evidence = t_pts_wrt_stim[t_idx] - t_E_aff  # time for evidence accumulator
                
                # Evidence CDF for ILD=+16
                c_E_up_pos = CDF_E_minus_small_t_NORM_omega_gamma_with_w_fn(t_evidence, gamma_pos, omega_pos, 1, w, K_max)
                c_E_down_pos = CDF_E_minus_small_t_NORM_omega_gamma_with_w_fn(t_evidence, gamma_pos, omega_pos, -1, w, K_max)
                c_E_pos = c_E_up_pos + c_E_down_pos
                cdf_samples_pos[idx, t_idx] = c_A + c_E_pos - c_A * c_E_pos
                
                # Evidence CDF for ILD=-16
                c_E_up_neg = CDF_E_minus_small_t_NORM_omega_gamma_with_w_fn(t_evidence, gamma_neg, omega_neg, 1, w, K_max)
                c_E_down_neg = CDF_E_minus_small_t_NORM_omega_gamma_with_w_fn(t_evidence, gamma_neg, omega_neg, -1, w, K_max)
                c_E_neg = c_E_up_neg + c_E_down_neg
                cdf_samples_neg[idx, t_idx] = c_A + c_E_neg - c_A * c_E_neg
...
with open('ILD_16_cond_fit_quantiles.pkl', 'wb') as f:
    pickle.dump(quantiles_data, f)
print("Saved quantiles data to ILD_16_cond_fit_quantiles.pkl")
