# %%
"""
Animal-wise alpha-normalized TIED fit with fixed ABL-specific ILD-dependent delays.
...
evidence-afferent delay is fitted separately for ABL 20, 40, and 60:

    delay_ms(ABL, ILD) = bias_ms[ABL] + c1[ABL] * |ILD| + c2[ABL] * ILD^2
"""
import os
import pickle
import random
...
batch_dir = os.environ.get(
    ...
    os.path.join(SCRIPT_DIR, 'batch_csvs')
)
abort_params_dir = os.environ.get(
    'ABORT_PARAMS_DIR_OVERRIDE',
    os.path.join(REPO_DIR, 'aborts_ipl_npl_time_fit_results')
)
output_dir = os.path.join(SCRIPT_DIR, 'NPL_alpha_ABL_specific_ILD2_delay_fit_results')
...
FINISHED_ALPHA_RESULT_KEY = 'vbmc_norm_alpha_abl_specific_ild2_delay_tied_results'
...
def alpha_result_pkl_path(batch_name, animal):
    return os.path.join(output_dir, f'results_{batch_name}_animal_{animal}_NORM_ALPHA_ABL_SPECIFIC_ILD2_DELAY_FROM_ABORTS.pkl')
...
def is_finished_alpha_result(pkl_path):
    ...
    required_keys = [
        ...
        'alpha_samples',
        'delay_abl_levels',
        'bias_ms_by_abl_samples',
        'abs_ild_delay_coeff_ms_per_unit_by_abl_samples',
        'ild2_delay_coeff_ms_per_unit2_by_abl_samples',
        'loglike',
    ]
    ...
    saved_delay_abl_levels = np.asarray(alpha_results['delay_abl_levels'], dtype=float)
    if (
            saved_delay_abl_levels.shape != HARD_CODED_DELAY_ABL_LEVELS.shape or
            not np.allclose(saved_delay_abl_levels, HARD_CODED_DELAY_ABL_LEVELS)):
        print(
            f"WARNING: Existing result pkl has delay_abl_levels={saved_delay_abl_levels.tolist()}; "
            f"expected {HARD_CODED_DELAY_ABL_LEVELS.tolist()}; will rerun: {os.path.relpath(pkl_path, SCRIPT_DIR)}"
        )
        return False
...
delay_abl_levels = HARD_CODED_DELAY_ABL_LEVELS.copy()
...
def unpack_norm_alpha_abl_specific_params(params):
    params = np.asarray(params, dtype=float)
    n_abl = len(delay_abl_levels)
    (
        rate_lambda,
        T_0,
        theta_E,
    ...
    delay_params_by_abl = params[7:].reshape(n_abl, 3)
    bias_ms_by_abl = delay_params_by_abl[:, 0]
    abs_ild_delay_coeff_ms_per_unit_by_abl = delay_params_by_abl[:, 1]
    ild2_delay_coeff_ms_per_unit2_by_abl = delay_params_by_abl[:, 2]
    return (
        rate_lambda,
        T_0,
        theta_E,
        ...
        alpha,
        bias_ms_by_abl,
        abs_ild_delay_coeff_ms_per_unit_by_abl,
        ild2_delay_coeff_ms_per_unit2_by_abl,
    )


def get_norm_alpha_abl_specific_param_names(delay_abl_levels_arg):
    names = [
        'rate_lambda',
        'T_0',
        'theta_E',
    ...
    for abl in delay_abl_levels_arg:
        abl_label = format_abl_suffix(abl)
        names.extend([
            f'bias_ms_ABL_{abl_label}',
            f'abs_ild_coeff_ABL_{abl_label}',
            f'ild2_coeff_ABL_{abl_label}',
        ])
    return names


def get_delay_abl_indices(abl, delay_abl_levels_arg):
    delay_abl_levels_arg = np.asarray(delay_abl_levels_arg, dtype=float)
    if len(delay_abl_levels_arg) == 0:
        raise ValueError("No delay ABL levels are available for this animal.")
    abl_arr = np.asarray(abl, dtype=float)
    idx = np.searchsorted(delay_abl_levels_arg, abl_arr)
    idx = np.clip(idx, 0, len(delay_abl_levels_arg) - 1)
    if np.any(delay_abl_levels_arg[idx] != abl_arr):
        raise ValueError(f"Encountered ABL outside delay_abl_levels: {np.unique(abl_arr).tolist()}")
    return idx
...
def get_t_E_aff_from_abl_ild(
    ...
    abs_ild = np.abs(ild)
    abl_idx = get_delay_abl_indices(abl, delay_abl_levels_arg)
    delay_ms = (
        np.asarray(bias_ms_by_abl, dtype=float)[abl_idx]
        + np.asarray(abs_ild_delay_coeff_ms_per_unit_by_abl, dtype=float)[abl_idx] * abs_ild
        + np.asarray(ild2_delay_coeff_ms_per_unit2_by_abl, dtype=float)[abl_idx] * (ild ** 2)
    )
...
def get_delay_ms_from_condition_pairs(
    ...
    abl_idx = get_delay_abl_indices(condition_pairs[:, 0], delay_abl_levels_arg)
    return (
        np.asarray(bias_ms_by_abl, dtype=float)[abl_idx]
        + np.asarray(abs_ild_delay_coeff_ms_per_unit_by_abl, dtype=float)[abl_idx] * condition_pairs[:, 1]
        + np.asarray(ild2_delay_coeff_ms_per_unit2_by_abl, dtype=float)[abl_idx] * condition_pairs[:, 2]
    )


def compute_loglike_norm_alpha_fn(
        row, rate_lambda, T_0, theta_E, Z_E,
        bias_ms_by_abl, abs_ild_delay_coeff_ms_per_unit_by_abl,
        ild2_delay_coeff_ms_per_unit2_by_abl,
...
def vbmc_norm_alpha_tied_loglike_fn(params):
    (
        ...
        abs_ild_delay_coeff_ms_per_unit_by_abl,
        ild2_delay_coeff_ms_per_unit2_by_abl,
    ) = unpack_norm_alpha_abl_specific_params(params)
...
def vbmc_prior_norm_alpha_tied_fn(params):
    ...
    bias_ms_logpdf = sum(
        ...
        for bias_ms in bias_ms_by_abl
    )

    abs_ild_delay_coeff_logpdf = sum(
        trapezoidal_logpdf(
        ...
        for abs_ild_delay_coeff_ms_per_unit in abs_ild_delay_coeff_ms_per_unit_by_abl
    )

    ild2_delay_coeff_logpdf = sum(
        trapezoidal_logpdf(
        ...
        for ild2_delay_coeff_ms_per_unit2 in ild2_delay_coeff_ms_per_unit2_by_abl
    )
...
def build_norm_tied_bounds_for_delay_abl_levels(delay_abl_levels_arg):
    delay_lb = []
    delay_ub = []
    delay_plb = []
    delay_pub = []
    for _ in delay_abl_levels_arg:
        delay_lb.extend([
            norm_tied_bias_ms_bounds[0],
            norm_tied_abs_ild_delay_coeff_ms_per_unit_bounds[0],
            norm_tied_ild2_delay_coeff_ms_per_unit2_bounds[0],
    ...
    return (
        np.concatenate([norm_tied_shared_lb, np.array(delay_lb, dtype=float)]),
        np.concatenate([norm_tied_shared_ub, np.array(delay_ub, dtype=float)]),
        np.concatenate([norm_tied_shared_plb, np.array(delay_plb, dtype=float)]),
        np.concatenate([norm_tied_shared_pub, np.array(delay_pub, dtype=float)]),
    )
...
for batch_name, animal in batch_animal_pairs:
    ...
    pdf_filename = os.path.join(output_dir, f'results_{batch_name}_animal_{animal}_NORM_ALPHA_ABL_SPECIFIC_ILD2_DELAY_FROM_ABORTS.pdf')
    pdf = PdfPages(pdf_filename)

    fig_text = plt.figure(figsize=(8.5, 11))
    fig_text.clf()
    fig_text.text(0.1, 0.9, "Alpha-Normalized TIED With ABL-Specific ILD2 Delay From Loaded Abort Parameters", fontsize=18, weight='bold')
    ...
    fig_text.text(0.1, 0.58, "delay_ms(ABL, ILD) = bias[ABL] + c1[ABL]*|ILD| + c2[ABL]*ILD^2", fontsize=12)
    fig_text.text(0.1, 0.53, f"Delay ABLs hardcoded to {HARD_CODED_DELAY_ABL_LEVELS.astype(int).tolist()}", fontsize=12)
    ...
    delay_abl_levels = HARD_CODED_DELAY_ABL_LEVELS.copy()
    norm_tied_lb, norm_tied_ub, norm_tied_plb, norm_tied_pub = build_norm_tied_bounds_for_delay_abl_levels(delay_abl_levels)
    abs_ild_levels_for_delay_check = np.sort(df_valid_animal_less_than_1['abs_ILD'].dropna().unique()).astype(float)
    observed_delay_condition_pairs = np.array(
        [
            [abl, abs_ild, abs_ild ** 2]
            for abl in delay_abl_levels
            for abs_ild in abs_ild_levels_for_delay_check
        ],
        dtype=float
    )
    print(f"Hardcoded delay ABL levels used for fit: {delay_abl_levels.tolist()}")
    print(f"Delay conditions checked for nonnegative delay: {observed_delay_condition_pairs.tolist()}")
    ...
    for abl_idx, abl in enumerate(delay_abl_levels):
        start_idx = 7 + 3 * abl_idx
        print(
            f"  ABL {format_abl_suffix(abl)} delay: "
            f"bias_ms={x_0[start_idx]:.5f}, "
            f"|ILD| coeff={x_0[start_idx + 1]:.5f} ms/unit, "
    ...
    vbmc.save(
        os.path.join(output_dir, f'vbmc_PKL_file_norm_alpha_abl_specific_ild2_delay_tied_results_batch_{batch_name}_animal_{animal}_FROM_ABORTS.pkl'),
        overwrite=True
    )
    ...
    for abl in delay_abl_levels:
        abl_label = format_abl_suffix(abl)
        param_labels.extend([
            f'bias ABL {abl_label} (ms)',
            f'|ILD| ABL {abl_label}',
            f'ILD^2 ABL {abl_label}',
        ])
    ...
    norm_tied_corner_fig.suptitle(f'Alpha-Normalized Tied ABL-Specific ILD2-Delay Posterior (Animal: {animal})', y=1.02)
    vp_samples[:, 1] /= 1e3
    ...
    bias_ms_by_abl_samples = vp_samples[:, 7::3]
    abs_ild_delay_coeff_ms_per_unit_by_abl_samples = vp_samples[:, 8::3]
    ild2_delay_coeff_ms_per_unit2_by_abl_samples = vp_samples[:, 9::3]
    bias_ms_by_abl = bias_ms_by_abl_samples.mean(axis=0)
    abs_ild_delay_coeff_ms_per_unit_by_abl = abs_ild_delay_coeff_ms_per_unit_by_abl_samples.mean(axis=0)
    ild2_delay_coeff_ms_per_unit2_by_abl = ild2_delay_coeff_ms_per_unit2_by_abl_samples.mean(axis=0)
    ...
    for abl_idx, abl in enumerate(delay_abl_levels):
        print(
            f"ABL {format_abl_suffix(abl)} delay: "
            f"bias_ms={bias_ms_by_abl[abl_idx]:.5f}, "
            f"|ILD| coeff={abs_ild_delay_coeff_ms_per_unit_by_abl[abl_idx]:.5f} ms/unit, "
            f"ILD^2 coeff={ild2_delay_coeff_ms_per_unit2_by_abl[abl_idx]:.5f} ms/unit^2"
        )

    posterior_mean_params = np.concatenate([
        np.array([rate_lambda, T_0, theta_E, w, del_go, rate_norm_l, alpha], dtype=float),
        np.column_stack([
            bias_ms_by_abl,
            abs_ild_delay_coeff_ms_per_unit_by_abl,
            ild2_delay_coeff_ms_per_unit2_by_abl,
    ...
    for abl_idx, abl in enumerate(delay_abl_levels):
        abl_label = format_abl_suffix(abl)
        posterior_means[f'bias_ms_ABL_{abl_label}'] = bias_ms_by_abl[abl_idx]
        posterior_means[f'abs_ild_coeff_ABL_{abl_label}'] = abs_ild_delay_coeff_ms_per_unit_by_abl[abl_idx]
        posterior_means[f'ild2_coeff_ABL_{abl_label}'] = ild2_delay_coeff_ms_per_unit2_by_abl[abl_idx]
        posterior_param_labels[f'bias_ms_ABL_{abl_label}'] = f'bias ABL {abl_label} (ms)'
        posterior_param_labels[f'abs_ild_coeff_ABL_{abl_label}'] = f'|ILD| coeff ABL {abl_label} (ms/unit)'
        posterior_param_labels[f'ild2_coeff_ABL_{abl_label}'] = f'ILD^2 coeff ABL {abl_label} (ms/unit^2)'

    save_posterior_summary_page(
        pdf_pages=pdf,
        title=f'Alpha-Normalized Tied ABL-Specific ILD2-Delay Model - Posterior Means ({animal})',
    ...
    delay_coeff_summary_rows = []
    for abl_idx, abl in enumerate(delay_abl_levels):
        bias_ci = np.percentile(bias_ms_by_abl_samples[:, abl_idx], [2.5, 97.5])
        abs_ild_ci = np.percentile(abs_ild_delay_coeff_ms_per_unit_by_abl_samples[:, abl_idx], [2.5, 97.5])
        ild2_ci = np.percentile(ild2_delay_coeff_ms_per_unit2_by_abl_samples[:, abl_idx], [2.5, 97.5])
        delay_coeff_summary_rows.append({
            'ABL': format_abl_suffix(abl),
            'bias_ms mean [2.5%, 97.5%]': f"{bias_ms_by_abl[abl_idx]:.3f} [{bias_ci[0]:.3f}, {bias_ci[1]:.3f}]",
            '|ILD| coeff mean [2.5%, 97.5%]': (
                f"{abs_ild_delay_coeff_ms_per_unit_by_abl[abl_idx]:.3f} "
                f"[{abs_ild_ci[0]:.3f}, {abs_ild_ci[1]:.3f}]"
            ),
            'ILD^2 coeff mean [2.5%, 97.5%]': (
                f"{ild2_delay_coeff_ms_per_unit2_by_abl[abl_idx]:.3f} "
                f"[{ild2_ci[0]:.3f}, {ild2_ci[1]:.3f}]"
            ),
        })
    delay_coeff_summary_df = pd.DataFrame(delay_coeff_summary_rows)
    render_df_to_pdf(delay_coeff_summary_df, f"ABL-Specific Delay Coefficients - Animal {animal}", pdf)
    ...
    vbmc_norm_alpha_abl_specific_ild2_delay_tied_results = {
        ...
        'delay_abl_levels': delay_abl_levels.copy(),
        'bias_ms_by_abl_samples': bias_ms_by_abl_samples,
        'abs_ild_delay_coeff_ms_per_unit_by_abl_samples': abs_ild_delay_coeff_ms_per_unit_by_abl_samples,
        'ild2_delay_coeff_ms_per_unit2_by_abl_samples': ild2_delay_coeff_ms_per_unit2_by_abl_samples,
        'message': results['message'],
        'elbo': results['elbo'],
        'elbo_sd': results['elbo_sd'],
        'loglike': norm_alpha_tied_loglike
    ...
    theory_results_up_and_down, theory_time_axis, bins, bin_centers = plot_rt_distributions(
        ...
        animal, pdf, model_name="Alpha-Normalized Tied ABL-Specific ILD2 Delay"
    )

    plot_tachometric_curves(
        sim_df_1, data_df_1, ILD_arr, ABL_arr, theory_results_up_and_down,
        theory_time_axis, bins, animal, pdf, model_name="Alpha-Normalized Tied ABL-Specific ILD2 Delay"
    )
    ...
    if tied_df is not None:
        ...
        print(f"Added alpha-normalized TIED ABL-specific ILD2-delay results table to PDF for animal {animal}")
