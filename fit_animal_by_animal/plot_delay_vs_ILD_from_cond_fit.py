# %%
"""
Plot t_E_aff vs ILD for all animals from 5-param condition fit.
- One figure per abs_ILD
- Y-axis: animal
- X-axis: t_E_aff (sensory delay)
...
# =============================================================================
def get_t_E_aff_from_pkl(batch_name, animal_id, ABL, ILD):
    """
    Load t_E_aff from 5-param condition-by-condition fit pkl file.
    Returns mean of vp samples for t_E_aff (index 2).
    """
...
# %%
# =============================================================================
# Collect t_E_aff for all animals, all ILDs, all ABLs
# =============================================================================
# Structure: {abs_ild: {abl: [(animal_label, t_E_aff), ...]}}
...
for batch_name, animal_id in batch_animal_pairs:
    ...
    for abs_ild in ILD_VALUES:
        for abl in ABL_arr:
            # Get t_E_aff for +ILD and -ILD, then average
            t_E_aff_pos = get_t_E_aff_from_pkl(batch_name, animal_id, abl, abs_ild)
            t_E_aff_neg = get_t_E_aff_from_pkl(batch_name, animal_id, abl, -abs_ild)
            
            if t_E_aff_pos is not None and t_E_aff_neg is not None:
                t_E_aff_avg = (t_E_aff_pos + t_E_aff_neg) / 2
            elif t_E_aff_pos is not None:
                t_E_aff_avg = t_E_aff_pos
            elif t_E_aff_neg is not None:
                t_E_aff_avg = t_E_aff_neg
            else:
                t_E_aff_avg = None
            
            if t_E_aff_avg is not None:
                delay_data[abs_ild][abl].append((animal_label, t_E_aff_avg))
...
for abs_ild in ILD_VALUES:
    ...
    ax.set_xlabel('t_E_aff (s)', fontsize=12)
    ax.set_ylabel('Animal', fontsize=12)
    ax.set_title(f't_E_aff for |ILD| = {abs_ild} dB', fontsize=14)
    ax.legend(loc='upper right')
    ax.grid(axis='x', alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(f't_E_aff_vs_animal_ILD_{abs_ild}.png', dpi=150, bbox_inches='tight')
    plt.show()
    print(f"Saved: t_E_aff_vs_animal_ILD_{abs_ild}.png")
