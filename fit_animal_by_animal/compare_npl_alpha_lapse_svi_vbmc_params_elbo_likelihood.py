# %%
"""
Compare suspicious NPL+alpha+lapse SVI reruns against refreshed old VBMC NPL+lapse.
...
VBMC is the old no-alpha NPL+lapse fit, so it is represented as alpha=1 when
using the common NPL+alpha+lapse likelihood.
"""
...
VBMC_ROOT = SCRIPT_DIR / "vbmc_npl_lapse_four_animal_rerun"

OUTPUT_DIR = VBMC_ROOT / "comparison_to_svi"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

PARAM_FIG = OUTPUT_DIR / "npl_alpha_lapse_svi_vbmc_parameter_means_ci.png"
OBJECTIVE_FIG = OUTPUT_DIR / "npl_alpha_lapse_svi_vbmc_elbo_likelihood.png"
PARAM_CSV = OUTPUT_DIR / "npl_alpha_lapse_svi_vbmc_parameter_means_ci.csv"
OBJECTIVE_CSV = OUTPUT_DIR / "npl_alpha_lapse_svi_vbmc_elbo_likelihood.csv"
VALIDATION_CSV = OUTPUT_DIR / "npl_alpha_lapse_svi_vbmc_vbmc_likelihood_validation.csv"
...
def load_summary(model_spec, batch_name, animal):
    ...
    if model_spec["kind"] == "svi":
        ...
        path = model_spec["root"] / key / "vbmc_norm_lapse_posterior_summary.csv"
    if not path.exists():
...
def parameter_stats(summary_df, parameter, model_kind):
    if parameter == "alpha" and model_kind == "vbmc":
        return VBMC_ALPHA, VBMC_ALPHA, VBMC_ALPHA, False
...
def old_vbmc_scalar_loglike(valid_df, abort_params, T_trunc, params):
...
def vbmc_elbo_from_summary(batch_name, animal):
    path = VBMC_ROOT / animal_key(batch_name, animal) / "vbmc_norm_lapse_run_summary.csv"
    if not path.exists():
        raise FileNotFoundError(path)
...
for batch_name, animal in ANIMALS:
    ...
    for model_spec in MODEL_SPECS:
        ...
        objective_rows.append(
            {
                ...
                "common_likelihood_alpha_policy": "VBMC alpha fixed to 1.0; SVI alpha from posterior mean",
            }
        )
        ...
        if model_spec["kind"] == "vbmc":
            ...
            old_loglike = old_vbmc_scalar_loglike(validation_df, abort_params, T_trunc, validation_params)
            validation_rows.append(
                {
                    ...
                    "T_trunc": float(T_trunc),
                    "alpha_used_in_common_likelihood": VBMC_ALPHA,
                    "vbmc_scalar_t_E_aff_s": scalar_delay,
                    "old_numpy_loglike": float(old_loglike),
                    "common_jax_alpha1_loglike": float(jax_loglike),
                    "abs_diff": float(abs(old_loglike - jax_loglike)),
                }
            )
...
fig.suptitle("NPL+alpha+lapse SVI vs refreshed old VBMC NPL+lapse parameter means", fontsize=14, y=0.99)
...
fig.legend(handles=handles, loc="lower center", ncol=3, frameon=False)
fig.suptitle("NPL+alpha+lapse SVI/VBMC objectives and common likelihoods", fontsize=14, y=0.98)
