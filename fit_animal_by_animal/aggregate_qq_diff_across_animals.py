...
for i, abs_ILD in enumerate(abs_ILDs):
    ...
    for abl in [20, 40]:
        ...
        for idx, row in enumerate(diffs):
            ...
            ax.plot(Q_60, row, color=colors[abl], alpha=0.5, linewidth=1)
        ...
        ax.plot(mean_Q_60, mean_diff, color=mean_colors[abl], label=labels[abl], linewidth=2)
        # Fit line to mean curve
        mask = ~np.isnan(mean_Q_60) & ~np.isnan(mean_diff)
        if np.sum(mask) > 1:
            slope, intercept, r_value, p_value, std_err = linregress(mean_Q_60[mask], mean_diff[mask])
            y_fit = slope * mean_Q_60 + intercept
            ax.plot(mean_Q_60, y_fit, linestyle=':', color=mean_colors[abl], linewidth=2)
            fit_texts.append(f"{labels[abl]}: slope={slope:.3f}, int={intercept:.3f}, R²={r_value**2:.3f}")
...
# Unified legend
# legend_elements = [
#     Line2D([0], [0], color=colors[20], lw=2, label='Indiv. 20-60', alpha=0.3),
#     Line2D([0], [0], color=mean_colors[20], lw=2, label='Mean±STD 20-60'),
#     Line2D([0], [0], color=colors[40], lw=2, label='Indiv. 40-60', alpha=0.3),
#     Line2D([0], [0], color=mean_colors[40], lw=2, label='Mean±STD 40-60'),
# ]
