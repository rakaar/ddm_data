# %%
"""
Compare w and del_go between fixed-condition-t_E_aff and ABL-specific ILD2-delay
NPL+alpha fits.

Each point is an animal-wise variational posterior mean; error bars are the
2.5% to 97.5% posterior sample quantiles.
"""
...
OUTPUT_DIR = SCRIPT_DIR / "fixed_condition_t_E_aff_vs_abl_specific_ild2_w_del_go_comparison"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

PARAM_VALUES_CSV = OUTPUT_DIR / "fixed_condition_t_E_aff_vs_abl_specific_ild2_w_del_go_values.csv"
PARAM_DIFF_CSV = OUTPUT_DIR / "fixed_condition_t_E_aff_minus_abl_specific_ild2_w_del_go_differences.csv"
SUMMARY_CSV = OUTPUT_DIR / "fixed_condition_t_E_aff_vs_abl_specific_ild2_w_del_go_summary.csv"
FIG_PNG = OUTPUT_DIR / "fixed_condition_t_E_aff_vs_abl_specific_ild2_w_del_go.png"
FIG_PDF = OUTPUT_DIR / "fixed_condition_t_E_aff_vs_abl_specific_ild2_w_del_go.pdf"
...
PARAM_SPECS = [
    {
        "param": "del_go_ms",
        "sample_key": "del_go_samples",
        "scale": 1e3,
        "ylabel": "del_go (ms)",
    },
    {
        "param": "w",
        "sample_key": "w_samples",
        "scale": 1.0,
        "ylabel": "w",
    },
]
...
# %%
# =============================================================================
# Load posterior means and intervals
# =============================================================================
rows = []
...
# %%
# =============================================================================
# Pair models and summarize differences
# =============================================================================
animal_order_df = (
...
for model_config in MODEL_CONFIGS:
    if model_config["model"] not in counts.index:
        raise RuntimeError(f"No values loaded for {model_config['label']}")
    for param_spec in PARAM_SPECS:
        if counts.loc[model_config["model"], param_spec["param"]] != EXPECTED_N_ANIMALS:
            raise RuntimeError(f"Incomplete {param_spec['param']} values for {model_config['label']}")
...
if wide_df[["fixed_condition_t_E_aff", "abl_specific_ild2_delay"]].isna().any().any():
    raise RuntimeError("Missing paired parameter values after pivot.")
...
fig.suptitle("w and del_go comparison: fixed condition t_E_aff vs ABL-specific ILD2", y=1.08)
