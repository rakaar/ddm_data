...
def extract_params_from_norm_lapse_pkl(pkl_path, n_samples):
    """
    Extract theta_E and lapse_prob samples from norm+lapse VBMC pickle file.
    ...
    Returns:
        dict with 'theta_samples' and 'lapse_prob_samples' arrays (or None if extraction fails)
    """
    try:
...
def load_theta_vs_lapse_data(n_samples_per_animal=5000):
    """Load theta_E and lapse_prob samples from all animals."""
...
def plot_theta_vs_lapse(animal_data, ellipse_quantile=0.95, animals_by_color=False, exclude_top_lapse_n=0):
    """
    Create scatter plot with samples from each animal's posterior.
    Shows theta_E vs lapse_prob with linear fit, correlation, and covariance ellipses.
    """
    if len(animal_data) == 0:
        print("No data to plot!")
        return None

    if exclude_top_lapse_n > 0:
        lapse_means = np.array([np.mean(d['lapse_prob_samples']) for d in animal_data])
        sorted_idx = np.argsort(lapse_means)[::-1]
        remove_count = min(exclude_top_lapse_n, len(animal_data))
        exclude_idx = set(sorted_idx[:remove_count])
        if exclude_idx:
            print("\nExcluding top lapse animals (by mean lapse rate):")
            for idx in sorted_idx[:remove_count]:
                entry = animal_data[int(idx)]
                print(
                    f"  {entry['batch']}_{entry['animal']}: "
                    f"{lapse_means[int(idx)] * 100:.2f}%"
                )
            animal_data = [d for idx, d in enumerate(animal_data) if idx not in exclude_idx]

    # Compute mean per animal for statistics
    theta_means = [np.mean(d['theta_samples']) for d in animal_data]
    ...
    for idx, d in enumerate(animal_data):
        n_samples = len(d['theta_samples'])
        all_theta.extend(d['theta_samples'])
    ...
    correlation = np.corrcoef(all_theta, all_lapse_prob_pct)[0, 1]

    # Fit a linear regression line using all samples (use percentage values)
    z = np.polyfit(all_theta, all_lapse_prob_pct, 1)
    p = np.poly1d(z)
    x_line = np.linspace(np.min(all_theta), np.max(all_theta), 100)
    y_line = p(x_line)
    ...
    if animals_by_color:
        for idx in range(n_animals):
            ...
            ax.scatter(all_theta[mask], all_lapse_prob_pct[mask],
                       alpha=0.15, s=20, c=[animal_colors[idx]], edgecolors='none')
    else:
        ax.scatter(all_theta, all_lapse_prob_pct, alpha=0.15, s=20,
    ...
    ax.set_xlabel(r'$\theta_E$', fontsize=20)
    ax.set_ylabel('Lapse rate (%)', fontsize=20)

    x_min = float(np.nanmin(all_theta))
    x_max = float(np.nanmax(all_theta))
    ...
    output_path = os.path.join(BASE_DIR, 'theta_vs_lapse_prob_scatter_with_ellipses.png')
    ...
    print("=" * 60)
    print(f"theta_E:  mean={np.mean(all_theta):.4f}, "
          f"std={np.std(all_theta):.4f}, "
          f"min={np.min(all_theta):.4f}, "
          f"max={np.max(all_theta):.4f}")
    ...
    print("=" * 60)
    print(f"theta_E:  mean={np.mean(theta_means):.4f}, "
          f"std={np.std(theta_means):.4f}, "
          f"min={np.min(theta_means):.4f}, "
          f"max={np.max(theta_means):.4f}")
    ...
    correlation_means = np.corrcoef(theta_means, lapse_prob_means)[0, 1]
    ...
    plot_data = {
        ...
        'statistics': {
            'all_samples': {
                'theta_E': {
                    'mean': np.mean(all_theta),
                    'std': np.std(all_theta),
                    'min': np.min(all_theta),
                    'max': np.max(all_theta),
                },
                'lapse_rate_pct': {
            ...
            'animal_means': {
                'theta_E': {
                    'mean': np.mean(theta_means),
                    'std': np.std(theta_means),
                    'min': np.min(theta_means),
                    'max': np.max(theta_means),
                },
                'lapse_rate_pct': {
    ...
    pkl_output_path = os.path.join(BASE_DIR, 'theta_vs_lapse_prob_data.pkl')
...
if __name__ == '__main__':
    print("\n" + "=" * 60)
    print("LOADING THETA VS LAPSE RATE DATA")
    print("=" * 60)
    data = load_theta_vs_lapse_data()
    plot_theta_vs_lapse(data, animals_by_color=True, exclude_top_lapse_n=1)
