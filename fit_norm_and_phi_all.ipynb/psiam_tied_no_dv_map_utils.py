...
###### EA - rate normalization #################
def rho_E_minus_small_t_NORM_rate_norm_fn(t, ABL, ILD, rate_lambda, T_0, theta_E, Z_E, bound, rate_norm_l, K_max):
    """
    ...
    omega = (1 / (T_0 * (theta_E ** 2))) * (10 ** (rate_lambda * (1 - rate_norm_l) * ABL / 20))
    t = omega * t # tau = omega  * t
...
def CDF_E_minus_small_t_NORM_rate_norm_fn(t, ABL, ILD, rate_lambda, T_0, theta_E, Z_E, bound, rate_norm_l, K_max):
    """
...
def cum_E_t_rate_norm_fn(t, ABL, ILD, rate_lambda, T_0, theta_E, Z_E, rate_norm_l, K_max):
    return CDF_E_minus_small_t_NORM_rate_norm_fn(t, ABL, ILD, rate_lambda, T_0, theta_E, Z_E, 1, rate_norm_l, K_max) + \
        CDF_E_minus_small_t_NORM_rate_norm_fn(t, ABL, ILD, rate_lambda, T_0, theta_E, Z_E, -1, rate_norm_l, K_max)

def up_or_down_RTs_fit_fn(t, V_A, theta_A, ABL, ILD, rate_lambda, T_0, theta_E, Z_E, t_stim, t_A_aff, t_E_aff, del_go, rate_norm_l, bound, K_max):
    """
    PDF of up RTs array
    """
    P_A = rho_A_t_fn(t-t_A_aff, V_A, theta_A)
    prob_EA_hits_either_bound = CDF_E_minus_small_t_NORM_rate_norm_fn(t - t_stim - t_E_aff + del_go, ABL, ILD, rate_lambda, T_0, theta_E, Z_E, 1, rate_norm_l, K_max) + \
                                CDF_E_minus_small_t_NORM_rate_norm_fn(t - t_stim - t_E_aff + del_go, ABL, ILD, rate_lambda, T_0, theta_E, Z_E, -1, rate_norm_l, K_max)
    prob_EA_survives = 1 - prob_EA_hits_either_bound
    random_readout_if_EA_surives = 0.5 * prob_EA_survives
    t2 = t - t_stim - t_E_aff + del_go
    t1 = t - t_stim - t_E_aff

    P_E_plus_cum = CDF_E_minus_small_t_NORM_rate_norm_fn(t2, ABL, ILD, rate_lambda, T_0, theta_E, Z_E, bound, rate_norm_l, K_max)  - \
                    CDF_E_minus_small_t_NORM_rate_norm_fn(t1, ABL, ILD, rate_lambda, T_0, theta_E, Z_E, bound, rate_norm_l, K_max) 

    P_E_plus = rho_E_minus_small_t_NORM_rate_norm_fn(t-t_E_aff-t_stim, ABL, ILD, rate_lambda, T_0, theta_E, Z_E, bound, rate_norm_l, K_max)
    C_A = cum_A_t_fn(t-t_A_aff, V_A, theta_A) 

    return (P_A*(random_readout_if_EA_surives + P_E_plus_cum) + P_E_plus*(1-C_A))
